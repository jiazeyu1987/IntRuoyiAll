...
- BDD: int_main 真实页面可信时间证据导出 -> Given 当前 `int_main` 已融合可信时间实现且 48081 后端基于当前提交重启 / When 管理员通过真实前端菜单进入“基础设施 > 监控中心 > 运行控制台”，点击“执行巡检”后再点击“导出时间戳证据” / Then 页面显示正式服与审查服可信时间项，浏览器真实下载当前巡检证据 ZIP，ZIP 固定三文件和 SHA-256 可离线校验。
- INT_MAIN E2E BUILD: 从当前 `int_main` HEAD `e8f572f2e` 创建干净 detached worktree `D:\IntRuoyiWorktree\int_main_e2e_build_20260908_current`，执行 `mvn.cmd -pl yudao-server -am -DskipTests package` -> PASS，`BUILD SUCCESS`，耗时 04:48，完成时间 `2026-09-08T18:41:58+08:00`。
- INT_MAIN E2E RUNTIME: 运行包 `E:\IntRuoyi\output\runtime\int_main\backend-int-main-e2e-20260908-184158.jar`，SHA-256 `64E795FD7D11771751AE5EDB9E72D3E9D81A7175D4E4408BFFD449D6029425A6`；nested jar 校验包含 `RuntimeTrustedTimeCollectorImpl`、`RuntimeTrustedTimeParser`、`RuntimeTimeEvidenceExporter`、`RuntimeControlTrustedTimeRespVO` 及 MES 归档/签名服务类。
- INT_MAIN BACKEND RESTART: 先启动构建产物 `E:\IntRuoyi\output\runtime\int_main\backend-int-main-e2e-20260908-184158.jar`；真实 E2E 前复核 48081 当前活跃 PID 为 `45676`，命令行 Jar 为 `E:\IntRuoyi\output\runtime\int_main\backend-runtime-control-20260908-185534.jar`，SHA-256 `C27197556A7ABF822B9958F6AF234020E4DFBAF67E29AB46896D7E46C6811644`，nested jar 同样包含可信时间 collector/parser/exporter 和 MES 签名/归档类；健康检查 HTTP 200，内容等价于 `{"status":"UP"}`。
- INT_MAIN THRESHOLD: 当前运行环境未配置有效 `INTRUOYI_TRUSTED_TIME_MAX_OFFSET_MILLIS`，真实导出的两项证据 `maxOffsetMillis=null`，未启用 `100 ms` 限制。
- GREEN: 真实 Playwright E2E -> PASS。通过真实登录页、真实菜单点击进入运行控制台，点击“执行巡检”触发自然 `POST /admin-api/infra/runtime-control/inspection-runs` HTTP 200、业务 code 0、保存巡检 ID `2`；页面断言显示“正式服”“审查服”，且不显示“备份服”“备份服务器”“备用服务器”；点击“导出时间戳证据”触发自然 ZIP 下载 HTTP 200。
- GREEN: ZIP 离线校验 -> PASS。下载文件 `E:\IntRuoyi\output\playwright\int-main-e2e-trusted-time-20260908-184158\可信时间证据_巡检2.zip` 包含 `审查摘要.html`、`原始证据.json`、`SHA256SUMS.txt`；`审查摘要.html` SHA-256 `96955b613c9f954ec940d7089ac363c4c5ba7e78ad69b7eb0fb37c917f0e42c1` 匹配，`原始证据.json` SHA-256 `30887d886d64c1bbf7a200bc831142928689edaace50de5bd6b5853fce72c014` 匹配。
- INT_MAIN E2E RESULT: 巡检 ID `2` 整体状态 `NO_GO`；整体不放行来自运行控制台其它相邻检查，不伪造成 PASS。目标可信时间项均为 PASS：正式服 `172.30.30.57`，时间源 `139.199.214.202`，Last/RMS `0.042953/0.59853 ms`，`maxOffsetMillis=null`；审查服 `172.30.30.59`，时间源 `139.199.214.202`，Last/RMS `-0.123604/0.380689 ms`，`maxOffsetMillis=null`。
- INT_MAIN E2E OBSERVATION: Playwright `pageErrors=0`；console 中 2 条审批待办数量加载“系统异常”和 2 条外部图片证书错误，failedRequests 为 2 个外部 `s1.ax1x.com` 图片证书错误，均非本次运行控制台巡检/导出目标请求。
