...
- 本文件是修复1实现后的独立验证合同；本轮不执行数据库写入或真实写入型 E2E，后端 Maven 因环境缺失未执行。
...
源码检索、任务目录阅读和文档结构扫描属于审计/结构证据，不计为 TDD RED 或 GREEN；生产实现的 RED、GREEN、REGRESSION 仅在后续任务运行并记录。

### RED

- `mvn -pl yudao-module-mes -am -Dtest=MesTeamLeaderActiveOrderPickListBindingTest test` -> 预期 FAIL：当前请求没有 `pickListId/idempotencyKey`，不存在绑定聚合和事务唯一性。
- `node IntRuoyiFronted/src/api/mes/pro/processpool/teamLeaderActiveOrderPickListBinding.static.spec.cjs` -> 预期 FAIL：当前 DTO、提交调用和弹窗没有领料单候选。
- `python scripts/tests/test_active_order_pick_list_binding_schema.py` -> 预期 FAIL：当前库/迁移没有三张正式关系表和条件唯一索引。

本轮不运行上述命令；`execution-log.md` 只记录预期 RED，不得预写 PASS。

### GREEN

- 增加最小后端 VO/BO、正式来源端口、绑定 service/mapper/DO、事务和唯一约束后，重复运行 T1-T8，逐个记录真实 PASS。
- 增加前端候选查询、领料单选择、提交 payload 和 blocker 展示后，运行 T9 并记录 PASS。
- 由流程修复6、7、9完成消费者后，运行 T11-T13，逐值核对 binding ID、明细 ID、哈希和状态 owner。

前端静态合同已完成 GREEN；后端 GREEN/REGRESSION 仍受 Maven 缺失阻断，不以文档审阅替代。

### REGRESSION

- `mvn -pl yudao-module-mes -am -Dtest=MesTeamLeaderActiveOrderServiceTest,MesTeamLeaderActiveOrderErpPlannedStartTest test`：确认旧的加入、排序、移除、重建和 QA 快照行为不被破坏；历史复用必须增加领料单兼容性断言。
- `pnpm --dir IntRuoyiFronted ts:check`：确认 API 类型、活跃订单列表、放行 blocker 类型无 TS 回归。
...
## Expected Blockers for This Planning Task

- 目标接口、绑定表和来源消费代码已实现；后端 GREEN/REGRESSION 命令因 Maven/Maven Wrapper 缺失为 NOT RUN。
- 流程修复6/7/9 owner 和 schema owner 未确认。
- 没有确认测试租户/账号/真实审核领料单/可清理数据，因此不执行真实 E2E。
