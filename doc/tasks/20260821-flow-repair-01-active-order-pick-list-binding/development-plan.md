...
| 位置 | 当前事实 | 影响 |
| --- | --- | --- |
| `IntRuoyiBackend/.../MesTeamLeaderActiveOrderAddReqVO.java` | 请求只有必填 `Long workOrderId` | 前端无法表达用户选择的领料单，后端无法验证用户选择和幂等载荷 |
| `IntRuoyiBackend/.../MesTeamLeaderActiveOrderAddReqBO.java` | BO 只有 `leaderUserId`、`workOrderId` | 服务层只能按工单处理 |
| `IntRuoyiBackend/.../MesProcessPoolActiveOrderDO.java` | 活跃订单保存工单、路线、QA、数量和状态，没有领料单绑定列/关系 | 活跃订单没有正式领料单身份或快照 |
| `MesTeamLeaderActiveOrderServiceImpl.addActiveOrder` | 先校验工单，再按工单读取历史；已有记录可直接 `REUSE` 或 `RECOVER`；新建只写工单/路线/QA快照 | 同工单重复加入不比较领料单；历史恢复会绕过领料单绑定 |
| `MesProcessPoolActiveOrderMapper` | 唯一/查询主要按 `workOrderId + routeId + routeVersionId + activeStatus` | 唯一性没有覆盖领料单关系 |
| `IntRuoyiFronted/src/api/mes/pro/processpool/teamLeader.ts` | `TeamLeaderActiveOrderAddReqVO` 只有 `workOrderId` | UI/API 契约缺少领料单候选和绑定回执 |
| `TeamLeaderWorkbenchPage.vue` | 新增弹窗只有订单号/产品远程选择，提交调用 `addTeamLeaderActiveOrder({ workOrderId })` | 用户无法查看或确认领料单号、审核状态、物料摘要 |
| `MesProductionPickListSourceServiceImpl.resolveValue` | 放行批记录映射时按 `productionOrderNo` 查明细，筛选已审核头 `documentStatus=C`，要求唯一头；同物料多明细按 `sourceEntryId` 升序取第一条 | 领料单关系发生在放行时临时反查，不是活跃订单正式绑定；来源变化会改变结果 |
| `MesProEdhrBatchExecutionDO` / `MesProEdhrBatchExecutionServiceImpl` | 批次执行有工单、批号、路线和 `activeContextKey`，没有领料单关系；创建入口可按工单/批号/路线复用 | 批次执行无法稳定证明其使用了哪张领料单和哪组明细 |
| `MesTeamLeaderActiveOrderReleaseBatchRecordWriterImpl` | 只有在放行 writer 计划阶段才收集 `pickListId`、`pickListItemId` 和 `evidenceHash` | 领料单证据太晚形成，不能作为加入和批次创建的前置事实 |
