...
function Replace-Required {
  ...
  $candidateOld = $Old
  $candidateNew = $New
  if (-not $script:source.Contains($candidateOld)) {
    $candidateOld = $Old.Replace("`n", "`r`n")
    $candidateNew = $New.Replace("`n", "`r`n")
  }
  if (-not $script:source.Contains($candidateOld)) {
    throw "Missing expected token: $($Old.Substring(0, [Math]::Min(80, $Old.Length)))"
  }
  $script:source = $script:source.Replace($candidateOld, $candidateNew)
}
