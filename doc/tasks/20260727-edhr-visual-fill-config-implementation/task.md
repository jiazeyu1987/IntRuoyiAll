...
9. `blocked`：T09 真实用户路径 E2E，当前运行态、测试租户、管理员和两名员工账号、显式写入授权已就绪；剩余阻塞为缺少可同时满足 DCC 项目名、MES 批次物料、工艺路线产品绑定和金蝶生产订单创建的正式路线产品夹具。
10. `pending`：回归验证、提交推送、融合 `int_main`、融合后完整 E2E。
...
blocked：T01-T08 已完成静态/后端聚焦验证；2026-07-27 复核确认 8083/48083 运行态仍在线且健康，且真实 E2E 已改为读取本地未跟踪 `local-input.json`，不再依赖 `EDHR_VISUAL_FILL_*` 环境变量；T09 继续阻塞于正式路线产品夹具，当前候选 `IDI`、`ID`、`CODXVFC20260726` 均无法在金蝶创建生产订单，`A001.02.092.60011` 虽可作为金蝶物料候选但 MES 物料名与 DCC 项目名不一致且未启用批次绑定，不能安全导入任务批记录路线。
