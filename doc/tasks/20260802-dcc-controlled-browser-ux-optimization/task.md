...
- 仅修改 DCC 受控浏览、DCC 受控文件详情/预览、DCC 原版上传/审批闭环中与本需求直接相关的前端展示和前端静态契约。
- 不修改数据库状态、权限配置、后端接口契约或其它业务场景。
...
- Git 共享分支门禁：当前工作区存在大量非本任务脏改动和 ahead 状态，本任务只触碰明确范围；提交/推送若无法隔离则记录 blocker。
