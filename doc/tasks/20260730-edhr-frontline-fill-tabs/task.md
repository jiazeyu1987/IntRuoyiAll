...
- GitHub `origin/int_main` 后续新增 `d1ffcef8 docs: complete frontend backend commit closeout`；临时集成分支已合入该远端主线并复跑 SQL 契约、DCC 目标 Maven、eDHR 后端 Maven、前端静态合同、E2E 脚本语法检查和 `pnpm ts:check`。
- 最终验证 HEAD `8a34cfe3` 已推送到 `origin/int_main`；cleanup preview/apply 已清理 DCC 临时 evidence，原任务 worktree 与临时集成 worktree 已删除，端口登记已释放。
