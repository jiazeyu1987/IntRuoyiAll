...
- 本轮追加：最大化填写工作区后，保存草稿结果弹框、提交执行签名弹框和提交结果弹框渲染在 `.edhr-fill-workspace` 内部，并禁用 body teleport，避免被浏览器全屏层遮挡。
...
- 本轮未启动本地服务或真实 Playwright；遮挡根因通过 DOM 挂载静态合同覆盖，后续如需截图验收需先确认本地 8081/48081 运行态。
