...
- completed：MES 单元格链接服务类回归 `mvn -pl yudao-module-mes "-Dtest=cn.iocoder.yudao.module.mes.service.pro.batchrecordcelllink.MesProBatchRecordCellLinkServiceImplTest" test` PASS，45 tests。
- completed：`batch-record-cell-link-process-pool-real-device-readonly.e2e.cjs` 扩展可选创建后清理模式并记录 `sourceCellKey`；`node --check tests\e2e\mes\batch-record-cell-link-process-pool-real-device-readonly.e2e.cjs` PASS。真实运行仍缺少本机登录密码前置，未执行写入/清理模式。
