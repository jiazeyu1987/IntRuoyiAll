...
- REGRESSION BLOCKED: `pnpm ts:check` -> FAIL，既有 DCC/NAS 类型错误阻塞，失败文件不在本任务范围。
- REGRESSION BLOCKED: `node tests/e2e/edhr-frontline-fill-tabs-static.spec.cjs` -> FAIL，既有 PQC 文案断言阻塞，失败文件不在本任务范围。
...
- blocked: Git closeout 未完成，当前分支已有 ahead 6 和非本任务多项脏改动，不能安全代替并行任务提交/推送。

## Blockers

- 当前主工作区已有非本任务脏改动；提交/推送阶段必须避免混入并行任务文件，必要时按项目 Git 门禁记录或阻塞。
- 全量 `pnpm ts:check` 被非本任务 DCC/NAS 类型错误阻塞；当前任务用专用静态合同和相邻组长/关系图合同证明本需求。
- 旧相邻 `edhr-frontline-fill-tabs-static.spec.cjs` 被非本任务 PQC 文案断言阻塞。
- Git closeout 阻塞：`git status --short --branch` 显示 `int_main...origin/int_main [ahead 6]`，且同时存在 DCC、NAS、pressure-pump、backend frontline、其它任务文档等非本任务改动；本任务不执行宽泛 baseline/commit/push，避免混入并行任务产物。
- 经验沉淀：已按 `project-experience-consolidation` 搜索 `docs/*memory*.md`、`docs/frontend-development.md` 和 `docs/experience-index.md`，本次没有新增通用门禁；既有“前端权限页签正向授权门禁”和“前端静态契约隔离门禁”已覆盖本任务经验。
