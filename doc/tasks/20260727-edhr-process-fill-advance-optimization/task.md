...
- [ ] M1: 审计现有工作任务、批次任务、动态表单和工序推进链路
- [ ] M2: 记录 BDD 场景并新增 RED 回归测试覆盖多填写人工作台和推进资格
- [ ] M3: 实施后端数据/权限/推进逻辑优化，不引入负责人或 fallback
- [ ] M4: 实施前端个人工作台和统一进入填写工作区优化
- [ ] M5: 运行后端、前端静态合同和完整真实数据 E2E 验证
- [ ] M6: 完成证据记录、经验沉淀和收尾门禁
