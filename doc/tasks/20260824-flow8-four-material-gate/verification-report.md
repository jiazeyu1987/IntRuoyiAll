...
- PASS: a source snapshot change now transitions the gate to `MATERIALS_RECHECK_REQUIRED`; it remains blocked while persisted task route bindings reference the old snapshot and only returns `MATERIALS_READY` after those bindings are refreshed. The gate no longer relies on process-local memory.
