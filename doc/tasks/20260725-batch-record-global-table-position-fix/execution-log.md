...
- SCREENSHOT: `mvn -pl yudao-module-mes -am "-Dtest=MesProBatchRecordPressurePumpVisualArtifactTest" "-Dsurefire.failIfNoSpecifiedTests=false" test` -> PASS，1 test, 0 failures；生成 `artifacts/verification-screenshots/light-cure-one-verification.png` 与 `clean-detail-boundary-verification.png`，随后删除临时 JUnit 生成器，仅保留截图/HTML 工件。
