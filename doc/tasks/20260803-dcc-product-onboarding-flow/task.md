...
1. 建立任务文档、适用门禁、BDD 场景和 RED/GREEN 证据结构。
2. 定位现有 DCC 项目代码、MDM 产品、DCC 受控文件提交、前端基础数据入口和测试结构。
3. 写 RED：后端服务/前端静态契约证明当前缺少产品建档申请、审批通过生成 DCC 项目代码、MDM 绑定和页面入口。
4. 实现最小正式链路：数据库字段/表、后端 API/服务、唯一性与状态校验、受控文件提交 MDM 绑定、前端入口。
5. 运行 GREEN：后端定向测试、数据库/前端静态契约和必要类型/编译验证。
6. 更新验证报告、任务状态和收尾证据；若提交/推送被无关脏工作区阻塞，按项目策略记录 blocker。
