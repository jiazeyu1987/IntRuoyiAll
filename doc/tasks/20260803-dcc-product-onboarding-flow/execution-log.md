...
- Current merge boundary: worktree branch is `codex/dcc-product-onboarding-flow-20260803`, currently behind `origin/int_main` by 5 commits after verification; merge back to `int_main` must first rebase/merge latest remote state and pass branch runtime port guard. Main workspace still contains many unrelated dirty files and must not be modified or committed as part of this task.
