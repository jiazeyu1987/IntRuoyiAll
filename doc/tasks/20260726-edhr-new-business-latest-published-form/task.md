...
修复个人工作台进入 eDHR 新业务待办时因批次或任务绑定了非最新已发布批记录表单而报“当前 eDHR 批次状态不允许该操作”的问题。新建批次、返工或其它新业务只能冻结并使用对应批记录定义下最新的已发布版本；历史业务继续使用既有快照，不回写或静默升级。

## Milestones

- [ ] 定位工作台进入处理、批次创建/返工和批记录版本冻结链路
- [ ] 补充 BDD 与失败回归测试，证明新业务仍可能使用旧发布版本
- [ ] 实施最小后端修复并保持历史快照、终态批次 fail-fast
- [ ] 运行后端定向回归及必要的前端静态/真实路径验证
- [ ] 完成提交、推送和任务收尾
...
- `是否从根因和长期维护角度解决`：是；版本选择在新业务创建边界统一收敛，历史业务继续尊重冻结快照。
- `是否存在临时补丁或绕过`：否。
