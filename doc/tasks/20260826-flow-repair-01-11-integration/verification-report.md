...
- 正常目标分支提交：已完成，当前 HEAD 为 `a5057c7a2fe83d4826d24e09626ce123cf4576cc`；不使用 `--no-verify`。
...
代码融合提交已经完成，当前目标 HEAD 为 `a5057c7a2fe83d4826d24e09626ce123cf4576cc`；`int_main` 已原子 fast-forward 到同一提交。流程4旧双写、流程7 AFTER_COMMIT witness handoff、流程8 receipt 持久化和 Stage2.5 receipt handoff 已补齐；但 `E:/IntRuoyi` 主工作树仍有未提交 overlay，不能把主工作树当前运行文件或定向测试写成流程1-11全链路完成。
