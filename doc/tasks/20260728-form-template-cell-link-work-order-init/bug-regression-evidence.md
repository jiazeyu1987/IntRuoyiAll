...
表单模板页签点击“链接”后，批记录单元格链接工作台先提示“批记录单元格链接引用的表单不存在：PRODUCTION_WORK_ORDER”。修复入口初始化后，继续加载目标模板时提示“批记录表单布局 JSON 无效：FORMTPL:32”。

## Expected Behavior

表单模板链接入口默认来源为生产工单时，工作台应直接显示生产工单字段列表，并允许把“生产批号”等字段链接到当前表单模板单元格。目标模板为 `FORMTPL:<versionId>` 时，后端应按 Form Center 模板正式布局 JSON 解析可链接目标单元格；无布局或非法布局仍应 fail-fast。
...
风险集中在单元格链接工作台初始化和 Form Center 模板单元格解析。修复只改变默认来源对应的 `sourceType` 同步顺序，并把已有正式模板布局形态纳入解析契约；不改变保存规则、表单模板作用域或批记录表单链路，也不吞掉真正无效的布局 JSON。
