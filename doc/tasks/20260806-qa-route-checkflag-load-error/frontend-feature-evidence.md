...
- Make QA 规程配置 route scope loading continue when the selected product route lacks unique `checkFlag` but has one deterministic formal BATCH batch-record process or one published route-process batch-record projection.
...
- R1: Selecting `ID / 球囊扩张压力泵 / 112` must not be blocked only because `checkFlag` is missing when the route has one enabled formal batch-record process or one published route-process batch-record projection.
...
- Data state rule: `checkFlag` is preferred; if absent, only an enabled BATCH config with formal `batchRecordReports` or a unique route-process `batchRecordReportId/code/name` projection can identify the process.
...
- BDD: QA 路线缺少 checkFlag 但已发布路线工序有唯一默认批记录投影 -> Given ACTIVE 版本发布时已把主批记录投影到路线工序 When BATCH 配置列表不可用或为空 Then QA 页面仍应按唯一路线工序 `batchRecordReportId/code/name` 定位适用工序。
...
RED: `node tests\e2e\qa-regulation-route-checkflag-fallback-static.spec.cjs` -> FAIL
- Expected reason: missing published route-process batchRecordReport projection resolver.
