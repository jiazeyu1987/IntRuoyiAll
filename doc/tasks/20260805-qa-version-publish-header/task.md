...
版本号、生效日期、`DRAFT` 状态和“发布规程”按钮已集中到顶部标题栏动作区；总览和隐藏发布区的重复入口已移除。聚焦静态契约、相邻回归、`pnpm ts:check` 和 evidence validator 均通过；任务中间 evidence 已按 cleanup 规则清理，任务实现提交为 `096651841`，收尾记录被并行基线提交 `633361dde` 纳入并已同步到 `origin/int_main`。真实页面只读核对被现有本机会话登录超时阻断。
