...
- 已按用户确认补充闭环：每个节点都包含固定样本或任务自有测试标识、前置复位、页面操作、页面验证、测后清理/作废/恢复/保留方式，保证失败后下次执行可以先复位再继续。

## Verification

- RED: 写入前 6 个期望节点均缺失。
- GREEN: 受控事务写入 6 个测试项和 24 个检查点。
- RED: 闭环准备度扫描显示 `node_cases=6`、`closed_loop_ready_nodes=0`，旧文案缺少固定样本、清理和恢复闭环。
- GREEN: 闭环修订事务更新 6 个节点和 24 个目标，写入前置复位、固定样本、清理/作废/恢复/保留描述。
...
- Evidence validation: `validate_database_schema.py` 通过，长期门禁索引 `rg` 命中，`git diff --check` 无 whitespace error。
...
- 收尾提交/推送未执行：当前工作区有非本任务改动且分支领先远端 1 个提交，任务状态保留为 `ready_for_closeout`，避免混入无关变更。
