...
- [x] M4: 运行静态、前端类型、后端编译和真实 Playwright E2E。
- [ ] M5: 完成验证报告、经验沉淀、cleanup 和最终状态。
...
- `docs/backend-development.md#切换填写人快照读取边界`：候选必须来自执行详情 `assistSwitchTasks`，不得在弹窗打开时调用全量 `getEdhrBatchExecution`，不得用当前登录人或空列表兜底。
