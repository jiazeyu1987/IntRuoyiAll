...
- EXPERIENCE: Updated `docs/backend-development.md#生产组长工序配置维护权限不得被工序开始快照误拦` and `docs/experience-index.md` to record that add-dialog verification must use `/mes/pro/process-pool/team-leader/process-config/list`, not the frontline device-account endpoint, and that SQL data-repair rechecks must resolve the current active route version after route publication.
