...
- [x] 按截图反馈将 picker 弹框放大 1.5 倍、选项卡缩到约 1/4 面积
- [x] 按最新反馈补偿“最大化 / 重填 / 提交”在 stage 缩放后的可见字号
- [x] 修复一线生产选择工序后弹框等待异步配置才消失的问题
- [x] 将选择弹框即时反馈经验合并到前端开发规则
...
实现与定向验证已通过：一线生产内部画布保持参考 HTML 的严格 1920×1080 布局，普通页通过外层 stage 等比例缩放避免横向溢出，右上按钮仍按 PQC 逻辑显式“最大化/主页”切换；生产 picker 弹框和选项卡按最新反馈改为 1920:1080 / 16:9 比例；“最大化 / 重填 / 提交”已在 stage 缩放后按可见字号反向补偿；选择工序时生产模式现在先关闭 picker，再等待运行配置和默认员工切换，PQC 模式仍保留成功校验后关闭。但当前 `int_main` 工作区存在非本任务脏改动/未跟踪文件；按项目提交推送门禁，本轮不提交、不推送、不标记 completed。

## 设计约束检查

- `是否引入 fallback/降级/吞异常`：否。
- `是否从根因和长期维护角度解决`：是，参考内部 canvas 仍由外层 stage scale 防溢出；picker 弹框比例作为最新明确反馈单独锁定为 16:9；关键操作按钮用生产 stage 作用域 CSS 变量补偿可见字号，避免再回到整体缩小后的文字。
- `是否存在临时补丁或绕过`：否。
...
- 前端选择弹框即时反馈门禁：生产模式点选即关闭的 picker 必须在耗时异步前关闭；PQC 等校验型 picker 必须保留校验成功后关闭。
