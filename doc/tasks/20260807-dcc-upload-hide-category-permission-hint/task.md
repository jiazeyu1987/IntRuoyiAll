...
用户要求的展示行为、聚焦静态合同和相邻分类合同已通过；实现已被共享分支并发基线提交并推送。cleanup 已完成，但最终任务记录所在的本地提交仍因 GitHub 代理不可连接而无法推送，任务不能标记 completed。

## 设计约束检查

- `是否引入 fallback/降级/吞异常`：否。
- `是否从根因和长期维护角度解决`：是；本任务直接移除不需要的展示节点，不用 CSS 遮挡或权限逻辑变化冒充展示修复；权限职责调整由独立任务负责。
- `是否存在临时补丁或绕过`：否。
