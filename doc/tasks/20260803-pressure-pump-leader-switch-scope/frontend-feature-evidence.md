...
- 工序开始字段列表显示“生产组长”。
- 生产组长面板独立展示，支持当前工艺路线负责范围、账号/权限角色来源、候选账号/角色多选。
- 前端 API 声明读取当前路线负责范围、读取生产组长配置、保存生产组长配置。
- 批次执行页签可见性不新增额外权限；切换范围由后端返回的生产组长配置结果决定。

## BDD:

- BDD: 生产组长配置入口 -> Given 配置人员打开工艺路线关系图 When 点击工序开始卡片 Then 字段明细中出现生产组长配置入口。
- BDD: 生产组长账号角色配置 -> Given 当前工艺路线就是工作站产线负责范围 When 新增生产组长配置 Then 可选择当前路线范围、账号或权限角色并保存。
...
- RED: `node tests\e2e\mes-route-start-production-leaders-static.spec.js` after 2026-08-04 clarification -> FAIL, expected reason: old UI still contained “工作站产线” prerequisite wording.

## GREEN:

- GREEN: `node tests\e2e\mes-route-start-production-leaders-static.spec.js` -> PASS.
- GREEN: `node tests\e2e\mes-route-start-production-leaders-static.spec.js` -> PASS after current-route-scope wording and static contract update.
...
- Static contract verifies `data-flow-boundary-field="productionLeader"`, `data-flow-panel="route-start-production-leader-detail"`, current route scope/source/candidate selectors, no “工作站产线” prerequisite wording, `USERS` and `ROLE` source options, and API method names.
