...
- Third continue attempt: `E:\IntRuoyi` had no lock at first, but remained dirty and `codex/dcc-upload-size-policy-fix` did not contain latest `int_main`. While attempting to preserve the latest main-worktree dirty state as a separate baseline, `git add` completed slowly, then `git commit` failed because another process created `index.lock`.
- Blocked: process evidence showed active `git commit -m "docs: close DCC download entry task"` and the index contained mixed staged/unstaged entries (`MM`/`AM`) plus new untracked task files. This task stopped to avoid committing or overwriting another task's staged state.
- Local `int_main` application: user asked to continue after confirming local `int_main` did not contain `627951dc7` and did not have `IntRuoyiBackend/sql/mysql/20260803_dcc_upload_size_policy_default_seed.sql`.
- Baseline before local application: `0fada3212 chore: baseline current main workspace before upload policy fix` preserved current dirty `E:\IntRuoyi` changes before applying this task's fix.
- Applied to local `int_main`: `git cherry-pick --no-commit 627951dc7` -> PASS, adding the SQL seed, SQL contract test, and related experience gates without conflicts.
- Local verification after main workspace cleanup: `python -X utf8 -m pytest IntRuoyiBackend\script\tests\test_dcc_upload_size_policy_seed_sql.py` -> PASS, 3 tests.
- Local verification after main workspace cleanup: `mvn.cmd -f IntRuoyiBackend\pom.xml -pl yudao-module-dcc -am "-Dtest=DccUploadSizePolicyServiceTest,DccControlledFileUploadApiTest#uploadPreviewFile_missingSizePolicy_throwsBeforeStorageOrTicket,DccControlledFileUploadApiTest#uploadPreviewFile_sourceDocx_successCreatesTicketAndDoesNotExposeFileId" "-Dsurefire.failIfNoSpecifiedTests=false" test` -> PASS, 8 tests.
...
- Closeout apply / worktree merge remains blocked by local main history and prior dirty main worktree state until this local application is committed and the linked worktree can be reconciled.
