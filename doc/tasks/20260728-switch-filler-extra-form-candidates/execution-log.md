...
- Experience consolidation: 已按 `$project-experience-consolidation` 更新 `docs/frontend-development.md#切换填写人 FormCenter 槽位导航门禁`、`docs/backend-development.md#切换填写人快照读取边界`、`docs/e2e-rules.md#eDHR 工作任务 FormCenter 动态表单夹具门禁` 和 `docs/experience-index.md`；长期门禁新增 `openTask` 模板渲染快照、禁止普通填写人依赖模板管理查询权限、真实 E2E 需记录无 `form:template:query` 403/空表单。
