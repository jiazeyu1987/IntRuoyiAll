...
- BLOCKED: automatic linked-worktree closeout preview cannot proceed because local `int_main` is ahead of `origin/int_main` and is not an ancestor of this branch; main worktree `E:\IntRuoyi` is also dirty.
...
- BLOCKED: final default closeout preview still cannot ff-only merge/remove the linked worktree because local `int_main` is at `1d145ff95` (ahead 1) while this branch merged `origin/int_main` `d8de70c08`, and `E:\IntRuoyi` has unrelated dirty 20260805 task files.
