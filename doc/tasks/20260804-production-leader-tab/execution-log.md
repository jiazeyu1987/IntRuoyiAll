...
- CLOSEOUT BLOCKED: automatic linked-worktree closeout preview -> BLOCKED，current branch cannot be fast-forward merged into `int_main`, and main worktree `E:\IntRuoyi` is dirty. Current branch will be pushed for safe review/integration; worktree is not removed.