...
| Runtime preflight | PASS_CHECK_RUNTIME_BLOCKED_FULL：授权 `real:check` PASS，0 SOURCE/ENV/RUNTIME；当前 `result.json` 来自 full real E2E 失败复验，失败原因是 48081 正运行无关 DCC product-onboarding Jar，不作为 M6 RRM runtime PASS |
...
| M6 real E2E coverage ledger | STRUCTURED_BLOCKED_HISTORICAL / RUNTIME_BLOCKED_CURRENT：历史 M6 RRM run 产生 6 phase evidence、10 action evidence、2 gate evidence；当前签名池修复后的 full rerun 因 48081 无关 DCC runtime 在 D29 前失败，未新增 M6 RRM full-run PASS |
