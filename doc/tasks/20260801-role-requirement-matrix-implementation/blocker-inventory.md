...
说明：`Downstream work can continue` 表示在 2026-08-02 用户批准的 M0 新口径下，当前或后续里程碑是否可继续推进不依赖该 blocker 的切片。当前主线程已完成 M1 activeOrderId source gate、M2 production coefficient snapshots gate、M3 QA/PQC source gate、M4 transfer/release source gate 和 M5 route configuration separation source gate，并切换到 M6；Excel 全量目标仍受 M6 验收门禁约束。
...
| RRM-BLK-008 | M4 / AC-M22, AC-M23 / TC-M22, TC-M23 | `CHECK_INSPECTION_RESULT` -> `MesOrderReleaseCompletenessService.evaluateInspectionResult` | `pnpm e2e:role-requirement-matrix:real:check` | 已接入放行检验结果来源 | eDHR 放行预检可通过正式 PQC 任务状态追溯过程检验 | 已实现：过程检验聚合来源适配器，缺 activeOrder/PQC task 或未确认任务时 fail-fast | 已验证关闭 |
| RRM-BLK-009 | M4 / AC-D36, AC-M22 / TC-D36, TC-M22 | `CHECK_DEVIATION_CLOSED` -> `MesOrderReleaseCompletenessService.evaluateDeviationClosed` | `pnpm e2e:role-requirement-matrix:real:check` | 已接入偏差关闭来源 | 有偏差时放行完整性可判断并阻塞未关闭项 | 已实现：偏差/质量异常关闭来源适配器 | 已验证关闭 |
| RRM-BLK-010 | M4 / AC-M22, AC-M23 / TC-M22, TC-M23 | `CHECK_REWORK_CLOSED` -> `MesOrderReleaseCompletenessService.evaluateReworkClosed` | `pnpm e2e:role-requirement-matrix:real:check` | 已接入返工关闭来源 | 返工未闭环时可阻塞放行 | 已实现：返工 trace 来源适配器 | 已验证关闭 |
| RRM-BLK-011 | M4 / AC-M22, AC-M23 / TC-M22, TC-M23 | `CHECK_SCRAP_RECORDED` -> `MesOrderReleaseCompletenessService.evaluateScrapRecorded` | `pnpm e2e:role-requirement-matrix:real:check` | 已接入报废记录来源 | 报废未完成时放行完整性可判断 | 已实现：报废 trace 来源适配器 | 已验证关闭 |
| RRM-BLK-012 | M4 / AC-M22, AC-M23 / TC-M22, TC-M23 | `CHECK_INVENTORY_CONSISTENCY` -> `MesOrderReleaseCompletenessService.evaluateInventoryConsistency` | `pnpm e2e:role-requirement-matrix:real:check` | 已接入库存一致性来源 | 物料批次/库存不一致时可阻塞放行 | 已实现：库存一致性来源适配器并关联 activeOrderId trace | 已验证关闭 |
| RRM-BLK-013 | M4 / AC-M02, AC-M07 / TC-M02, TC-M07 | `activeOrderTransferRelation` | `pnpm e2e:role-requirement-matrix:real:check` | 已新增 activeOrderId 与调拨头/行正式关系 | 多调拨、多物料、多批次可追溯到统一订单身份 | 已实现：active order transfer trace schema / mapper / service / idempotency key | 已验证关闭 |
| RRM-BLK-014 | M4 / AC-M05 / TC-M05 | `activeOrderShipmentSource` | `pnpm e2e:role-requirement-matrix:real:check` | 已新增 activeOrderId 与发货事实正式关系源 | 开工检查可追溯发货物料覆盖来源 | 已实现：trace schema 覆盖 SHIPMENT 来源和索引 | 已验证关闭 |
| RRM-BLK-015 | M4 / AC-M05, AC-M06 / TC-M05, TC-M06 | `activeOrderReplenishmentReturnSource` | `pnpm e2e:role-requirement-matrix:real:check` | 已新增 activeOrderId 与补料/退料正式关系源 | 净物料覆盖计算具备方向、数量、批次来源 | 已实现：trace schema 覆盖 REPLENISHMENT/RETURN 来源和方向索引 | 已验证关闭 |
| RRM-BLK-016 | M4 / AC-M06, AC-M08 / TC-M06, TC-M08 | `activeOrderBatchTraceSource` | `pnpm e2e:role-requirement-matrix:real:check` | 已新增 activeOrderId 与物料批次/库存追溯正式关系源 | 开工检查和放行库存一致性可追溯批次/库存 | 已实现：trace schema 覆盖 BATCH_TRACE、batchId、materialStockId 和库存索引 | 已验证关闭 |
...
| RRM-BLK-029 | M0, M5 / AC-D14 / TC-D14 | `normalizeRecordBindingSlotTypeDefaultMain` | `pnpm e2e:role-requirement-matrix:real:check` | 已移除工艺路线前端缺槽位默认 `MAIN` 路径 | `formBindings` / 旧字段不再被误归为正式批记录表单 | 已实现：缺正式批记录槽位时 fail-fast，逐工序绑定只读正式表 | 已验证关闭 |
| RRM-BLK-030 | M0, M5 / AC-D14 / TC-D14 | `batchRecordFormNamesFormBindingsSeparation` | `pnpm e2e:role-requirement-matrix:real:check` | 已证明 `batchRecordFormNames` 与 `formBindings` 分离 | 三类工艺路线配置不再互相替代 | 已实现：静态合同和真实检查分别证明工序开始、批记录表单、表单槽位来源 | 已验证关闭 |
| RRM-BLK-031 | M0, M5 / AC-D14 / TC-D14 | `edhrRuntimeDefaultMainSlot` | `pnpm e2e:role-requirement-matrix:real:check` | eDHR 运行态缺 `formSlotType` 时已 fail-fast | 正式批记录绑定缺失不再被表单槽位或旧字段掩盖 | 已实现：eDHR 运行态读取正式逐工序绑定；缺失时 fail-fast，不默认 `MAIN` | 已验证关闭 |
...
说明：本表按 `IntRuoyiFronted/test-results/role-requirement-matrix-real-flow/result.json` 当前结果映射到结构化 blocker ID。最新结果为 `PASS` 且 `blockers=[]`，RRM-BLK-001..032 均已在 `Status Register` 标为 `RESOLVED_VERIFIED`。该映射用于防止后续清单改名、合并描述或中文说明导致 `real:check` 输出无法追踪。

| result.json key | Category | Blocker ID | Current status |
|---|---|---|---|
| none | none | RRM-BLK-001..032 | RESOLVED_VERIFIED |
...
| RRM-BLK-008 | RESOLVED_VERIFIED | 2026-08-02 |
| RRM-BLK-009 | RESOLVED_VERIFIED | 2026-08-02 |
| RRM-BLK-010 | RESOLVED_VERIFIED | 2026-08-02 |
| RRM-BLK-011 | RESOLVED_VERIFIED | 2026-08-02 |
| RRM-BLK-012 | RESOLVED_VERIFIED | 2026-08-02 |
| RRM-BLK-013 | RESOLVED_VERIFIED | 2026-08-02 |
| RRM-BLK-014 | RESOLVED_VERIFIED | 2026-08-02 |
| RRM-BLK-015 | RESOLVED_VERIFIED | 2026-08-02 |
| RRM-BLK-016 | RESOLVED_VERIFIED | 2026-08-02 |
...
| RRM-BLK-029 | RESOLVED_VERIFIED | 2026-08-02 |
| RRM-BLK-030 | RESOLVED_VERIFIED | 2026-08-02 |
| RRM-BLK-031 | RESOLVED_VERIFIED | 2026-08-02 |
...
- M4 transfer / release blockers：已验证关闭；后续不得重新引入 `buildSourceNotIntegratedItem` 覆盖五类放行来源，也不得删除 activeOrderId transfer trace schema/source gate。
- M5 route configuration separation blockers：已验证关闭；工艺路线前端、eDHR 运行态和真实 E2E 已共同证明三类配置互不替代。
...
当前门禁：本表把 SOURCE blocker 转换成后续可执行的 RED 计划和依赖关系。M0 已按新口径 accepted，M1 activeOrderId 切片、M2 production coefficient snapshots 切片、M3 QA/PQC 切片、M4 调拨/放行来源切片和 M5 路线配置分离切片已验证关闭；`Allowed in current milestone` 现在只允许 M6 迁移、并发、性能和全量真实 E2E 验证启动。
...
| Transfer, shipment, batch trace and release sources | RRM-BLK-008..016 | M4 | M1/M2/M3 GREEN；ERP/MES 调拨、发货、补退料和批次关系源冻结 | `mvn -pl yudao-module-mes -am "-Dtest=MesActiveOrderTransferTraceSchemaTest,MesProEdhrReleaseServiceImplTest" "-Dsurefire.failIfNoSpecifiedTests=false" test` | 已 GREEN；`real:check` 不再输出 RRM-BLK-008..016 | 已完成 |
| Route batch-record and formBindings separation | RRM-BLK-029..031 | M5 | M4 GREEN 后进入；解除需真实页面证明三类配置互不替代 | `pnpm e2e:role-requirement-matrix:real:check` plus focused static/E2E contract | 已 GREEN；`real:check` 不再输出 RRM-BLK-029..031 | 已完成 |
...
| M4 transfer/release production/frontend/backend implementation | 已完成 | RRM-BLK-008..016 已通过 GREEN/REGRESSION/real:check 验证关闭 |
| M5 route configuration separation production/frontend/backend implementation | 已完成 | RRM-BLK-029..031 已通过 GREEN/REGRESSION/real:check 验证关闭 |
| M6 production/frontend/backend implementation | 允许 | 当前里程碑为 M6，但必须先 BDD 和 RED，不得跳过迁移/并发/性能/真实 E2E 门禁 |
| Future RED commands listed above | M1-M5 已完成；M6 命令允许启动 | M6 命令是当前 TDD 起点 |

## Verification Methods

当前门禁：以下 M1-M5 命令作为已完成 blocker 的验证入口或 RED/GREEN 记录。M0/M1/M2/M3/M4/M5 已 accepted；当前主线程只允许启动 M6 迁移、并发、性能、真实 E2E 和收尾验收命令。
...
- M4 completed static contract gate：`pnpm e2e:role-matrix-transfer-start-check:static`
- M5 completed static contract gate：`pnpm --dir IntRuoyiFronted e2e:role-matrix-route-config-separation:static`

## Risks

- 临时 QA 模板 `6 / RRM-20260801-QA-REG-PP-V21` 只能作为本机测试夹具，不解除正式 QA 规程版本模型 blocker。
- M6 仍缺迁移、并发、性能、全量真实 E2E 和收尾验收；不得用单个 source `real:check` PASS 伪造 Excel 全量完成。
...
- M6 需要确认迁移、并发、性能、全量真实 E2E、任务数据清理和上线/readiness gate 的最终执行范围。
