...
2026-08-03 最新更新：当前 48081 listener PID `43876` 为本任务 RRM M6 runtime jar `backend-runtime-control-20260803-115911-rrm-m6-pqc-skip-submitted.jar`，后端 health `UP`、前端 8081 HTTP `200`。AC-D29 已通过真实页面生成 `submittedTaskId=31`、`eventId=26`、`signatureId=23`；AC-D32 已通过 PQC 组长同筛选分页证明 `total=2`，第 1/2 页事件分别为 `24`、`26`。M6 仍保持 `in_progress`，剩余 blocker 为 `activeOrderCleanupDeferred`、`m6ConcurrencyGateDeferred`、`m6PerformanceGateDeferred` 和 62 个 `E2E_COVERAGE`。
...
| M6 AC-D32 PQC leader submission filter/pagination | PASS_ACTION_NOT_ACCEPTED | 提交看板 mapper 已按产品、检验类型、轮次、复核状态和 `pqcTaskId` 精确关联筛选；前端 PQC 组长筛选与结果列已接入；D32 local fixture/static、node syntax、preflight static、mapper static、`ts:check`、`ProcessPoolTimelineFilterTest`、授权 `real:check` 和 full real E2E 同筛选分页动作均 PASS。仍缺失败路径、权限/只读隔离、性能门禁、清理和全量 M6 coverage 准出。 |
| M6 AC-D29 PQC formal submission event | PASS_ACTION_NOT_ACCEPTED | `submitPqcInspection` 已在正式提交时调用 `createPqcInspectionEvent` 生成可追溯 PQC event；`MesFrontlinePqcContextServiceTest` 6 tests PASS，真实 E2E 脚本语法、preflight static 和 mapper static PASS，`yudao-server` package PASS；固定签名 ID 25 复用已由 `resolveUnusedPqcSignatureId` 脚本门禁解决，full real E2E 已用 `signatureId=23` 创建 `submittedTaskId=31/eventId=26`。仍缺失败路径、重复/并发提交证明、复核/只读追溯、清理和全量 M6 coverage 准出。 |
...
| M6 concurrency/performance gate ledger | STRUCTURED_BLOCKED | `gateEvidence=2`；`m6ConcurrencyGateDeferred` 覆盖 12 个 CONC AC，`m6PerformanceGateDeferred` 覆盖 4 个 PERF AC，已观察 `AC-D27` 和 `AC-D32`，但日结、PQC 列表和逐件明细仍缺完整 N+1 / paging drift proof。 |
| M6 real E2E coverage ledger | STRUCTURED_BLOCKED | 最新 M6 RRM run 已加载 62 AC 并写出 6 个 phase evidence、10 个 action evidence、2 个 gate evidence、65 个 blocker；D29/D32 动作 PASS，但 full AC coverage、cleanup、concurrency、performance 仍未准出。 |
...
- 当前 blocker：`role-requirement-matrix-real-flow.e2e.js` 已具备 62 AC coverage ledger、6 个入口阶段证据、AC-M04 `joinActiveOrder`、`activeOrderConflictRouteRejected`、`activeOrderCrossRoleReadOnly`、`activeOrderUnauthorizedMutationBlocked` 四个 PASS 动作证据、PQC `pqcRegulationItemsRendered`（含 AC-D17 页面可见方法/标准/判定元信息）、`pqcPieceDetailQuantityPrepared`、`pqcActualEmployeeSelected`、`pqcFormalSubmissionCreated`、`pqcLeaderSubmissionFilterPaginationConsistent` 五个 PASS 动作证据，以及 M6 迁移预检静态/策略/运行库门禁 PASS_RUNTIME_PREFLIGHT；权限隔离已由专用错误角色 `aoteman` 真实验证通过；清理追溯门禁已加入但因 activeOrderId 仍是后续 M6 共享夹具而记录 `E2E_CLEANUP` blocker；并发/性能已显式结构化为 12 个 CONC AC 与 4 个 PERF AC 的 gate blocker，其中 `AC-D27` 与 `AC-D32` 已有页面观察，但日结、PQC 列表和逐件明细仍缺完整 N+1 / paging drift proof。62 个 AC 均尚未达到完整真实动作、失败路径、权限隔离、只读核验和清理闭环 `ACCEPTED`；已记录为结构化 M6 BLOCKED，不作为通过。
