...
| M6 AC-D29 PQC formal submission event | CODE_UNIT_STATIC_BUILD_SIGNATURE_POOL_GREEN_RUNTIME_BLOCKED | `submitPqcInspection` 已在正式提交时调用 `createPqcInspectionEvent` 生成可追溯 PQC event；`MesFrontlinePqcContextServiceTest` 6 tests PASS，真实 E2E 脚本语法、preflight static 和 mapper static PASS，`yudao-server` package PASS；固定签名 ID 25 复用已由 `resolveUnusedPqcSignatureId` 脚本门禁解决。full real E2E 仍需在 M6 RRM runtime 上复验，目前被无关 DCC runtime 占用 48081 阻塞。 |
...
| M6 real E2E coverage ledger | STRUCTURED_BLOCKED_HISTORICAL / RUNTIME_BLOCKED_CURRENT | 历史 M6 RRM run 已加载 62 AC 并写出 6 个 phase evidence、10 个 action evidence、2 个 gate evidence；当前签名池修复后的 full rerun 因 48081 无关 DCC runtime 在 D29 前失败，未新增 M6 RRM full-run PASS。 |
| 文档结构 | PASS | `task-state.json`、当前 failed `result.json` 均可解析；任务 Markdown 可 UTF-8 读取，且 current result 明确归因为 runtime ownership blocker。 |
