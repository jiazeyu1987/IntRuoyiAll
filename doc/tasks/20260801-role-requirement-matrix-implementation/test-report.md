...
| M6 concurrency/performance gate ledger | STRUCTURED_BLOCKED | `gateEvidence=2`；`m6ConcurrencyGateDeferred` 覆盖 12 个 CONC AC，`m6PerformanceGateDeferred` 覆盖 4 个 PERF AC 且已观察 `AC-D27`，均仍结构化为后续 M6 门禁 blocker。 |
| M6 real E2E coverage ledger | STRUCTURED_BLOCKED | 真实 E2E 已加载 62 AC，写出 6 个 phase evidence、8 个 action evidence（7 PASS + 1 BLOCKED）、2 个 gate evidence 和 65 个 blocker；不再使用泛化未实现占位。 |
...
- 当前 blocker：`role-requirement-matrix-real-flow.e2e.js` 已具备 62 AC coverage ledger、6 个入口阶段证据、AC-M04 `joinActiveOrder`、`activeOrderConflictRouteRejected`、`activeOrderCrossRoleReadOnly`、`activeOrderUnauthorizedMutationBlocked` 四个 PASS 动作证据、PQC `pqcRegulationItemsRendered`、`pqcPieceDetailQuantityPrepared`、`pqcActualEmployeeSelected` 三个 PASS 动作证据，以及 M6 迁移预检静态/策略/运行库门禁 PASS_RUNTIME_PREFLIGHT；权限隔离已由专用错误角色 `aoteman` 真实验证通过；清理追溯门禁已加入但因 activeOrderId 仍是后续 M6 共享夹具而记录 `E2E_CLEANUP` blocker；并发/性能已显式结构化为 12 个 CONC AC 与 4 个 PERF AC 的 gate blocker，其中 `AC-D27` 已有逐件数量页面观察但未完成完整性能证明。62 个 AC 均尚未达到完整真实动作、失败路径、权限隔离、只读核验和清理闭环 `ACCEPTED`；已记录为结构化 M6 BLOCKED，不作为通过。
