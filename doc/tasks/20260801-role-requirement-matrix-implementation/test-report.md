...
| M6 real E2E coverage ledger | STRUCTURED_BLOCKED | 真实 E2E 已加载 62 AC，写出 6 个 phase evidence、5 个 action evidence（4 PASS + 1 BLOCKED）和 63 个 blocker；不再使用泛化未实现占位。 |
...
- 当前 blocker：`role-requirement-matrix-real-flow.e2e.js` 已具备 62 AC coverage ledger、6 个入口阶段证据和 AC-M04 `joinActiveOrder`、`activeOrderConflictRouteRejected`、`activeOrderCrossRoleReadOnly`、`activeOrderUnauthorizedMutationBlocked` 四个 PASS 动作证据；权限隔离已由专用错误角色 `aoteman` 真实验证通过；清理追溯门禁已加入但因 activeOrderId 仍是后续 M6 共享夹具而记录 `E2E_CLEANUP` blocker。62 个 AC 均尚未达到完整真实动作、失败路径、权限隔离、只读核验和清理闭环 `ACCEPTED`；已记录为结构化 M6 BLOCKED，不作为通过。
