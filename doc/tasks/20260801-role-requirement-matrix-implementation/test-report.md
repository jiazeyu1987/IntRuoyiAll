...
| M6 PQC actual employee switch | STRUCTURED_BLOCKED | `pqcActualEmployeeSelected` 已通过正式 `/pqc/personnel` 来源和 `/pqc/switch-employee` 路径接入真实 E2E；当前业务响应 `1040760117`，PQC 员工和 PQC 组长来源为空，需要补齐正式 `EMPLOYEE` scope 后才能证明 actualEmployeeId 不默认登录人。 |
| M6 real E2E coverage ledger | STRUCTURED_BLOCKED | 真实 E2E 已加载 62 AC，写出 6 个 phase evidence、7 个 action evidence（5 PASS + 2 BLOCKED）和 64 个 blocker；不再使用泛化未实现占位。 |
...
| 2026-08-02 | authorized `pnpm --dir IntRuoyiFronted e2e:role-requirement-matrix:real` after PQC actual employee switch gate | STRUCTURED_BLOCKED，6 phase evidence / 7 action evidence / 34 surfaceObserved / 21 uncovered / 62 pending / 64 blockers；`pqcActualEmployeeSelected` action BLOCKED，原因是 PQC 员工和 PQC 组长 `EMPLOYEE` scope 来源为空 |
