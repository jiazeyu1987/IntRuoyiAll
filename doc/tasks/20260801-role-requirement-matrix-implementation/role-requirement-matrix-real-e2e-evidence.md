...
- Generated At: `2026-08-06T08:02:44.815Z`
- Status: `BLOCKED`
- Frontend: `--`
- Backend: `--`
- Tenant: `--`
...
- ENV:RRM_TRANSFER_IDS -> 加入活跃订单时提交的正式调拨单 ID 正整数列表。
- ENV:RRM_TRANSFER_TRACE_ACTIVE_ORDER_ID -> 已有正式调拨追溯数据的活跃订单 ID，用于只读验收。
...
- ENV:RRM_TRANSFER_TRACE_ACTIVE_ORDER_ID -> 调拨追溯只读验收必须提供已有正式追溯数据的活跃订单 ID，不通过新增活跃订单流程写入补数据。
- ENV:RRM_TRANSFER_IDS -> 加入活跃订单必须提供逗号分隔的大于 0 的正式调拨单 ID 列表。
