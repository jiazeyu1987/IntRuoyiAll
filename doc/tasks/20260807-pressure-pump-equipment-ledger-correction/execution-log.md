...
- PUSH RECOVERY: GitHub 直连恢复后，使用一次性清空 URL 级代理的方式完成 `git push origin int_main`；推送后本地 `HEAD`、`origin/int_main` 和远端 `refs/heads/int_main` 对齐，未修改全局 Git 配置。
...
- 当前会话缺少 `测试租户` 有效登录凭据，因此未执行真实页面只读复验；该缺口不影响已完成的本地数据库事务和精确数据校验，但不能宣称页面 E2E 已通过。
