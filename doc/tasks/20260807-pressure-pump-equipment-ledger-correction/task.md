...
在本地 `int_main` 系统的租户 `122` 设备台账中完成三项用户明确指定的主数据变更：新增 `C01017 / 撤压机`，将光固机 `A05059` 的设备编码改为 `A05075`，将箱型干燥机 `B09041` 的设备编码改为 `B04091`；同时保持设备 ID、正式关联和条码业务信息一致。

## Milestones

- [x] 创建任务记录并确认用户指定范围。
- [ ] 只读核对目标设备、目标编码唯一性、关联表、设备类型、车间和条码数据。
- [ ] 保存精确变更前快照并记录回滚条件。
...
- 新设备 `C01017` 使用已核对的正式设备类型、车间和生产状态，并生成一致的设备条码记录。
- 所有以设备 ID 或冗余设备编码引用目标设备的正式关联保持一致，不出现旧编码残留。
- 其它租户和其它设备不被修改，事务影响行数与预期完全一致。

## Current Status

in_progress - 正在核对正式关联和写入前置条件，尚未修改业务数据。
