...
- 计划新增：5 个生产任务和 5 个 OPEN 记录本；随后由真实一线生产页面生成 5 条生产来源事件，再由真实一线 PQC 页面提交既有的 5 条 FINAL PENDING 任务。
- 正式来源：tenant 1 工单 `980022..980026`、活跃订单 `35..39`、路线 `980091/V622`、路线工序 `980631/922985`、已有岗位 14 且具备正式接口权限并命中运行时员工候选的账号 `659/shangmengying`、发布 QA 规程版本 `36`、PQC 任务 `198/202/206/210/214`。
- 任务唯一身份：生产和 PQC 幂等键使用 `CODX-PQC-20260807-01..05`，任务自有生产前置的 creator/updater 为任务标识。

## Data Safety Analysis

- 写入限定在本机测试数据库、单一测试租户、目标 PQC 人员和精确任务标识。
- SQL 仅创建真实一线生产提交所需的生产任务和记录本；不直接写生产/PQC 事件、PQC 记录、逐件明细或 PQC 任务状态。
...
- 旧路线正式上下文 -> 路线 `922119/V24` 当前工序缺工作站且规程仍引用旧工序，正式候选接口以 `1040760101/1040760102` 失败；本任务不修改共享主数据。
- 新对象只读预检 -> 活跃订单 `35..39` 与当前路线工序、工作站、设备账号岗位绑定、发布 QA 规程及 5 条 FINAL PENDING 任务完整一致。
