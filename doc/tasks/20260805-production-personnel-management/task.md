...
- [ ] M6：收尾、经验沉淀、cleanup、提交并推送当前分支；功能分支已融合进 `origin/int_main`，原 worktree 已从 Git 注册移除且运行态停止；物理残留目录删除被本地执行策略拦截，slot 暂不释放。
...
更新：生产人员功能分支已通过独立集成 worktree 融合进 `origin/int_main`，远端主线包含该功能分支。原生产人员 worktree 的 Git 注册已移除，运行态已停止，`8082/48082` 已释放；但物理目录 `D:\IntRuoyiWorktree\20260805-production-personnel-management` 仍存在且不含 `.git`，递归物理删除被本地执行策略拦截，因此 slot `1` 暂不释放。
