...
- [x] M2d：补齐末检不适用依据的正式持久化模型。
- [x] M4：运行目标后端、前端静态合同和必要回归验证。
- [x] M5：记录验证报告、经验沉淀、清理与提交推送。
...
completed：已完成并验证前端数量锁定/不截断、后端提交侧数量 fail-fast、正式 PQC 任务生成器、上午/下午巡检身份分离、`301×5%` 向上取整为 `16`、适用末检任务生成、末检不适用正式持久化依据和放行完整性预期任务集合校验；目标静态合同、结构检查、后端 Maven/JUnit、经验沉淀和 cleanup 均已完成，收尾记录将随本任务选择性提交并推送。
