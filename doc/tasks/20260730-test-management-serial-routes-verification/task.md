...
验证 `系统管理 > 测试管理` 下测试租户中的 3 个“工艺路线节点闭环”串行路线是否都能通过真实前端路径完整跑完，并给出 PASS / BLOCKED / FAIL 结论。

## Milestones

- [ ] M1: 核对测试管理、登录、Runner、本机前后端、Playwright 与测试租户前置条件
- [ ] M2: 通过真实测试管理页面确认 3 个目标串行路线与节点完整性
- [ ] M3: 分别执行 3 个串行路线并记录执行结果、检查点和清理状态
- [ ] M4: 汇总验证报告并标记最终状态
