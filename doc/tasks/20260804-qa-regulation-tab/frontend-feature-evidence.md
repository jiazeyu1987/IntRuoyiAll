...
- The tab visibly states `正式保存/发布接口未接入` and `未写入后台`.
...
| --- | --- | --- | --- |
| QA 配置过程检验规程 | No `QA 规程` tab or QA selectors exist. | Add tab, stable root selector, scope, rules, items, completeness, and PQC preview blocks. | New static contract and `ts:check` pass. |
...
- RED: `node IntRuoyiFronted\tests\e2e\role-matrix-qa-regulation-tab-static.spec.cjs` -> FAIL, expected reason: existing workbench did not expose `QA 规程` tab.
...
- Accessibility: form labels, table headers, tags, alerts, and stable data selectors are present for the tab.
- Loading: QA local draft tab does not trigger existing production/PQC list loading.
- Empty/error: publish precheck lists missing scope, version, rules, items, and numeric limits instead of silently succeeding.
- Permission: no new route/menu/permission change in this slice; the existing workbench entry owns tab visibility.
