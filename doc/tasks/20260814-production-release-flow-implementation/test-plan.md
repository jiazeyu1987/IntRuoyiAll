...
- environment or setup: 先核验当前 `int_main` Jar/Vite 与监听来源；通过登录页选择或输入测试租户后执行当前租户业务验证；生产组长、zhulijiang、来料、灭菌、成品、xujianhai、非候选账号。准备三条任务自有订单：双 100% 主链、双 100% PQC 拒绝、进度不足；四附件、灭菌批号、签核证据和明确的页面清理计划。AC-30 的跨租户自动负向验证如需执行，再另备第二测试租户账号；缺少“租户管理”菜单不再阻塞当前测试租户手工验收。
...
## P11 真实验收执行单（测试租户手工验收）

范围变更：用户于 2026-08-17 要求 Agent 改在“芋道源码”验证，并进一步授权从用户列表选择现有业务账号、使用用户提供的临时测试密码以及创建订单等任务自有虚拟数据；密码不得回显或落盘。用户于 2026-08-18 进一步明确：从测试租户登录后，后续操作即属于该测试租户，“可见租户管理菜单”不是当前手工验收前置。先前“由用户手工执行”的责任安排和 admin-only 只读限制被授权扩展取代，但数据范围仍只限带 `PRFLOW-T11-20260817` 标识、可追踪且可通过正式页面清理的本轮数据。30 项 `EDHR_FULL_E2E_*` 前置仍是自动 TC-13 的正式输入；当前测试租户手工验收只要求账号角色、三类正式来源、文件存储和清理入口在任何写入前完成确认。AC-30 跨租户自动负向验证保留为后续独立补充项；不将只读入口或未执行的 Playwright 记为 PASS，也不删减 AC-01 至 AC-34。

### 固定前置

1. 记录当前 `int_main` 提交、前后端运行产物来源、监听 PID、测试租户和开始时间；运行态必须包含本任务当前代码，不能复用旧 Jar 或其它分支产物。
2. 准备七个相互独立且可登录的测试租户账号：生产组长、PQC、来料检负责人、灭菌负责人、成品检负责人、管理者代表和非候选账号；另准备一名不归属目标订单的生产组长。只有执行 AC-30 跨租户自动负向验证时，才需要另准备第二测试租户的同名或同角色账号。
...
6. 若本轮包含 AC-30 跨租户自动负向验证，则在第二测试租户重复只读查询或按计划的最小写入验证，确认同名账号、订单号或批号不能跨租户读取、处理或泄露详情；跨租户请求必须显示无权或不存在。若用户只执行当前测试租户手工验收，则记录“跨租户自动负向未执行”，不得写成通过。
...
| 数据清理与完整性 | AC-30、AC-31、AC-34 | 当前测试租户登录记录、审计记录、清理前后只读结果；跨租户自动负向未执行时必须明确标记未覆盖 |

执行完成后，Agent 与独立 tester 必须保留并回填每一步的通过/失败、截图或结果文件位置、申请/任务/批次/事务 ID 和清理结果；缺任何 AC 的真实执行证据时，P11 继续保持 `blocked`。只读 `芋道源码/admin` 入口验证不能替代 30 项自动 E2E 前置或同一真实页面链；“租户管理”菜单不可见也不能再作为当前测试租户手工验收的 blocker。
...
P11 责任更新：P1-P10 的既有独立测试证据保留。用户授权的三个生产放行按钮、`MES_PQC_RELEASE_OWNER`、`MES_MANAGEMENT_REPRESENTATIVE`、两套精确最小权限和两个固定用户绑定均已通过“芋道源码”真实页面落地并回读；未创建订单、附件或其它业务 fixture。业务账号已在新 worktree 运行态完成密码重置和真实登录权限核验。用户已确认测试租户登录即可限定本轮业务操作范围；由用户按本执行单继续手工真实业务验证，P11 在证据回填前保持 `blocked`。
