...
- 禁止用 `formBindings`、默认 `MAIN`、工序开始配置或空值替代逐工序正式 `MAIN` 批记录绑定；路线明确绑定的 template 28/25 仅作为 PI/LOSS 正式目标。
...
- 路线目标为 template 28 时，必须按已发布 template version 的精确字段映射写入并提交 FormCenter instance；动态写入链路未完成时精确阻塞。
...
- 写正式损耗单或正式无损耗确认。
- 路线目标为 template 25 时，必须按已发布 template version 的精确字段映射写入并提交 FormCenter instance；动态写入链路未完成时精确阻塞。
...
| G-13 | 禁止替代来源 | MAIN 批记录不来自 `formBindings` 或默认 MAIN；PI/LOSS 动态目标不冒充业务来源 |
