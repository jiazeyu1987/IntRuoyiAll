...
- [ ] 建立 RED：用后端测试证明 OQ/PQ 与图纸等测试服文件名在现有规则下仍会歧义或未分类。
- [ ] 实现正式类别匹配规则表、迁移、DO/Mapper、测试夹具与服务读取逻辑。
- [ ] GREEN：目标 JUnit 通过，保留原有泛化歧义仍为 `AMBIGUOUS` 的行为。
- [ ] 完成 schema/backend evidence、验证报告、经验沉淀、提交与推送。

## Expected Verification

- RED：`mvn -pl yudao-module-dcc -am "-Dtest=DccProjectCodeServiceImplTest" "-Dsurefire.failIfNoSpecifiedTests=false" test` 在新增测试上失败，失败原因是现有硬编码别名无法被正式可维护规则覆盖或无法识别目标类别。
