...
- [x] M5 从用户指定 PDF 提取并核对所有工序检验数据。
- [x] M6 将 IDI 初始规程数据更新为 PDF 全量工序数据。
...
用户补充范围已实现并验证：IDI 初始规程数据已按指定 PDF 5.1 表覆盖 6 个工序、22 条检验行。最终收尾提交仍需避开当前工作区无关并行改动。
...
- `清洁`：外观。
- `组装螺杆八组件`：外观、无跳压。
- `光固外套四组件`：光固旋转接头/光固压力表/光固延长管的外观与牢固度。
- `装配`：装配活塞外观、硅化活塞环外观、装配活塞环外观与配合。
- `整体粘结`：外套组件与套筒组件装配外观/配合、气密性外观、无卡阻、牢固度、气密性负压/高压/低压检测。
