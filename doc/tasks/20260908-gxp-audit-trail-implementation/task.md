...
3. in_progress - M3 首批 GxP 高风险链路接入：已登记并注解 eDHR、DCC、电子签名、权限/角色配置、系统配置、发布/迁移变更；电子签名 `sign`、DCC 发布审批发起 `publishControlledFile`、权限/角色配置三处 assign 入口、系统配置包导入已接入统一 `GxpAuditService.append` 并验证审计失败不返回成功；仍需继续逐业务方法接入 eDHR 与发布/迁移变更完整 before/after。
...
in_progress - M1/M2/M4 已完成；M3 已完成电子签名、DCC 发布审批发起、权限/角色配置、系统配置包导入的同事务统一审计 append 切片，剩余 eDHR 与发布/迁移变更仍需逐入口接入；M5 运行态证据仍 blocked，当前不得宣称 `PASS FOR OPERATIONAL COMPLIANCE`。
