...
基于用户截图中 PQC 填写页“检验内容”卡片内红框区域，先完成独立 HTML 原型设计，再按已确认方向落到正式 Vue 页面：检验项以最多 10 个完整显示的 2 行 x 5 列 tab 呈现，仅展开当前检验项详情，并将设备、编号、接收标准、检验方法改为与一线操作端一致的触控式信息卡；正式系统 PQC 填写页必须与更新后的 HTML 预览关键效果一致。
