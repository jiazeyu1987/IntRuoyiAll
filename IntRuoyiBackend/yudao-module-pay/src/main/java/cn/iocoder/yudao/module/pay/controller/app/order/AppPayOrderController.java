...
public class AppPayOrderController {
    ...
    public CommonResult<PayOrderRespVO> getOrder(@RequestParam(value = "id", required = false) Long id,
        ...
        PayOrderDO order = getAccessibleOrder(id, no);
    ...
    public CommonResult<PayOrderRespVO> syncOrder(@RequestParam(value = "id", required = false) Long id,
                                                  @RequestParam(value = "no", required = false) String no) {
        PayOrderDO order = getAccessibleOrder(id, no);
        if (order == null) {
            return success(null);
        }
        payOrderService.syncOrder(order.getId());
        return success(BeanUtils.toBean(payOrderService.getOrder(order.getId()), PayOrderRespVO.class));
    }

    private PayOrderDO getAccessibleOrder(Long id, String no) {
