...
public class PayOrderServiceImpl implements PayOrderService {
    ...
    public int syncOrder(LocalDateTime minCreateTime) {
        ...
        for (PayOrderExtensionDO orderExtension : orderExtensions) {
            count += syncOrderQuietly(orderExtension) ? 1 : 0;
        }
        return count;
    }
    ...
    public void syncOrder(Long id) {
        ...
        // 2. 遍历执行
        for (PayOrderExtensionDO orderExtension : orderExtensions) {
            syncOrderStrictly(orderExtension);
        }
    }

    private boolean syncOrderQuietly(PayOrderExtensionDO orderExtension) {
        try {
            return syncOrderStrictly(orderExtension);
        } catch (Throwable e) {
     ...
     * @return 是否已支付
     */
    private boolean syncOrderStrictly(PayOrderExtensionDO orderExtension) {
        // 1.1 查询支付订单信息
        PayClient<?> payClient = channelService.getPayClient(orderExtension.getChannelId());
        if (payClient == null) {
            throw exception(CHANNEL_NOT_FOUND);
