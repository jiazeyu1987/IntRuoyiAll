...
public class PayOrderController {
    ...
    @Parameter(name = "id", description = "编号", required = true, example = "1024")
    @PreAuthorize("@ss.hasPermission('pay:order:query')")
    public CommonResult<PayOrderRespVO> syncOrder(@RequestParam("id") Long id) {
        orderService.syncOrder(id);
        return success(BeanUtils.toBean(orderService.getOrder(id), PayOrderRespVO.class));
    }
