...
public class IotOtaTaskServiceImpl implements IotOtaTaskService {
    ...
    public void cancelOtaTask(Long id) {
        ...
        if (ObjUtil.notEqual(upgradeTask.getStatus(), IotOtaTaskStatusEnum.IN_PROGRESS.getStatus())
                && ObjUtil.notEqual(upgradeTask.getStatus(), IotOtaTaskStatusEnum.PAUSED.getStatus())) {
    ...
    @Override
    public void pauseOtaTask(Long id) {
        // 1.1 校验升级任务是否存在
        IotOtaTaskDO upgradeTask = validateUpgradeTaskExists(id);
        // 1.2 只有进行中的任务可以暂停
        if (ObjUtil.notEqual(upgradeTask.getStatus(), IotOtaTaskStatusEnum.IN_PROGRESS.getStatus())) {
            throw exception(OTA_TASK_PAUSE_FAIL_STATUS_ERROR);
        }

        // 2. 原子更新任务状态，避免并发状态漂移
        int updateCount = otaTaskMapper.updateByIdAndStatus(id, IotOtaTaskStatusEnum.IN_PROGRESS.getStatus(),
                new IotOtaTaskDO().setStatus(IotOtaTaskStatusEnum.PAUSED.getStatus()));
        if (updateCount == 0) {
            throw exception(OTA_TASK_PAUSE_FAIL_STATUS_ERROR);
        }
    }
    ...
    @Override
    public void resumeOtaTask(Long id) {
        // 1.1 校验升级任务是否存在
        IotOtaTaskDO upgradeTask = validateUpgradeTaskExists(id);
        // 1.2 只有已暂停的任务可以继续
        if (ObjUtil.notEqual(upgradeTask.getStatus(), IotOtaTaskStatusEnum.PAUSED.getStatus())) {
            throw exception(OTA_TASK_RESUME_FAIL_STATUS_ERROR);
        }

        // 2. 原子更新任务状态，避免并发状态漂移
        int updateCount = otaTaskMapper.updateByIdAndStatus(id, IotOtaTaskStatusEnum.PAUSED.getStatus(),
                new IotOtaTaskDO().setStatus(IotOtaTaskStatusEnum.IN_PROGRESS.getStatus()));
        if (updateCount == 0) {
            throw exception(OTA_TASK_RESUME_FAIL_STATUS_ERROR);
        }
    }
