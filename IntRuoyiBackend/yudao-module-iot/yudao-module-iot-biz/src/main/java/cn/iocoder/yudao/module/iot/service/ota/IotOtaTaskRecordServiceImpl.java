...
public class IotOtaTaskRecordServiceImpl implements IotOtaTaskRecordService {
    ...
    @Override
    public List<IotOtaTaskRecordDO> getRunnableOtaRecordListByStatus(Integer status) {
        List<IotOtaTaskRecordDO> records = otaTaskRecordMapper.selectListByStatus(status);
        if (CollUtil.isEmpty(records)) {
            return records;
        }
        return records.stream()
                .filter(record -> {
                    IotOtaTaskDO task = otaTaskService.getOtaTask(record.getTaskId());
                    return task != null && IotOtaTaskStatusEnum.IN_PROGRESS.getStatus().equals(task.getStatus());
                })
                .toList();
    }
