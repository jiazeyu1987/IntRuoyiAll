...
public class IotOtaTaskController {
    ...
    @PutMapping("/pause")
    @Operation(summary = "暂停 OTA 升级任务")
    @Parameter(name = "id", description = "升级任务编号", required = true)
    @PreAuthorize(value = "@ss.hasPermission('iot:ota-task:pause')")
    public CommonResult<Boolean> pauseOtaTask(@RequestParam("id") Long id) {
        otaTaskService.pauseOtaTask(id);
        return success(true);
    }

    @PutMapping("/resume")
    @Operation(summary = "继续 OTA 升级任务")
    @Parameter(name = "id", description = "升级任务编号", required = true)
    @PreAuthorize(value = "@ss.hasPermission('iot:ota-task:resume')")
    public CommonResult<Boolean> resumeOtaTask(@RequestParam("id") Long id) {
        otaTaskService.resumeOtaTask(id);
        return success(true);
    }
