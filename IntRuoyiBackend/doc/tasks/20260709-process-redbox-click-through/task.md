...
- 后端为工序分页返回批记录表单与填写对象的稳定 ID、类型和名称，前端按真实对象跳转并过滤目标页面。
...
- 是否从根因和长期维护角度解决：是；后端返回正式结构化对象，前端只按对象类型与 ID 跳转。
- 是否存在临时补丁或绕过：否。
...
- [x] M3：实现后端结构化字段与批记录 `reportId` 过滤。
- [x] M4：实现前端红框列点击跳转与目标页 query 过滤。
- [x] M5：运行目标验证、closeout preview 并处理提交边界。

## 预期验证

- `mvn.cmd -pl yudao-module-mes "-Dtest=MesProProcessServiceImplTest,MesProBatchRecordReportServiceImplDbTest" "-Dsurefire.failIfNoSpecifiedTests=false" test`
- `node tests/e2e/mes-pro-process-redbox-click-through-static.spec.js`
- `NODE_OPTIONS=--max-old-space-size=8192 pnpm ts:check`
- `python C:\Users\BJB110\.codex\skills\backend-api-delivery\scripts\validate_backend_api.py --evidence doc/tasks/20260709-process-redbox-click-through/backend-api-evidence.md`
...
COMPLETED_WITH_COMMIT_BLOCKER：后端新增目标用例已通过，前端静态测试与类型检查已通过；完整后端测试类仍有既有升版预检用例失败，且当前混合工作区存在大量既有脏改与目标文件重叠，暂不提交以避免夹带无关改动。

## Cleanup Keep

- `doc/tasks/20260709-process-redbox-click-through/backend-api-evidence.md`
