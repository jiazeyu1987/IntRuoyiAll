...
- RED: `mvn.cmd -pl yudao-module-mes "-Dtest=MesProProcessServiceImplTest" "-Dsurefire.failIfNoSpecifiedTests=false" test` -> FAIL，缺少 `routeId`、`routeList` 和批量按工序查询路线工序接口。
...
- BLOCKER: evidence-validation -> backend evidence 初次缺少 `Scope` / `Verification` 章节，已补齐后重验。
