...
BLOCKED: test server runtime-control backend as execution host -> FAIL, `/opt/intruoyi/runtime/script/deploy/promote-int-ruoyi-test-to-prod.ps1` and `/opt/intruoyi/ops/backup-ops/linux-native/script/deploy/promote-int-ruoyi-test-to-prod.ps1` are missing; test server container also has no PowerShell executable. Per existing publish task, real publish/promote must be executed by the local runtime-control backend.
...
GREEN: forbidden data-sync scan on promote log -> PASS, no `mysqldump`, no `DROP DATABASE`, no database import command, and no `mc mirror --overwrite`.

GREEN: production runtime after promotion -> PASS, `.env` has `IMAGE_TAG=20260525_221321` and `JAVA_OPTS=-Xms1g -Xmx2g -Djava.security.egd=file:/dev/./urandom`; backend, frontend and website containers run tag `20260525_221321`.
...
GREEN: `python -m pytest script\tests\test_publish_int_ruoyi_to_test_tooling.py -q` -> PASS, 25 tests passed.
