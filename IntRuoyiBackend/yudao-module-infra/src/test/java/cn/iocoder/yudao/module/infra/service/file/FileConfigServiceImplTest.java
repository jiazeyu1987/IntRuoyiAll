...
import static cn.iocoder.yudao.module.infra.enums.ErrorCodeConstants.FILE_CONFIG_PROTECTED;
...
public class FileConfigServiceImplTest extends BaseDbUnitTest {
    ...
    @Test
    public void testCreateFileConfig_protectedShowroomConfigId() {
        Map<String, Object> config = MapUtil.<String, Object>builder().put("basePath", "/yunai")
                .put("domain", "https://www.iocoder.cn").build();
        FileConfigSaveReqVO reqVO = randomPojo(FileConfigSaveReqVO.class,
                o -> o.setId(28L).setStorage(FileStorageEnum.LOCAL.getStorage()).setConfig(config));

        assertServiceException(() -> fileConfigService.createFileConfig(reqVO), FILE_CONFIG_PROTECTED);
        assertNull(fileConfigMapper.selectById(28L));
    }
    ...
    @Test
    public void testUpdateFileConfig_protectedShowroomConfigId() {
        FileConfigDO dbFileConfig = randomPojo(FileConfigDO.class, o -> o.setId(28L)
                .setName("展厅默认文件配置")
        ...
        assertServiceException(() -> fileConfigService.updateFileConfig(reqVO), FILE_CONFIG_PROTECTED);
        FileConfigDO fileConfig = fileConfigMapper.selectById(28L);
        assertEquals("展厅默认文件配置", fileConfig.getName());
        assertEquals("/yunai", ((LocalFileClientConfig) fileConfig.getConfig()).getBasePath());
        assertEquals("https://www.iocoder.cn", ((LocalFileClientConfig) fileConfig.getConfig()).getDomain());
    }
    ...
    @Test
    public void testUpdateFileConfigMaster_protectedShowroomConfigExists() {
        FileConfigDO protectedFileConfig = randomFileConfigDO().setId(28L).setMaster(true);
        fileConfigMapper.insert(protectedFileConfig);
        FileConfigDO dbFileConfig = randomFileConfigDO().setMaster(false);
        fileConfigMapper.insert(dbFileConfig);

        assertServiceException(() -> fileConfigService.updateFileConfigMaster(dbFileConfig.getId()),
                FILE_CONFIG_PROTECTED);
        assertTrue(fileConfigMapper.selectById(28L).getMaster());
        assertFalse(fileConfigMapper.selectById(dbFileConfig.getId()).getMaster());
    }
    ...
    @Test
    public void testDeleteFileConfig_protectedShowroomConfigId() {
        FileConfigDO dbFileConfig = randomFileConfigDO().setId(28L).setMaster(false);
        fileConfigMapper.insert(dbFileConfig);

        assertServiceException(() -> fileConfigService.deleteFileConfig(28L), FILE_CONFIG_PROTECTED);
        assertNotNull(fileConfigMapper.selectById(28L));
    }
    ...
    @Test
    public void testDeleteFileConfigList_protectedShowroomConfigId() {
        FileConfigDO protectedFileConfig = randomFileConfigDO().setId(28L).setMaster(false);
        fileConfigMapper.insert(protectedFileConfig);
        FileConfigDO dbFileConfig = randomFileConfigDO().setMaster(false);
        fileConfigMapper.insert(dbFileConfig);

        assertServiceException(() -> fileConfigService.deleteFileConfigList(List.of(28L, dbFileConfig.getId())),
                FILE_CONFIG_PROTECTED);
        assertNotNull(fileConfigMapper.selectById(28L));
        assertNotNull(fileConfigMapper.selectById(dbFileConfig.getId()));
    }

    @Test
    public void testGetFileConfigPage() {
        // mock 数据
        FileConfigDO dbFileConfig = randomFileConfigDO().setName("瑛泰源码")
                .setStorage(FileStorageEnum.LOCAL.getStorage());
