...
public class DBFileClient extends AbstractFileClient<DBFileClientConfig> {
    ...
    public String move(String sourcePath, String targetPath, String type) {
        List<FileContentDO> list = fileContentMapper.selectListByConfigIdAndPath(getId(), sourcePath);
        if (CollUtil.isEmpty(list)) {
            throw new IllegalStateException("待移动 DB 文件不存在：" + sourcePath);
        }
        for (FileContentDO contentDO : list) {
            contentDO.setPath(targetPath);
            fileContentMapper.updateById(contentDO);
        }
