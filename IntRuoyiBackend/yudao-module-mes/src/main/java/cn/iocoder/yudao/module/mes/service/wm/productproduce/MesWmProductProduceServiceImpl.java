...
public class MesWmProductProduceServiceImpl implements MesWmProductProduceService {
    ...
    public void finishProductProduce(Long id) {
        ...
        // 4. 以生产入库单批次回填工单批次号
        backfillWorkOrderBatchCode(produce);
    }

    private void backfillWorkOrderBatchCode(MesWmProductProduceDO produce) {
        if (produce.getWorkOrderId() == null) {
            return;
        }
        List<String> batchCodes = productProduceLineService.getDistinctBatchCodesByWorkOrderId(produce.getWorkOrderId()).stream()
                .map(StrUtil::trimToNull)
                .filter(StrUtil::isNotBlank)
                .distinct()
                .toList();
        if (CollUtil.isEmpty(batchCodes)) {
            return;
        }
        if (batchCodes.size() > 1) {
            throw exception(WM_PRODUCT_PRODUCE_BATCH_AMBIGUOUS);
        }
        MesProWorkOrderDO workOrder = workOrderService.getWorkOrder(produce.getWorkOrderId());
        if (workOrder == null) {
            throw exception(PRO_WORK_ORDER_NOT_EXISTS);
        }
        if (StrUtil.isNotBlank(workOrder.getBatchCode())) {
            return;
        }
        workOrderService.updateBatchCodeIfBlank(produce.getWorkOrderId(), batchCodes.get(0));
    }
    ...
    public MesWmProductProduceDO generateProductProduce(MesProFeedbackDO feedback, boolean checkFlag) {
        ...
        // 1.5 获取虚拟线边库
        MesWmWarehouseDO virtualWarehouse = warehouseService.ensureWarehouseByCode(
