...
public class MesMdItemController {
    ...
    @PreAuthorize("@ss.hasPermission('mes:md-item:create')")
    public CommonResult<MesKingdeeItemSyncRespVO> syncKingdeeItems() {
        MesKingdeeItemSyncResult result = kingdeeItemSyncService.syncItems();
        return success(buildKingdeeItemSyncRespVO(result));
    }

    @PostMapping("/sync-kingdee-by-codes")
    @Operation(summary = "按编码同步金蝶物料产品")
    @PreAuthorize("@ss.hasPermission('mes:md-item:create')")
    public CommonResult<MesKingdeeItemSyncRespVO> syncKingdeeItemsByCodes(
            @Valid @RequestBody MesKingdeeItemCodeSyncReqVO reqVO) {
        MesKingdeeItemSyncResult result = kingdeeItemSyncService.syncItemsByProductCodes(reqVO.getItemCodes());
        return success(buildKingdeeItemSyncRespVO(result));
    }

    private MesKingdeeItemSyncRespVO buildKingdeeItemSyncRespVO(MesKingdeeItemSyncResult result) {
        MesKingdeeItemSyncRespVO respVO = new MesKingdeeItemSyncRespVO();
