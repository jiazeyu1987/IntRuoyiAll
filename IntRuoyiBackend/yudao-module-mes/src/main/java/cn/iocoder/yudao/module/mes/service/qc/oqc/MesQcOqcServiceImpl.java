...
public class MesQcOqcServiceImpl implements MesQcOqcService {
    ...
    @Resource
    private MesProEdhrBatchDossierItemMapper edhrDossierItemMapper;
    ...
    public void finishOqc(Long id) {
        ...
        // 4. 同步 eDHR 成品检卷宗项
        syncEdhrFinalInspectionDossier(oqc);
    }
    ...
    private void syncEdhrFinalInspectionDossier(MesQcOqcDO oqc) {
        if (StrUtil.isBlank(oqc.getBatchCode())) {
            return;
        }
        List<MesProEdhrBatchExecutionDO> candidateBatches =
                edhrBatchExecutionMapper.selectUnarchivedListByBatchCode(oqc.getBatchCode());
        if (candidateBatches.isEmpty()) {
            return;
        }
        if (oqc.getItemId() == null) {
            throw new IllegalArgumentException(
                    "OQC 单[" + oqc.getId() + "] 已匹配 eDHR 批次但 itemId 为空，无法回写成品检卷宗");
        }
        List<MesProEdhrBatchExecutionDO> matchedBatches = candidateBatches.stream()
                .filter(batch -> Objects.equals(batch.getProductId(), oqc.getItemId()))
                .toList();
        if (matchedBatches.isEmpty()) {
            throw new IllegalStateException(
                    "OQC 单[" + oqc.getId() + "] batchCode=" + oqc.getBatchCode()
                            + " 已匹配 eDHR 批次但物料不一致，无法回写成品检卷宗");
        }
        if (matchedBatches.size() > 1) {
            throw new IllegalStateException(
                    "OQC 单[" + oqc.getId() + "] batchCode=" + oqc.getBatchCode()
                            + "、itemId=" + oqc.getItemId() + " 匹配多个未归档 eDHR 批次，无法确定成品检卷宗");
        }

        MesProEdhrBatchDossierItemDO finalInspection =
                edhrDossierItemMapper.selectRequiredFinalInspection(matchedBatches.get(0).getId());
        if (finalInspection == null) {
            throw new IllegalStateException(
                    "eDHR 批次[" + matchedBatches.get(0).getId() + "] 缺少必需成品检卷宗项，无法回写 OQC");
        }

        LocalDateTime now = LocalDateTime.now();
        MesProEdhrBatchDossierItemDO updateItem = new MesProEdhrBatchDossierItemDO()
                .setId(finalInspection.getId())
                .setSourceDocType(MesProEdhrDossierConstants.SOURCE_DOC_TYPE_OQC)
                .setSourceDocId(oqc.getId())
                .setSourceDocCode(oqc.getCode())
                .setSourceDocStatus(MesProEdhrDossierConstants.SOURCE_DOC_STATUS_FINISHED)
                .setSourceDocHash(buildEdhrOqcSourceHash(oqc))
                .setVerifiedAt(now);
        if (Objects.equals(oqc.getCheckResult(), MesQcCheckResultEnum.PASS.getType())) {
            updateItem.setItemStatus(MesProEdhrDossierConstants.ITEM_STATUS_COMPLETED)
                    .setSourceDocResult(MesProEdhrDossierConstants.SOURCE_DOC_RESULT_PASS)
                    .setCompletedAt(now)
                    .setBlockerCode(null)
                    .setBlockerMessage(null);
        } else if (Objects.equals(oqc.getCheckResult(), MesQcCheckResultEnum.FAIL.getType())) {
            updateItem.setItemStatus(MesProEdhrDossierConstants.ITEM_STATUS_BLOCKED)
                    .setSourceDocResult(MesProEdhrDossierConstants.SOURCE_DOC_RESULT_FAIL)
                    .setCompletedAt(null)
                    .setBlockerCode(MesProEdhrDossierConstants.BLOCKER_CODE_OQC_RESULT_FAIL)
                    .setBlockerMessage(MesProEdhrDossierConstants.BLOCKER_MESSAGE_OQC_RESULT_FAIL);
        } else {
            throw new IllegalArgumentException(
                    "OQC 单[" + oqc.getId() + "] checkResult=" + oqc.getCheckResult()
                            + " 无法映射到 eDHR 成品检卷宗结果");
        }
        edhrDossierItemMapper.updateFinalInspectionFromOqc(updateItem);
    }

    private String buildEdhrOqcSourceHash(MesQcOqcDO oqc) {
        return DigestUtil.sha256Hex(String.join("|",
                MesProEdhrDossierConstants.SOURCE_DOC_TYPE_OQC,
                Objects.toString(oqc.getId(), ""),
                Objects.toString(oqc.getCode(), ""),
                Objects.toString(oqc.getBatchCode(), ""),
                Objects.toString(oqc.getItemId(), ""),
                Objects.toString(oqc.getCheckResult(), ""),
                Objects.toString(oqc.getInspectorUserId(), ""),
                Objects.toString(oqc.getInspectDate(), "")));
    }
