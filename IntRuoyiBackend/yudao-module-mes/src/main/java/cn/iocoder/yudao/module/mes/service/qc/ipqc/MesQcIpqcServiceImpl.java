...
public class MesQcIpqcServiceImpl implements MesQcIpqcService {
    ...
    public Long createIpqc(MesQcIpqcSaveReqVO createReqVO) {
        ...
        // 1.4 获取来源单据编号
        SourceDocContext sourceDocContext = validateAndGetSourceDocContext(
        ...
        MesQcIpqcDO ipqc = BeanUtils.toBean(createReqVO, MesQcIpqcDO.class)
                ...
                .setProcessId(getProcessId(workOrder.getProductId(), workstation.getProcessId(),
                        sourceDocContext.processId()))
                .setTemplateId(templateId)
                .setSourceDocCode(sourceDocContext.code())
    ...
    public void updateIpqc(MesQcIpqcSaveReqVO updateReqVO) {
        ...
        updateObj.setProcessId(getProcessId(existIpqc.getItemId(), workstation.getProcessId(),
                resolveSourceFeedbackProcessId(existIpqc)));
    ...
    private SourceDocContext validateAndGetSourceDocContext(Integer sourceDocType, Long sourceDocId, Long sourceLineId) {
        if (sourceDocType == null || sourceDocId == null) {
            return new SourceDocContext(null, null);
        }
        if (Objects.equals(sourceDocType, MesBizTypeConstants.PRO_FEEDBACK)) {
            ...
            return new SourceDocContext(feedback.getCode(), feedback.getProcessId());
        }
        // 未知来源类型应报错，而不是静默忽略
     ...
     */
    private Long getProcessId(Long productId, Long workstationProcessId, Long sourceProcessId) {
        if (isPositiveId(sourceProcessId)) {
        ...
        // 2. 将工位保存的历史工序身份解析为当前路线工序
        return routeProcessService.resolveCurrentRouteProcess(
                null, routeProduct.getRouteId(), workstationProcessId).getProcessId();
    }
    ...
    private Long resolveSourceFeedbackProcessId(MesQcIpqcDO ipqc) {
        if (ipqc == null
                || !Objects.equals(ipqc.getSourceDocType(), MesBizTypeConstants.PRO_FEEDBACK)
                || ipqc.getSourceDocId() == null) {
            return null;
        }
        MesProFeedbackDO feedback = feedbackService.validateFeedbackExists(ipqc.getSourceDocId());
        return feedback == null ? null : feedback.getProcessId();
    }

    private record SourceDocContext(String code, Long processId) {
    }
