...
public class MesMdAutoCodeRecordServiceImpl implements MesMdAutoCodeRecordService {
    ...
    public String generateAutoCode(String ruleCode, String inputChar) {
        ...
        for (MesMdAutoCodePartDO part : parts) {
            ...
            // 统一截取指定长度；流水号长度表示最小补零宽度，超过后不能截断回旧编码
            partCode = normalizePartCode(part, partCode);
            // 拼接分段编码
    ...
    private String normalizePartCode(MesMdAutoCodePartDO part, String partCode) {
        if (MesMdAutoCodePartTypeEnum.SERIAL_NUMBER.getType().equals(part.getType())
                && partCode != null && partCode.length() > part.getLength()) {
            return partCode;
        }
        return StrUtil.sub(partCode, 0, part.getLength());
    }
