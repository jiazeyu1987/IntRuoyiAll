...
import cn.iocoder.yudao.module.mes.controller.admin.md.item.vo.bom.MesKingdeeProductBomSyncRespVO;
...
public class MesMdProductBomController {
    ...
    @Resource
    private MesKingdeeProductBomSyncService kingdeeProductBomSyncService;
    ...
    @PostMapping("/{itemId}/sync-erp-bom")
    @Operation(summary = "鍚屾鎸囧畾鐗╂枡/浜у搧鐨?ERP BOM")
    @Parameter(name = "itemId", description = "鐗╂枡/浜у搧缂栧彿", required = true, example = "1")
    @PreAuthorize("@ss.hasPermission('mes:md-item:update')")
    public CommonResult<MesKingdeeProductBomSyncRespVO> syncErpBom(@PathVariable("itemId") Long itemId) {
        MesKingdeeProductBomSyncResult result = kingdeeProductBomSyncService.syncErpBom(itemId);
        return success(BeanUtils.toBean(result, MesKingdeeProductBomSyncRespVO.class));
    }
