...
public class MesProTaskServiceImpl implements MesProTaskService {
    ...
    public void updateProducedQuantity(Long id,
        ...
        // 校验任务存在
        MesProTaskDO task = validateTaskExists(id);
        if (MesProTaskStatusEnum.PREPARE.getStatus().equals(task.getStatus())
                && incrProducedQuantity != null
                && incrProducedQuantity.signum() > 0) {
            taskMapper.updateById(new MesProTaskDO()
                    .setId(id)
                    .setStatus(MesProTaskStatusEnum.IN_PROGRESS.getStatus()));
        }
        // 更新数量
    ...
    @Override
    public void lockTask(Long id, String lockedReason) {
        validateTaskNotFinished(id);
        MesProTaskScheduleExtDO ext = taskScheduleExtMapper.selectByTaskId(id);
        if (ext == null) {
            taskScheduleExtMapper.insert(MesProTaskScheduleExtDO.builder()
                    .taskId(id)
                    .scheduleSource("MANUAL")
                    .locked(Boolean.TRUE)
                    .lockedReason(StrUtil.blankToDefault(lockedReason, "MANUAL_LOCK"))
                    .riskStatus("NONE")
                    .remark("LOCK")
                    .build());
            return;
        }
        ext.setLocked(Boolean.TRUE);
        ext.setLockedReason(StrUtil.blankToDefault(lockedReason, "MANUAL_LOCK"));
        taskScheduleExtMapper.updateById(ext);
    }
    ...
    public void unlockTask(Long id) {
        ...
        ext.setLocked(Boolean.FALSE);
        ext.setLockedReason(null);
        taskScheduleExtMapper.updateById(ext);
    }
