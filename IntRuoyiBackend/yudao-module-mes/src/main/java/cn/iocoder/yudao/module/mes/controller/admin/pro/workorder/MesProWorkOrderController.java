...
public class MesProWorkOrderController {
    ...
    private List<MesProWorkOrderRespVO> buildWorkOrderRespVOList(List<MesProWorkOrderDO> list) { if (CollUtil.isEmpty(list)) return Collections.emptyList(); Map<Long, MesProWorkOrderDO> workOrderMap = list.stream().collect(java.util.stream.Collectors.toMap(MesProWorkOrderDO::getId, workOrder -> workOrder, (left, right) -> left, LinkedHashMap::new)); Map<Long, MesMdItemDO> itemMap = itemService.getItemMap(convertSet(list, MesProWorkOrderDO::getProductId)); Map<Long, MesMdClientDO> clientMap = clientService.getClientMap(convertSet(list, MesProWorkOrderDO::getClientId)); Map<Long, MesMdVendorDO> vendorMap = vendorService.getVendorMap(convertSet(list, MesProWorkOrderDO::getVendorId)); Map<Long, MesMdUnitMeasureDO> unitMeasureMap = unitMeasureService.getUnitMeasureMap(convertSet(itemMap.values(), MesMdItemDO::getUnitMeasureId)); Map<Long, MesProWorkOrderDO> parentMap = workOrderService.getWorkOrderMap(convertSet(list, MesProWorkOrderDO::getParentId)); Map<Long, List<MesKingdeeProductionMaterialListDO>> materialListMap = buildMaterialListMapByWorkOrderId(list); return BeanUtils.toBean(list, MesProWorkOrderRespVO.class, vo -> { MesProWorkOrderDO workOrder = workOrderMap.get(vo.getId()); if (workOrder != null) { vo.setProductSpecification(workOrder.getMaterialSpecification()); } MapUtils.findAndThen(itemMap, vo.getProductId(), item -> { vo.setProductName(item.getName()).setProductCode(item.getCode()); MapUtils.findAndThen(unitMeasureMap, item.getUnitMeasureId(), unitMeasure -> vo.setUnitMeasureName(unitMeasure.getName())); }); MapUtils.findAndThen(clientMap, vo.getClientId(), client -> vo.setClientName(client.getName()).setClientCode(client.getCode())); MapUtils.findAndThen(vendorMap, vo.getVendorId(), vendor -> vo.setVendorName(vendor.getName()).setVendorCode(vendor.getCode())); MapUtils.findAndThen(parentMap, vo.getParentId(), parent -> vo.setParentCode(parent.getCode())); List<MesKingdeeProductionMaterialListDO> materialRows = materialListMap.get(vo.getId()); if (CollUtil.isNotEmpty(materialRows)) { LinkedHashSet<String> billNos = new LinkedHashSet<>(convertList(materialRows, MesKingdeeProductionMaterialListDO::getSourceBillNo)); billNos.remove(null); billNos.remove(""); vo.setProductionMaterialListCount((long) billNos.size()); vo.setProductionMaterialListSummary(billNos.isEmpty() ? null : String.join("、", billNos)); } else { vo.setProductionMaterialListCount(0L); vo.setProductionMaterialListSummary(null); } }); }
