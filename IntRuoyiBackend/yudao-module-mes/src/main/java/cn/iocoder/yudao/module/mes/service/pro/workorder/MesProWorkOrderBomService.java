...
public interface MesProWorkOrderBomService {
     ...
     *
     * @param workOrderId 工单编号
     * @return 叶子物料 itemId -> 需求数量
     */
    Map<Long, BigDecimal> getWorkOrderMaterialDemandByWorkOrderId(Long workOrderId);

    /**
     * 根据工单编号列表获得展开后的叶子物料需求
     *
     * @param workOrderIds 工单编号列表
     * @return 工单编号 -> (叶子物料 itemId -> 需求数量)
     */
    Map<Long, Map<Long, BigDecimal>> getWorkOrderMaterialDemandMapByWorkOrderIds(Collection<Long> workOrderIds);
    ...
    /**
     * 根据已变更的产品 BOM 重算未关闭工单 BOM。
     *
     * @param productIds 产品物料编号集合
     * @return 重算的工单数量
     */
    int regenerateOpenWorkOrderBomByProductIds(Collection<Long> productIds);
