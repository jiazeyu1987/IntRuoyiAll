...
public interface MesProTaskMapper extends BaseMapperX<MesProTaskDO> {
    ...
    default MesProTaskDO selectByCode(String code) {
        return selectOne(MesProTaskDO::getCode, code);
    }

    default List<MesProTaskDO> selectListByCodes(Collection<String> codes) {
        if (codes == null || codes.isEmpty()) {
            return Collections.emptyList();
        }
        return selectList(new LambdaQueryWrapperX<MesProTaskDO>()
                .in(MesProTaskDO::getCode, codes)
                .orderByDesc(MesProTaskDO::getId));
    }
    ...
    default List<MesProTaskDO> selectListByWorkstationIds(Collection<Long> workstationIds) {
        if (workstationIds == null || workstationIds.isEmpty()) {
            return Collections.emptyList();
        }
        return selectList(new LambdaQueryWrapperX<MesProTaskDO>()
                .in(MesProTaskDO::getWorkstationId, workstationIds)
                .orderByAsc(MesProTaskDO::getStartTime)
                .orderByAsc(MesProTaskDO::getId));
    }

    default List<MesProTaskDO> selectListByIds(Collection<Long> ids) {
    ...
    default List<MesProTaskDO> selectListByStartTimeRange(LocalDateTime startTime, LocalDateTime endTime) {
        return selectList(new LambdaQueryWrapperX<MesProTaskDO>()
                ...
                .ltIfPresent(MesProTaskDO::getStartTime, endTime)
                .orderByAsc(MesProTaskDO::getStartTime)
                .orderByAsc(MesProTaskDO::getId));
    }

    default List<MesProTaskDO> selectListByCalendarRange(LocalDateTime startTime, LocalDateTime endTime) {
        return selectList(new LambdaQueryWrapperX<MesProTaskDO>()
                .ltIfPresent(MesProTaskDO::getStartTime, endTime)
                .gtIfPresent(MesProTaskDO::getEndTime, startTime)
                .orderByAsc(MesProTaskDO::getStartTime)
                .orderByAsc(MesProTaskDO::getId));
    }

    default Long selectCurrentScheduleCount() {
        return selectCount(new LambdaQueryWrapperX<>());
    }

    default MesProTaskDO selectLatestUpdatedTask() {
        return selectOne(new LambdaQueryWrapperX<MesProTaskDO>()
                .orderByDesc(MesProTaskDO::getUpdateTime)
                .orderByDesc(MesProTaskDO::getId)
                .last("LIMIT 1"));
    }
