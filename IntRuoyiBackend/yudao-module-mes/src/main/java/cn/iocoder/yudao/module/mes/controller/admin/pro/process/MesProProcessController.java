...
public class MesProProcessController {
    ...
    public CommonResult<MesProProcessRespVO> getProcess(@RequestParam("id") Long id,
        ...
        return success(processService.getProcessWithCapacity(id, routeId));
    }
    ...
    public CommonResult<PageResult<MesProProcessRespVO>> getProcessPage(@Valid MesProProcessPageReqVO pageReqVO) {
        return success(processService.getProcessPageWithCapacity(pageReqVO));
    }

    @GetMapping("/machinery-list")
    @Operation(summary = "获得生产工序关联设备产能明细")
    @Parameter(name = "processId", description = "工序编号", required = true)
    @PreAuthorize("@ss.hasPermission('mes:pro-process:query')")
    public CommonResult<List<MesProProcessMachineryRespVO>> getProcessMachineryList(@RequestParam("processId") Long processId) {
        return success(processService.getProcessMachineryList(processId));
    }
    ...
    public void exportProcessExcel(@Valid MesProProcessPageReqVO pageReqVO,
        ...
        List<MesProProcessRespVO> data = toBean(processService.getProcessPage(pageReqVO).getList(), MesProProcessRespVO.class);
