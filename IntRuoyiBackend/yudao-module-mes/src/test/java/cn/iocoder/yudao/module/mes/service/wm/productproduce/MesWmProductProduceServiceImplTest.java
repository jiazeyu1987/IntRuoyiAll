...
public class MesWmProductProduceServiceImplTest {
    ...
    @Test
    public void testFinishProductProduce_backfillsWorkOrderBatchCodeFromSingleProduceBatch() {
        Long workOrderId = randomLongId();
        Long produceId = randomLongId();
        MesWmProductProduceDO produce = MesWmProductProduceDO.builder()
                .id(produceId)
                .workOrderId(workOrderId)
                .status(MesWmProductProduceStatusEnum.PREPARE.getStatus())
                .build();
        when(productProduceMapper.selectById(produceId)).thenReturn(produce);
        MesWmProductProduceLineDO line = MesWmProductProduceLineDO.builder()
                .id(100L)
                .produceId(produceId)
                .quantity(BigDecimal.TEN)
                .batchCode("MES-BATCH-001")
                .build();
        when(productProduceLineService.getProductProduceLineListByProduceId(produceId)).thenReturn(ListUtil.of(line));
        when(productProduceLineService.getDistinctBatchCodesByWorkOrderId(workOrderId)).thenReturn(ListUtil.of("MES-BATCH-001"));
        when(productProduceDetailService.getProductProduceDetailListByLineId(100L)).thenReturn(ListUtil.of(
                MesWmProductProduceDetailDO.builder()
                        .lineId(100L)
                        .itemId(randomLongId())
                        .quantity(BigDecimal.TEN)
                        .batchCode("MES-BATCH-001")
                        .build()));
        when(workOrderService.getWorkOrder(workOrderId)).thenReturn(MesProWorkOrderDO.builder()
                .id(workOrderId)
                .batchCode(null)
                .build());

        productProduceService.finishProductProduce(produceId);

        verify(workOrderService).updateBatchCodeIfBlank(workOrderId, "MES-BATCH-001");
        assertProduceFinishedUpdated(produceId);
    }

    @Test
    public void testFinishProductProduce_blocksBackfillWhenProduceBatchesAreAmbiguous() {
        Long workOrderId = randomLongId();
        Long produceId = randomLongId();
        MesWmProductProduceDO produce = MesWmProductProduceDO.builder()
                .id(produceId)
        ...
        MesWmProductProduceLineDO line = MesWmProductProduceLineDO.builder()
                ...
                .build();
        when(productProduceLineService.getProductProduceLineListByProduceId(produceId)).thenReturn(ListUtil.of(line));
        when(productProduceLineService.getDistinctBatchCodesByWorkOrderId(workOrderId))
                .thenReturn(ListUtil.of("MES-BATCH-001", "MES-BATCH-002"));
        when(productProduceDetailService.getProductProduceDetailListByLineId(100L)).thenReturn(ListUtil.of(
                MesWmProductProduceDetailDO.builder()
                        .lineId(100L)
                        .itemId(randomLongId())
        ...
        ServiceException ex = assertThrows(ServiceException.class, () -> productProduceService.finishProductProduce(produceId));

        assertEquals(WM_PRODUCT_PRODUCE_BATCH_AMBIGUOUS.getCode(), ex.getCode());
        verify(workOrderService, never()).updateBatchCodeIfBlank(any(), any());
    }
    ...
    public void testSplitPendingAndFinishProduce_withUnqualified() {
        ...
        MesWmProductProduceDO produce = MesWmProductProduceDO.builder()
                ...
                .status(MesWmProductProduceStatusEnum.PREPARE.getStatus())
                .build();
        when(productProduceMapper.selectByFeedbackId(feedbackId)).thenReturn(produce);
        when(productProduceMapper.selectById(produce.getId())).thenReturn(produce);
        ...
        when(warehouseService.ensureWarehouseByCode(MesWmWarehouseDO.WIP_VIRTUAL_WAREHOUSE)).thenReturn(wh);
        ...
        // 断言 4：产出单状态更新为已完成
        assertProduceFinishedUpdated(produce.getId());
    ...
    public void testSplitPendingAndFinishProduce_allQualified() {
        ...
        MesWmWarehouseLocationDO loc = new MesWmWarehouseLocationDO(); loc.setId(locId);
        MesWmWarehouseAreaDO area = new MesWmWarehouseAreaDO(); area.setId(areaId);
        when(warehouseService.ensureWarehouseByCode(any())).thenReturn(wh);
    ...
    private void assertProduceFinishedUpdated(Long produceId) {
        ArgumentCaptor<MesWmProductProduceDO> updateCaptor = ArgumentCaptor.forClass(MesWmProductProduceDO.class);
        verify(productProduceMapper).updateById(updateCaptor.capture());
        MesWmProductProduceDO update = updateCaptor.getValue();
        assertEquals(produceId, update.getId());
        assertEquals(MesWmProductProduceStatusEnum.FINISHED.getStatus(), update.getStatus());
    }
