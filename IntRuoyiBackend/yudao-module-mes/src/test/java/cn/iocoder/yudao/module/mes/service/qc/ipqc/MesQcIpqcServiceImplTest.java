...
public class MesQcIpqcServiceImplTest extends BaseDbUnitTest {
    ...
    public void testCreateIpqc_processId_workstationHasProcess() {
        ...
        Long legacyProcessId = randomLongId();
        Long currentProcessId = randomLongId();
        ...
        workstation.setProcessId(legacyProcessId);
        ...
        // mock 工序在路线中
        when(routeProcessService.resolveCurrentRouteProcess(null, routeId, legacyProcessId))
                .thenReturn(MesProRouteProcessDO.builder()
                        .routeId(routeId).processId(currentProcessId).build());
        ...
        assertEquals(currentProcessId, ipqc.getProcessId());
    }
    ...
    public void testCreateIpqc_processId_processNotInRoute() {
        ...
        when(routeProcessService.resolveCurrentRouteProcess(null, routeId, processId))
                .thenThrow(exception(PRO_ROUTE_PROCESS_IDENTITY_NOT_FOUND,
                        routeId, processId, null, null));
    ...
    @Test
    public void createIpqc_fromFeedbackSource_shouldUseFeedbackFrozenProcessWithoutCurrentRouteRemap() {
        Long workstationId = randomLongId();
        Long workOrderId = randomLongId();
        Long productId = randomLongId();
        Long feedbackId = randomLongId();
        Long sourceLineId = randomLongId();
        Long templateId = randomLongId();
        Long legacyWorkstationProcessId = randomLongId();
        Long frozenFeedbackProcessId = randomLongId();
        Long routeId = randomLongId();

        MesQcIpqcSaveReqVO reqVO = new MesQcIpqcSaveReqVO();
        reqVO.setCode("IPQC-FROZEN-FB-001");
        reqVO.setWorkstationId(workstationId);
        reqVO.setWorkOrderId(workOrderId);
        reqVO.setInspectorUserId(randomLongId());
        reqVO.setSourceDocType(MesBizTypeConstants.PRO_FEEDBACK);
        ...
        MesMdWorkstationDO workstation = new MesMdWorkstationDO();
        workstation.setId(workstationId);
        workstation.setProcessId(legacyWorkstationProcessId);
        when(workstationService.validateWorkstationExists(workstationId)).thenReturn(workstation);
        MesProWorkOrderDO workOrder = new MesProWorkOrderDO();
        workOrder.setId(workOrderId);
        workOrder.setProductId(productId);
        ...
        MesProFeedbackDO feedback = new MesProFeedbackDO();
        feedback.setId(feedbackId);
        feedback.setCode("FB-FROZEN-001");
        feedback.setProcessId(frozenFeedbackProcessId);
        when(feedbackService.validateFeedbackExists(feedbackId)).thenReturn(feedback);
        MesWmProductProduceLineDO line = new MesWmProductProduceLineDO();
        line.setId(sourceLineId);
        line.setFeedbackId(feedbackId);
        ...
        routeProduct.setRouteId(routeId);
        when(routeProductService.getRouteProductByItemId(productId)).thenReturn(routeProduct);
        when(routeProcessService.resolveCurrentRouteProcess(null, routeId, legacyWorkstationProcessId))
                .thenReturn(MesProRouteProcessDO.builder()
                        .routeId(routeId)
        ...
        MesQcIpqcDO ipqc = ipqcMapper.selectById(ipqcId);
        assertEquals(frozenFeedbackProcessId, ipqc.getProcessId());
        verify(routeProcessService, never())
                .resolveCurrentRouteProcess(null, routeId, legacyWorkstationProcessId);
    }

    @Test
    public void createIpqc_fromFeedbackSourceWithZeroProcess_shouldResolveFromRouteIdentity() {
        Long workstationId = randomLongId();
        Long workOrderId = randomLongId();
        Long productId = randomLongId();
        Long feedbackId = randomLongId();
        ...
        MesQcIpqcSaveReqVO reqVO = new MesQcIpqcSaveReqVO();
        reqVO.setCode("IPQC-FB-ZERO-PROCESS");
        reqVO.setWorkstationId(workstationId);
        reqVO.setWorkOrderId(workOrderId);
        reqVO.setInspectorUserId(randomLongId());
        reqVO.setSourceDocType(MesBizTypeConstants.PRO_FEEDBACK);
        ...
        MesProFeedbackDO feedback = new MesProFeedbackDO();
        feedback.setId(feedbackId);
        feedback.setCode("FB-ZERO-PROCESS");
        feedback.setProcessId(0L);
        when(feedbackService.validateFeedbackExists(feedbackId)).thenReturn(feedback);
        MesWmProductProduceLineDO line = new MesWmProductProduceLineDO();
        line.setId(sourceLineId);
        line.setFeedbackId(feedbackId);
        ...
        MesQcIpqcDO ipqc = ipqcMapper.selectById(ipqcId);
        assertEquals(currentProcessId, ipqc.getProcessId());
        verify(routeProcessService).resolveCurrentRouteProcess(null, routeId, legacyWorkstationProcessId);
    }
