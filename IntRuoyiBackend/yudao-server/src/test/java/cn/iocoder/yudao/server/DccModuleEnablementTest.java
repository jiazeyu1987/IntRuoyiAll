...
class DccModuleEnablementTest {

    private final Path projectDir = findProjectDir();

    @Test
    void rootPomShouldIncludeDccModule() throws Exception {
        Document document = readPom(projectDir.resolve("pom.xml"));

        assertTrue(hasModule(document, "yudao-module-dcc"),
                "root pom.xml must include yudao-module-dcc in <modules>");
    }

    @Test
    void serverPomShouldDependOnDccModule() throws Exception {
        Document document = readPom(projectDir.resolve("yudao-server/pom.xml"));

        assertTrue(hasDependency(document, "cn.iocoder.boot", "yudao-module-dcc"),
                "yudao-server/pom.xml must depend on cn.iocoder.boot:yudao-module-dcc");
    }
