...
class ReportModuleEnablementTest {

    private final Path projectDir = findProjectDir();

    @Test
    void rootPomShouldIncludeReportModule() throws Exception {
        Document document = readPom(projectDir.resolve("pom.xml"));

        assertTrue(hasModule(document, "yudao-module-report"),
                "root pom.xml must include yudao-module-report in <modules> because Report Management menus are enabled");
    }

    @Test
    void serverPomShouldDependOnReportModule() throws Exception {
        Document document = readPom(projectDir.resolve("yudao-server/pom.xml"));

        assertTrue(hasDependency(document, "cn.iocoder.boot", "yudao-module-report"),
                "yudao-server/pom.xml must depend on cn.iocoder.boot:yudao-module-report because Report Management routes are enabled");
    }
