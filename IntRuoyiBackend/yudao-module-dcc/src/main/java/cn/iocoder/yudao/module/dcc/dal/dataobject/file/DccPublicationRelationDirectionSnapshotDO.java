package cn.iocoder.yudao.module.dcc.dal.dataobject.file;

import cn.iocoder.yudao.framework.tenant.core.db.TenantBaseDO;
import com.baomidou.mybatisplus.annotation.TableId;
import com.baomidou.mybatisplus.annotation.TableName;
import lombok.AllArgsConstructor;
import lombok.Builder;
import lombok.Data;
import lombok.EqualsAndHashCode;
import lombok.NoArgsConstructor;
import lombok.ToString;

@TableName("dcc_publication_relation_direction_snapshot")
@Data
@EqualsAndHashCode(callSuper = true)
@ToString(callSuper = true)
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class DccPublicationRelationDirectionSnapshotDO extends TenantBaseDO {

    @TableId
    private Long id;
    private Long batchId;
    private Long relationSnapshotId;
    private String direction;
    private Long sourceRelationId;
    private Long sourceControlledFileId;
    private Long targetControlledFileId;
    private String relationSourceSnapshot;
}
