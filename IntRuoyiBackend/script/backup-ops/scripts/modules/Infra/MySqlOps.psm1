...
function Assert-BackupOpsMySqlParentPayloadIntegrity {
    ...
    $payloads = @(
        if ([string]$Manifest.backupKind -eq 'FULL') {
            [pscustomobject]@{ path = [string]$Manifest.mysqlEvidence.dumpPath; sha256 = [string]$Manifest.mysqlEvidence.sha256 }
        } else {
            @($Manifest.mysqlEvidence.segments)
        }
    )
...
function Get-BackupOpsPreviousMySqlChainPoint {
    ...
    if ([string]$manifest.status -ne 'success' -or [string]$manifest.backupId -ne $expectedBackupId -or $kind -notin @('FULL', 'INCREMENTAL') -or
        [string]::IsNullOrWhiteSpace($baseBackupId) -or [string]::IsNullOrWhiteSpace($endFile) -or $endPosition -le 0) {
    ...
    Assert-BackupOpsMySqlParentPayloadIntegrity -SshRequest $testRequest -BackupPointPath $path.TrimEnd('/') -Manifest $manifest
    return [pscustomobject]([ordered]@{
...
function Export-BackupOpsMySqlBinlogIncrement {
    ...
    for ($index = $startIndex; $index -le $endIndex; $index++) {
        ...
        $mysqlbinlog = 'mysqlbinlog --verify-binlog-checksum --read-from-remote-server --host=127.0.0.1 --user=root --password={0} --start-position={1} --stop-position={2} {3}' -f
