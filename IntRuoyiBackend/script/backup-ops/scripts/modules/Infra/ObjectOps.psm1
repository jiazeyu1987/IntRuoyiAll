...
function Get-BackupOpsPreviousManifestObjects {
    ...
    if ($ExpectedParentBackupId -notmatch '^\d{8}-\d{6}$') {
        throw (New-BackupOpsObjectException -Code 'INTBK-4001' -Message "Object incremental expected parent backup ID is invalid: $ExpectedParentBackupId.")
    }
    $manifestPath = $BackupPointsRoot.TrimEnd('/') + "/$ExpectedParentBackupId/manifest/manifest.json"
    try {
        ...
        throw (New-BackupOpsObjectException -Code 'INTBK-4001' -Message "Object incremental expected parent manifest is unreadable: $manifestPath. $($_.Exception.Message)")
    }
    ...
    if ($manifestStatus -ne 'success' -or $manifestBackupId -ne $ExpectedParentBackupId -or
        $manifestKind -notin @('FULL', 'INCREMENTAL') -or
        -not $manifest.PSObject.Properties['objects']) {
        throw (New-BackupOpsObjectException -Code 'INTBK-4001' -Message "Object incremental expected parent manifest is not a valid chain point: $manifestPath.")
    }
...
function Write-BackupOpsObjectInventoryMarker {
    ...
    $marker = [pscustomobject]([ordered]@{
            ...
            parentBackupId = $ExpectedParentBackupId
            bucket = $Bucket
...
function Export-BackupObjectSnapshotToRemoteNas {
    ...
    $expectedParentBackupId = if ($backupKind -eq 'INCREMENTAL') {
        Get-BackupObjectFieldValue -Request $Request -Name 'ExpectedParentBackupId'
    } else {
        ''
    }
    ...
    try {
        ...
        $previousObjects = if ($backupKind -eq 'FULL') { @() } else {
            @(Get-BackupOpsPreviousManifestObjects -BackupPointsRoot $backupPointsRoot -ExpectedParentBackupId $expectedParentBackupId -SshRequest $sshRequest)
        }
        ...
        $markerPath = Write-BackupOpsObjectInventoryMarker -TargetPath $targetPath -Bucket $bucket -ObjectStoreRoot $objectStoreRoot -BackupKind $backupKind -ExpectedParentBackupId $expectedParentBackupId -Inventory $inventory
        try {
            ...
            if ($copyPlan.Count -gt 0) {
                ...
                $copyScriptLines = @(
                    ...
                    '    existing=$(sha256sum "/object-store/$repo" | cut -d '' '' -f1)',
                    '    if [ "$existing" != "$repo" ]; then',
                    '      if [ ! -f "/object-store/$existing" ]; then',
                    '        ln "/object-store/$repo" "/object-store/$existing"',
                    '      else',
                    '        existing_copy=$(sha256sum "/object-store/$existing" | cut -d '' '' -f1)',
                    ...
                    '  while true; do',
                    '    rm -f "$incoming"',
                    ('    if mc cp "src/' + $bucket + '/$rel" "$incoming" >/dev/null; then'),
                    '      break',
                    '    fi',
...
function Backup-BackupOpsObjectBucket {
    ...
    if ($BackupKind -eq 'INCREMENTAL' -and $ExpectedParentBackupId -notmatch '^\d{8}-\d{6}$') {
        throw (New-BackupOpsObjectException -Code 'INTBK-4001' -Message 'Object INCREMENTAL backup requires the MySQL direct parent backup ID.')
    }
    ...
    return Export-BackupObjectSnapshotToRemoteNas -Request @{
        ...
        BackupKind = $BackupKind
        ExpectedParentBackupId = $ExpectedParentBackupId
