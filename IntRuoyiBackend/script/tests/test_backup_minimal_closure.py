...
function Invoke-BackupSshCommand {{
    ...
    if ($command -like 'cat*manifest/manifest.json*') {{
            return [pscustomobject]@{{ output = '{{"status":"success","backupKind":"FULL","backupId":"20260907-000000","baseBackupId":"20260907-000000","mysqlEvidence":{{"schemaVersion":"mysql-full-dump-v1","dumpPath":"mysql/ruoyi-vue-pro.sql.gz","sha256":"{('b' * 64)}","endPosition":{{"file":"mysql-bin.000001","position":100}}}}}}' }}
    }}
...
def test_full_dcc_manifest_does_not_reuse_a_previous_restore_point() -> None:
    ...
    assert "INCREMENTAL requires a previous DCC backup manifest" in dcc
