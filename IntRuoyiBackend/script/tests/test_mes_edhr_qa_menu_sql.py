...
def test_edhr_qa_menu_declares_release_metadata_and_fail_fast_guards() -> None:
    ...
    assert "system_menu id 900436 is already used by another active menu" in text
    assert "system_menu id 900437 is already used by another active menu" in text
    ...
    assert "Production leader menu route already exists on a different menu id" in text
    assert "Frontline production menu route already exists on a different menu id" in text
...
def test_edhr_qa_menu_inserts_between_batch_record_form_and_batch_execution() -> None:
    ...
    expected_children = [
        ...
        (
            ...
            "mes/pro/processpool/ProductionLeaderWorkbenchPage",
            "MesProProcessPoolProductionLeaderWorkbench",
        ),
        (
            900437,
            "一线生产",
            ...
            "mes/pro/edhr-batch/BatchProductionFillPage",
            "MesProEdhrBatchProductionFill",
        ),
        (
            900033,
            "批次执行",
    ...
    assert "COUNT(*) FROM `tmp_mes_edhr_qa_visible_order`) <> 8" in text
...
def test_edhr_qa_menu_is_bound_to_tenant_packages_and_admin_roles() -> None:
    ...
    for required in [
        ...
        "QA, leader, or frontline production menu is not bound to any admin role",
        "QA, leader, or frontline production menu is missing from target tenant packages",
    ]:
