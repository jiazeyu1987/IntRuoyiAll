...
DROP PROCEDURE IF EXISTS ensure_mes_scheduler_edhr_route_900026_route_edit_permission;

DELIMITER //
CREATE PROCEDURE ensure_mes_scheduler_edhr_route_900026_route_edit_permission()
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM `mes_pro_route`
    ...
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Missing route 900026 for scheduler tenant';
  END IF;
  ...
  SELECT
    ...
    'ENABLED',
    1,
    'scheduler-edhr-route-900026-route-edit',
    NOW(),
    'scheduler-edhr-route-900026-route-edit',
    NOW(),
    b'0',
  ...
  SET `scope`.`scope_name` = IF(
      ...
      `scope`.`updater` = 'scheduler-edhr-route-900026-route-edit',
      `scope`.`update_time` = NOW()
  WHERE `scope`.`object_type` = 'ROUTE'
    AND `scope`.`object_id` = '900026'
    AND `scope`.`deleted` = b'0'
  ...
  IF NOT EXISTS (
    ...
    WHERE `object_type` = 'ROUTE'
      AND `object_id` = '900026'
      AND `status` = 'ENABLED'
      AND `deleted` = b'0'
  ) THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Missing enabled eDHR permission scope ROUTE:900026';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Missing enabled scheduler role for eDHR scope ROUTE:900026';
  END IF;
  ...
  SET `rule`.`decision` = 'ALLOW',
      ...
      `rule`.`status` = 'ENABLED',
      `rule`.`deleted` = b'0',
      `rule`.`updater` = 'scheduler-edhr-route-900026-route-edit',
...
CALL ensure_mes_scheduler_edhr_route_900026_route_edit_permission();
