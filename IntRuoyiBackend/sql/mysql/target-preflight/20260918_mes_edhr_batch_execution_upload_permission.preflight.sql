...
SELECT CASE
  WHEN EXISTS (SELECT 1 FROM `system_menu` WHERE `id` = 900033 AND `permission` = 'mes:pro-edhr-batch-execution:query' AND `type` = 2 AND `deleted` = b'0')
   ...
   AND (NOT EXISTS (SELECT 1 FROM `system_menu` WHERE `id` = 900043 AND `deleted` = b'0')
        OR EXISTS (SELECT 1 FROM `system_menu` WHERE `id` = 900043 AND `name` = 'eDHR批次执行上传' AND `permission` = 'mes:pro-edhr-batch-execution:upload' AND `type` = 3 AND `parent_id` = 900033 AND `status` = 0 AND `deleted` = b'0'))
   AND EXISTS (
     SELECT 1
