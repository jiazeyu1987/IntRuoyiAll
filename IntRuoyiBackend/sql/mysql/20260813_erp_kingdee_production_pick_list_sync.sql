...
-- 生产领料单只读快照；不得写入本地库存、出库、审批或生产用料清单表。

DROP PROCEDURE IF EXISTS preflight_erp_production_pick_list_menu;

DELIMITER $$
CREATE PROCEDURE preflight_erp_production_pick_list_menu()
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE id = 6032
      AND NOT (
        ...
        AND path = 'pick-list'
        AND component = 'erp/production/pick-list/index'
      )
  ) THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'ERP production pick-list menu id 6032 is occupied';
  END IF;
  ...
  IF EXISTS (
    ...
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'ERP production pick-list sibling path is occupied';
  END IF;
  ...
  IF EXISTS (
    ...
    WHERE id = 6033
      AND NOT (
        deleted = b'0'
        AND parent_id = 6032
        AND permission = 'erp:production-pick-list:query'
      )
  ) THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'ERP production pick-list menu id 6033 is occupied';
  END IF;
  ...
  IF EXISTS (
    ...
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'ERP production pick-list permission erp:production-pick-list:query is occupied';
  END IF;
END$$
DELIMITER ;

CALL preflight_erp_production_pick_list_menu();
DROP PROCEDURE IF EXISTS preflight_erp_production_pick_list_menu;

CREATE TABLE IF NOT EXISTS erp_kingdee_production_pick_list (
  id bigint NOT NULL AUTO_INCREMENT COMMENT '编号',
  source_form_id varchar(64) NOT NULL DEFAULT 'PRD_PickMtrl' COMMENT 'ERP来源表单标识',
  source_fid varchar(64) NOT NULL COMMENT 'ERP单据FID',
  source_bill_no varchar(128) NOT NULL COMMENT 'ERP生产领料单号',
  ...
  department_number varchar(64) DEFAULT NULL COMMENT '领料部门编码',
  department_name varchar(255) DEFAULT NULL COMMENT '领料部门名称',
  ...
  PRIMARY KEY (id),
  UNIQUE KEY uk_erp_kingdee_prod_pick_list_source (tenant_id, source_form_id, source_fid, deleted),
  KEY idx_erp_kingdee_prod_pick_list_bill_no (tenant_id, source_bill_no, deleted),
  KEY idx_erp_kingdee_prod_pick_list_modify_time (tenant_id, source_modify_time, deleted)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='ERP金蝶生产领料单只读快照';

CREATE TABLE IF NOT EXISTS erp_kingdee_production_pick_list_item (
  id bigint NOT NULL AUTO_INCREMENT COMMENT '编号',
  production_pick_list_id bigint NOT NULL COMMENT '生产领料单快照编号',
  source_form_id varchar(64) NOT NULL DEFAULT 'PRD_PickMtrl' COMMENT 'ERP来源表单标识',
  source_fid varchar(64) NOT NULL COMMENT 'ERP单据FID',
  source_entry_id varchar(64) NOT NULL COMMENT 'ERP分录ID',
  source_line_key varchar(255) NOT NULL COMMENT 'ERP领料分录唯一键',
  source_bill_no varchar(128) NOT NULL COMMENT 'ERP生产领料单号',
  ...
  tenant_id bigint NOT NULL DEFAULT 0,
  PRIMARY KEY (id),
  UNIQUE KEY uk_erp_kingdee_prod_pick_list_item_source (tenant_id, source_line_key, deleted),
  KEY idx_erp_kingdee_prod_pick_list_item_parent (tenant_id, production_pick_list_id, deleted),
  KEY idx_erp_kingdee_prod_pick_list_item_material (tenant_id, material_number, deleted),
  KEY idx_erp_kingdee_prod_pick_list_item_mo (tenant_id, production_order_no, deleted)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='ERP金蝶生产领料单分录只读快照';
...
SELECT '每 10 分钟同步 ERP 生产领料单列表', 2, 'kingdeeProductionPickListSyncJob', '', '0 6/10 * * * ?', 3, 60, 0,
       '1', NOW(), '1', NOW(), b'0'
WHERE NOT EXISTS (
  SELECT 1 FROM infra_job WHERE handler_name = 'kingdeeProductionPickListSyncJob'
);

UPDATE infra_job
SET name = '每 10 分钟同步 ERP 生产领料单列表',
    status = 2,
    handler_param = '',
    ...
    update_time = NOW(),
    deleted = b'0'
WHERE handler_name = 'kingdeeProductionPickListSyncJob';
...
SELECT 6032, '生产领料单列表', '', 2, 20, @erp_production_parent_menu_id,
       'pick-list', 'ep:list', 'erp/production/pick-list/index', 'ErpProductionPickList',
       0, b'1', b'1', b'1', '1', NOW(), '1', NOW(), b'0'
WHERE @erp_production_parent_menu_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM system_menu
    WHERE id = 6032 OR (parent_id = @erp_production_parent_menu_id AND path = 'pick-list')
  );
...
SELECT 6033, '生产领料单列表查询', 'erp:production-pick-list:query', 3, 1, 6032,
       '', '', '', '', 0, b'1', b'1', b'1', '1', NOW(), '1', NOW(), b'0'
WHERE EXISTS (SELECT 1 FROM system_menu WHERE id = 6032 AND deleted = b'0')
  AND NOT EXISTS (
    SELECT 1 FROM system_menu
    WHERE id = 6033 OR permission = 'erp:production-pick-list:query'
  );
