...
DROP PROCEDURE IF EXISTS `drop_dcc_reg_cert_access_request_project_check_20260904c`$$
CREATE PROCEDURE `drop_dcc_reg_cert_access_request_project_check_20260904c`()
BEGIN
  IF NOT EXISTS (
    SELECT 1
    ...
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'DCC access request table missing for download without project code migration';
  END IF;
...
CALL `drop_dcc_reg_cert_access_request_project_check_20260904c`()$$
DROP PROCEDURE IF EXISTS `drop_dcc_reg_cert_access_request_project_check_20260904c`$$
