...
-- Purpose: Group registration certificate related visible menus under the registration certificate management parent without changing page URLs or permissions.
...
BEGIN
  ...
  IF EXISTS (
      SELECT 1
        FROM `system_menu`
       WHERE `id` IN (990230, 990231, 990232, 990249, 605071320, 605071321)
         AND `deleted` = b'1'
  ) THEN
    SIGNAL SQLSTATE '45000'
  ...
  IF EXISTS (
      SELECT 1
        FROM `system_menu`
       WHERE `deleted` = b'0'
         AND `id` NOT IN (990230, 990231, 990232, 990249, 605071320, 605071321)
         AND `path` IN (
  ...
  IF NOT EXISTS (
      SELECT 1
       ...
       WHERE `id` = 605071320
         ...
         AND (
           (`parent_id` = 990200 AND `path` = 'company-scope')
           OR (`parent_id` = 990253 AND `path` = '/mdm/company-scope')
         )
  ) THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Enterprise company scope page menu contract mismatch';
  END IF;
  ...
  IF NOT EXISTS (
      SELECT 1
       ...
       WHERE `id` = 605071321
         ...
         AND (
           (`parent_id` = 990200 AND `path` = 'historical-import')
           OR (`parent_id` = 990253 AND `path` = '/mdm/historical-import')
         )
  ) THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'Registration certificate historical import page menu contract mismatch';
  END IF;
  ...
  INSERT INTO `tmp_registration_certificate_management_page_menu` (`menu_id`) VALUES
    (990230), (990249), (605071320), (605071321);
  ...
  CREATE TEMPORARY TABLE `tmp_registration_certificate_management_package_menu_json` AS
      ...
      SELECT 1
       ...
       WHERE `deleted` = b'0'
         AND (
           ...
           OR (`id` = 605071320 AND (`parent_id` <> 990253 OR `path` <> '/mdm/company-scope' OR `sort` <> 30))
           OR (`id` = 605071321 AND (`parent_id` <> 990253 OR `path` <> '/mdm/historical-import' OR `sort` <> 40))
