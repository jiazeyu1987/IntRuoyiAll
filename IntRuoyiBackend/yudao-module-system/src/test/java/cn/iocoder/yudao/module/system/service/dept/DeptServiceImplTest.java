...
public class DeptServiceImplTest extends BaseDbUnitTest {
    ...
    @Test
    public void testDeleteDept_cascadeDeleteChildren() {
        // mock 数据
        ...
        DeptDO grandChildDeptDO = randomPojo(DeptDO.class, o -> {
            o.setParentId(childrenDeptDO.getId());
            o.setStatus(randomCommonStatus());
        });
        deptMapper.insert(grandChildDeptDO);

        // 调用
        deptService.deleteDept(parentDept.getId());

        // 断言整棵公司树已删除
        assertNull(deptMapper.selectById(parentDept.getId()));
        assertNull(deptMapper.selectById(childrenDeptDO.getId()));
        assertNull(deptMapper.selectById(grandChildDeptDO.getId()));
    }

    @Test
    public void testDeleteDept_whenTreeHasUsers() {
        // mock 数据
        DeptDO parentDept = randomPojo(DeptDO.class);
        ...
        deptMapper.insert(childrenDeptDO);
        AdminUserDO userDO = randomPojo(AdminUserDO.class, o -> {
        ...
        // 调用，并断言异常
        assertServiceException(() -> deptService.deleteDept(parentDept.getId()), DEPT_EXITS_USERS);

        // 断言部门树仍保留
        assertNotNull(deptMapper.selectById(parentDept.getId()));
        assertNotNull(deptMapper.selectById(childrenDeptDO.getId()));
    }
    ...
    @Test
    public void testGetSimpleDeptList() throws Exception {
        DeptDO enabledDept = randomPojo(DeptDO.class, o -> {
            o.setName("启用部门");
            o.setStatus(CommonStatusEnum.ENABLE.getStatus());
        });
        deptMapper.insert(enabledDept);
        deptMapper.insert(randomPojo(DeptDO.class, o -> {
            o.setName("停用部门");
            o.setStatus(CommonStatusEnum.DISABLE.getStatus());
        }));

        List<DeptDO> result = deptService.getSimpleDeptList();

        assertEquals(1, result.size());
        assertPojoEquals(enabledDept, result.get(0));

        Method method = DeptServiceImpl.class.getMethod("getSimpleDeptList");
        DataPermission dataPermission = method.getAnnotation(DataPermission.class);
        assertNotNull(dataPermission);
        assertFalse(dataPermission.enable());
    }
    ...
    @Test
    public void testGetChildDeptList_cycleStopsAndDeduplicates() {
        DeptDO dept1 = randomPojo(DeptDO.class, o -> o.setName("cycle-1").setParentId(DeptDO.PARENT_ID_ROOT));
        deptMapper.insert(dept1);
        DeptDO dept2 = randomPojo(DeptDO.class, o -> o.setName("cycle-2").setParentId(dept1.getId()));
        deptMapper.insert(dept2);
        dept1.setParentId(dept2.getId());
        deptMapper.updateById(dept1);

        List<DeptDO> result = assertTimeoutPreemptively(Duration.ofSeconds(1),
                () -> deptService.getChildDeptList(dept1.getId()));

        assertEquals(List.of(dept2.getId(), dept1.getId()), result.stream().map(DeptDO::getId).toList());
    }
