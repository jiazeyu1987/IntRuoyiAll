...
public class PermissionServiceTest extends BaseDbUnitTest {
    ...
    @Test
    public void testHasAnyPermissions_dynamicEntitlementWithoutStaticRole() {
        try (MockedStatic<SpringUtil> springUtilMockedStatic = mockStatic(SpringUtil.class)) {
            springUtilMockedStatic.when(() -> SpringUtil.getBean(eq(PermissionServiceImpl.class)))
                    .thenReturn(permissionService);

            Long userId = 200L;
            String allowedPermission = "mes:pro-edhr-batch-execution:update";
            String forbiddenPermission = "mes:pro-edhr-batch-execution:create";
            when(roleService.getRoleListFromCache(eq(Set.of()))).thenReturn(new java.util.ArrayList<>());
            when(systemEntitlementService.hasAnyPermission(eq(userId), eq(allowedPermission))).thenReturn(true);
            when(systemEntitlementService.hasAnyPermission(eq(userId), eq(forbiddenPermission))).thenReturn(false);

            assertTrue(permissionService.hasAnyPermissions(userId, allowedPermission));
            assertFalse(permissionService.hasAnyPermissions(userId, forbiddenPermission));
        }
    }

    @Test
    public void testHasAnyPermissionsInRoles_requiresExplicitRoleMenuOnly() {
        try (MockedStatic<SpringUtil> springUtilMockedStatic = mockStatic(SpringUtil.class)) {
            springUtilMockedStatic.when(() -> SpringUtil.getBean(eq(PermissionServiceImpl.class)))
                    .thenReturn(permissionService);

            Long roleId = 100L;
            String permission = "mes:pro-batch-record-execution:golden-finger";
            RoleDO role = randomPojo(RoleDO.class, o -> o.setId(roleId)
                    .setStatus(CommonStatusEnum.ENABLE.getStatus()));
            when(roleService.getRoleListFromCache(eq(Set.of(roleId)))).thenReturn(toList(role));
            Long menuId = 3000L;
            when(menuService.getMenuIdListByPermissionFromCache(eq(permission))).thenReturn(singletonList(menuId));
            roleMenuMapper.insert(randomPojo(RoleMenuDO.class).setRoleId(roleId).setMenuId(menuId));

            assertTrue(permissionService.hasAnyPermissionsInRoles(Set.of(roleId), permission));
            verifyNoInteractions(systemEntitlementService);
            verify(roleService, never()).hasAnySuperAdmin(any());
        }
    }
    ...
    @Test
    public void testHasAnyRolesOrSuperAdmin() {
        try (MockedStatic<SpringUtil> springUtilMockedStatic = mockStatic(SpringUtil.class)) {
            springUtilMockedStatic.when(() -> SpringUtil.getBean(eq(PermissionServiceImpl.class)))
                    .thenReturn(permissionService);
            ...
            Long userId = 1L;
            userRoleMapper.insert(randomPojo(UserRoleDO.class).setUserId(userId).setRoleId(100L));
            RoleDO role = randomPojo(RoleDO.class, o -> o.setId(100L).setCode("super_admin")
                    .setStatus(CommonStatusEnum.ENABLE.getStatus()));
            when(roleService.getRoleListFromCache(eq(singleton(100L)))).thenReturn(toList(role));
            when(roleService.hasAnySuperAdmin(eq(asSet(100L)))).thenReturn(true);

            assertTrue(permissionService.hasAnyRolesOrSuperAdmin(userId, "approval_admin"));
        }
    }
