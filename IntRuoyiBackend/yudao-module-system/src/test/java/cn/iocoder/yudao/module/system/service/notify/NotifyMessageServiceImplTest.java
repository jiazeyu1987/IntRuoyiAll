...
public class NotifyMessageServiceImplTest extends BaseDbUnitTest {
    ...
    @Test
    public void testGetMyNotifyMessagePage_sortUnreadFirstThenCreateTimeDesc() {
        // mock 数据
        NotifyMessageDO unreadOld = randomPojo(NotifyMessageDO.class, o -> {
            o.setUserId(1L);
            o.setUserType(UserTypeEnum.ADMIN.getValue());
        ...
        NotifyMessageDO readNew = cloneIgnoreId(unreadOld, o -> {
            o.setReadStatus(true);
            o.setCreateTime(buildTime(2022, 1, 5));
        });
        notifyMessageMapper.insert(readNew);
        NotifyMessageDO unreadNew = cloneIgnoreId(unreadOld, o -> o.setCreateTime(buildTime(2022, 1, 4)));
        notifyMessageMapper.insert(unreadNew);
        NotifyMessageDO readOld = cloneIgnoreId(unreadOld, o -> {
            o.setReadStatus(true);
            o.setCreateTime(buildTime(2022, 1, 3));
        });
        notifyMessageMapper.insert(readOld);
        // 准备参数
        Long userId = 1L;
        Integer userType = UserTypeEnum.ADMIN.getValue();
        NotifyMessageMyPageReqVO reqVO = new NotifyMessageMyPageReqVO();
        ...
        assertEquals(Arrays.asList(unreadNew.getId(), unreadOld.getId(), readNew.getId(), readOld.getId()),
                pageResult.getList().stream().map(NotifyMessageDO::getId).toList());
    }
