...
public class RoleServiceImplTest extends BaseDbUnitTest {
    ...
    public void testCreateRole() {
        ...
        verify(roleCategoryService).validateRoleCategoryEnabled(reqVO.getCategoryId());
    }
    ...
    public void testGetRolePage() {
        ...
        reqVO.setName("土豆");
        reqVO.setCode("tu");
        reqVO.setCategoryId(dbRole.getCategoryId());
    ...
    @Test
    public void testGetAssignedUserCountMap() {
        // mock 数据
        RoleDO assignedRole = randomPojo(RoleDO.class);
        roleMapper.insert(assignedRole);
        RoleDO emptyRole = randomPojo(RoleDO.class);
        roleMapper.insert(emptyRole);
        RoleDO otherRole = randomPojo(RoleDO.class);
        roleMapper.insert(otherRole);
        userRoleMapper.insert(randomPojo(UserRoleDO.class).setRoleId(assignedRole.getId()).setUserId(101L));
        userRoleMapper.insert(randomPojo(UserRoleDO.class).setRoleId(assignedRole.getId()).setUserId(102L));
        userRoleMapper.insert(randomPojo(UserRoleDO.class).setRoleId(otherRole.getId()).setUserId(103L));

        // 调用
        Map<Long, Long> countMap = roleService.getAssignedUserCountMap(List.of(assignedRole.getId(), emptyRole.getId()));

        // 断言
        assertEquals(2L, countMap.get(assignedRole.getId()));
        assertEquals(0L, countMap.get(emptyRole.getId()));
        assertFalse(countMap.containsKey(otherRole.getId()));
    }
