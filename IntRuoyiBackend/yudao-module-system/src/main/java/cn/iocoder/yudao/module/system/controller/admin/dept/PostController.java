...
public class PostController {
    ...
    @Operation(summary = "导出岗位配置包")
    @PreAuthorize("@ss.hasPermission('system:post:export')")
    @ApiAccessLog(operateType = EXPORT)
    public void exportConfigPackage(HttpServletResponse response) throws IOException {
        byte[] data = postConfigPackageService.exportPackage();
        response.addHeader("Content-Disposition",
                "attachment;filename=" + HttpUtils.encodeUtf8("组织角色配置包.json"));
        response.setContentType("application/json;charset=UTF-8");
        response.getOutputStream().write(data);
    }

    @PostMapping("/config-package/import")
    @Operation(summary = "导入岗位配置包")
    @PreAuthorize("@ss.hasPermission('system:post:create') and @ss.hasPermission('system:post:update')")
    public CommonResult<Boolean> importConfigPackage(@RequestParam("file") org.springframework.web.multipart.MultipartFile file)
            throws IOException {
        postConfigPackageService.importPackage(file.getBytes());
        return success(true);
    }
