...
public class BpmProcessInstanceServiceImpl implements BpmProcessInstanceService {
    ...
    public void cancelProcessInstanceByStartUser(Long userId, @Valid BpmProcessInstanceCancelReqVO cancelReqVO) {
        ...
        updateProcessInstanceCancel(cancelReqVO.getId(),
                BpmReasonEnum.CANCEL_PROCESS_INSTANCE_BY_START_USER.format(cancelReqVO.getReason()), userId);
    }
    ...
    private void updateProcessInstanceCancel(String id, String reason, Long actorUserId) {
        Assert.notNull(actorUserId, "取消流程操作者不能为空");
        // 1. 更新流程实例 status
        ...
        runtimeService.setVariable(id, BpmnVariableConstants.PROCESS_INSTANCE_VARIABLE_LAST_APPROVER_USER_ID,
                actorUserId);
    ...
    public void processProcessInstanceCompleted(ProcessInstance instance) {
        Map<String, Object> processVariables = loadProcessInstanceCompletedVariables(instance);
        // 1.1 获取当前状态
        Integer status = (Integer) processVariables
        ...
        processInstanceEventPublisher.sendProcessInstanceResultEvent(
                BpmProcessInstanceConvert.INSTANCE.buildProcessInstanceStatusEvent(
                        this, instance, status, reason, processVariables));
    ...
    private Map<String, Object> loadProcessInstanceCompletedVariables(ProcessInstance instance) {
        Map<String, Object> processVariables = new HashMap<>();
        if (CollUtil.isNotEmpty(instance.getProcessVariables())) {
            processVariables.putAll(instance.getProcessVariables());
        }
        Map<String, Object> runtimeVariables = runtimeService.getVariables(instance.getId());
        if (CollUtil.isNotEmpty(runtimeVariables)) {
            processVariables.putAll(runtimeVariables);
        }
