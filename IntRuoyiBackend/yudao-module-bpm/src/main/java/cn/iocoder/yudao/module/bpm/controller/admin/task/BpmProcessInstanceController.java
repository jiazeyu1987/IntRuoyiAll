...
public class BpmProcessInstanceController {
    ...
    public CommonResult<BpmProcessInstanceRespVO> getProcessInstance(@RequestParam("id") String id) {
        ...
        assertCanReadProcessInstance(getLoginUserId(), processInstance);
    ...
    public CommonResult<BpmProcessInstanceBpmnModelViewRespVO> getProcessInstanceBpmnModelView(
        ...
        assertCanReadProcessInstance(getLoginUserId(), id);
    ...
    public CommonResult<BpmProcessPrintDataRespVO> getProcessInstancePrintData(
        ...
        assertCanReadProcessInstance(getLoginUserId(), historicProcessInstance);
    ...
    private void assertCanReadProcessInstance(Long loginUserId, String processInstanceId) {
        HistoricProcessInstance historicProcessInstance = processInstanceService.getHistoricProcessInstance(processInstanceId);
        if (historicProcessInstance == null) {
            throw exception(PROCESS_INSTANCE_NOT_EXISTS);
        }
        assertCanReadProcessInstance(loginUserId, historicProcessInstance);
    }

    private void assertCanReadProcessInstance(Long loginUserId, HistoricProcessInstance processInstance) {
        if (permissionApi.hasAnyRolesOrSuperAdmin(loginUserId, RoleCodeEnum.BPM_ADMIN.getCode())) {
            return;
        }
        if (StrUtil.equals(processInstance.getStartUserId(), String.valueOf(loginUserId))) {
            return;
        }
        if (hasTaskParticipant(loginUserId, processInstance.getId())) {
            return;
        }
        throw exception(PROCESS_INSTANCE_ACCESS_DENIED);
    }

    private boolean hasTaskParticipant(Long loginUserId, String processInstanceId) {
        if (loginUserId == null || StrUtil.isBlank(processInstanceId)) {
            return false;
        }
        ...
        if (CollUtil.isNotEmpty(runningTasks) && runningTasks.stream()
                .anyMatch(task -> isTaskParticipant(loginUserIdStr, task))) {
            return true;
        }
        List<HistoricTaskInstance> historicTasks = taskService.getTaskListByProcessInstanceId(processInstanceId, true);
        return CollUtil.isNotEmpty(historicTasks) && historicTasks.stream()
                .anyMatch(task -> isTaskParticipant(loginUserIdStr, task));
    }

    private boolean isTaskParticipant(String loginUserId, TaskInfo task) {
        return StrUtil.equals(loginUserId, task.getAssignee())
                || StrUtil.equals(loginUserId, task.getOwner());
    }
