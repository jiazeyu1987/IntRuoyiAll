...
public interface BpmProcessInstanceConvert {
    ...
    @Mapping(source = "from.modelType", target = "to.type")
    @Mapping(target = "version", ignore = true)
    @Mapping(target = "name", ignore = true)
    @Mapping(target = "key", ignore = true)
    @Mapping(target = "categoryName", ignore = true)
    @Mapping(target = "formName", ignore = true)
    @Mapping(target = "suspensionState", ignore = true)
    ...
    default BpmProcessInstanceStatusEvent buildProcessInstanceStatusEvent(Object source, ProcessInstance instance,
        ...
        return buildProcessInstanceStatusEvent(source, instance, status, reason, instance.getProcessVariables());
    }

    default BpmProcessInstanceStatusEvent buildProcessInstanceStatusEvent(Object source, ProcessInstance instance,
                                                                          Integer status, String reason,
                                                                          Map<String, Object> processVariables) {
        Long actorUserId = parseActorUserId(processVariables);
    ...
    default Long parseActorUserId(ProcessInstance instance) {
        return parseActorUserId(instance.getProcessVariables());
    }

    default Long parseActorUserId(Map<String, Object> processVariables) {
        Object value = processVariables == null ? null : processVariables
                .get(BpmnVariableConstants.PROCESS_INSTANCE_VARIABLE_LAST_APPROVER_USER_ID);
        return value == null ? null : NumberUtils.parseLong(String.valueOf(value));
    }
    ...
    default BpmProcessPrintDataRespVO buildProcessInstancePrintData(HistoricProcessInstance historicProcessInstance,
        ...
        tasks.forEach(item -> {
            ...
            approveTask.setDescription(StrUtil.format("{} / {} / {} / {} / {}",
                    resolvePrintTaskAssigneeName(userMap, item.getAssignee()),
                    item.getName(),
    ...
    static String resolvePrintTaskAssigneeName(Map<Long, AdminUserRespDTO> userMap, String assignee) {
        Long assigneeUserId = NumberUtils.parseLong(assignee);
        if (assigneeUserId == null) {
            return "用户不存在";
        }
        AdminUserRespDTO user = userMap == null ? null : userMap.get(assigneeUserId);
        if (user == null) {
