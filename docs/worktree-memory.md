...
- 经验规则：已监听但 health 短暂失败时，先结合日志最新写入时间和 PID 归属判断是否仍在启动窗口，复测 health 后再决定是否重启；命令行可能包含 datasource password、token 等敏感参数，任务日志只记录脱敏摘要和归属证据，不复制原始秘密。
