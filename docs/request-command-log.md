...
2026-08-22T23:26:08+08:00	20260821-test-only-head-release	BLOCKED	授权继续执行测试服发布收束：冻结提交 maintenance=`66c4ea05...`、app=`c4d2c637...`，r2 构建包 manifest/version/changeSet 校验通过；正确冻结发布脚本在 route snapshot enforcement 前置门禁失败，未切换容器，未执行 mark-tested/promote-prod/promote-backup。已记录 P068-P075；下一步只允许通过官方 test-only DB 入口恢复该 releaseTag 的 stale RUNNING lock/migration，并核验 RUNNING=0，之后停止本轮发布，不复用 r2 成功发布结论。凭据、连接串和敏感命令输出均不写入日志。
2026-08-23T03:30:57+08:00	20260821-test-only-head-release	RECOVERY	精确 stale-migration-recovery 已通过官方 test-only DB 入口执行并核验：`infra_release_operation_lock` 与 `infra_release_migration` 均为 `FAILED`，test 环境 `RUNNING` lock/migration 均为 `0`；这只清除了失败发布残留，不代表测试发布成功，亦未执行 mark-tested/promote-prod/promote-backup。
2026-08-23T03:59:17+08:00	20260821-test-only-head-release	SMOKE_FIX	补齐测试服重试参数后，冻结 r2 发布进入远端 smoke 阶段并失败于 `MES_SMOKE_NODE_IMAGE is required`；新建干净 worktree `D:\IntRuoyiWorktree\r260823a\a` / branch `codex/release-smoke-node-image-export`，新增 RED 契约并修复发布脚本三个 smoke wrapper 调用显式导出 `MES_SMOKE_NODE_IMAGE`。目标测试 `test_scheduler_smoke_release_contract.py` 已由 FAIL 转 PASS；尚未提交、重建新 releaseTag 或发布成功。
2026-09-02T14:12:56+08:00	20260902-invoice-voucher-print-assistant-permission-fix	REQUEST	用户反馈测试服重新登录后仍显示“无权访问发票凭证打印助手 / 没有发票凭证打印权限，请从 ERP 系统入口重新进入”，截图入口为 http://172.30.30.58:8081/erp/finance/invoice-voucher-print。
2026-09-02T14:12:56+08:00	20260902-invoice-voucher-print-assistant-permission-fix	COMMAND_INTENT	在既有授权范围内继续只读诊断测试服 ERP 签票、助手 callback 与助手运行日志；如需写入，仅允许更新并重启“发票凭证打印助手”服务，不重新发布 ERP 后端或前端。
2026-09-02T14:12:56+08:00	20260902-invoice-voucher-print-assistant-permission-fix	AUTHORIZATION	用户要求“用本机的对应设置”；解释为授权把本机 `yudao.erp.kingdee.connection.production` 中的金蝶 SimPas 三字段同步到测试服同名配置，仍不输出明文凭据、不重新发布 ERP、不重启 ERP 后端。
2026-09-02T14:12:56+08:00	20260902-invoice-voucher-print-assistant-permission-fix	BLOCKED	按本机对应设置同步测试服 SimPas 三字段时，测试服 `infra_config.value` 字段 `varchar(500)` 无法容纳补齐后的完整 production connection JSON；门禁 `TEST_PRODUCTION_CONFIG_TOO_LONG` 在 UPDATE 前中止，复核测试服三字段仍为空。需要明确授权测试服配置表字段扩容后才能继续写入。
2026-09-02T15:21:00+08:00	20260902-invoice-voucher-print-assistant-permission-fix	AUTHORIZATION	用户选择方案 1：只在测试服把 `infra_config.value` 扩到更合理长度后同步本机三字段。
2026-09-02T15:21:00+08:00	20260902-invoice-voucher-print-assistant-permission-fix	RESULT	PASS: 测试服目标配置行已备份到 `/root/intruoyi-backups/invoice-voucher-print-config/infra_config-production-20260902-152100.sql`；`infra_config.value` 已从 `varchar(500)` 扩为 `varchar(2000)`；本机 production connection 的 `appId`/`signedData`/`timestamp` 已同步到测试服同名配置且未输出明文；复核 active=`PRODUCTION`、JSON_VALID=1、全部必需字段 SET；ERP backend/frontend 未重新发布，backend health HTTP 200。
2026-09-02T15:35:00+08:00	20260902-invoice-voucher-print-assistant-permission-fix	REQUEST	用户询问下次发布是否复发后要求“继续”，目标为把测试服 `infra_config.value` 扩容热修固化进 IntRuoyi 仓库迁移和基线建表。
2026-09-02T15:35:00+08:00	20260902-invoice-voucher-print-assistant-permission-fix	RESULT	PASS: 新增 `sql/mysql/20260902_infra_config_value_expand_for_kingdee_connection.sql`，更新 `sql/mysql/ruoyi-vue-pro.sql` 中 `infra_config.value` 为 `varchar(2000)`，新增合同测试 `script/tests/test_infra_config_value_expansion_sql.py`；目标测试 3 passed，发票助手权限 SQL 回归 10 passed，单文件迁移门禁 PASS，完整迁移门禁 562 migrations PASS；未发布、未提交、未推送。
2026-09-02T15:50:00+08:00	20260902-invoice-voucher-print-assistant-permission-fix	REQUEST	用户反馈点击“一键查询并生成打印包”后，助手页面暴露 `Query-ErpInvoiceVoucherMappings-List.py` traceback，失败为 `ERP HTML5 session did not switch to organization 881`。
2026-09-02T15:50:00+08:00	20260902-invoice-voucher-print-assistant-permission-fix	RESULT	PASS: 本机助手包已修复 881 组织切换顺序，SSO 后先对当前 HTML5 浏览器会话执行 `SwitchOrg`，再 reload 和验证页面组织；`RuntimeError` 改为单行 stderr，不再暴露 traceback；新增组织切换顺序和无 traceback 回归，相关助手回归均 PASS。未获远程服务器操作或助手重启授权，因此尚未部署到测试服。
2026-09-02T20:23:08+08:00	20260902-invoice-voucher-print-assistant-permission-fix	RESULT	PASS: 用户授权本机修改；本机 IntRuoyi DB `yudao.erp.kingdee.connection.active 已从 TEST 切为 PRODUCTION；复核 production connection 的 appId/signedData/timestamp 均 SET；本机助手直接访问仍 HTTP 403，保持必须从 ERP 入口进入。未操作测试服务器、未发布、未重启服务。
2026-09-02T20:35:06+08:00	20260902-invoice-voucher-print-assistant-permission-fix	RESULT	PARTIAL: 用户反馈本机报 `ERP HTML5 organization switch failed`；本机脱敏探针确认 WebAPI ValidateUser 成功且 WebAPI SwitchOrg(881) 成功，问题限定在 HTML5 浏览器会话 SwitchOrg；已更新本机助手脚本提取 `ResponseStatus.Errors` 并在失败时输出具体金蝶业务原因，相关静态/语法回归 PASS；callback replay 回归因本机 18733 已被用户验证用 node 进程 PID 16408 占用而未重跑，未擅自停止。
2026-09-02T21:25:29+08:00	20260902-invoice-voucher-html5-session-loss	RESULT	PARTIAL/BLOCKED: 已只更新并重启测试服“发票凭证打印助手”服务，当前镜像为 erp-invoice-voucher-print-assistant:html5-sso-prompt-r260902h，直连 18733 返回 HTTP 403 符合入口授权门禁；助手定向回归和 Python 语法检查通过。只读探针确认 WebAPI ValidateUser 与 WebAPI SwitchOrg(881) 成功，但金蝶 HTML5 SimPas SSO 页面返回“用户【int】签名校验错误次数超限，请稍后再试，或者联系系统管理员解锁。”因此根因不是 IntRuoyi 权限或组织切换代码，而是 SimPas 签名/账号锁定；已改为在 SSO 阶段 fail fast 暴露金蝶原文。未发布/重启 IntRuoyi 后端前端，未修改测试服数据库。
...
2026-09-15T19:21:00+08:00	one-button-app-release-20260911	RESULT	R56 build-release 在迁移测试自动发现阶段 fail fast：缺少 `20260812_mes_route_version_snapshot_identity_enforce` 对应 `script/tests` 合同测试，返回 `MIGRATION_TEST_MISSING`；未进入后端打包/Docker/NAS/测试服写入。应用仓补充同名静态合同测试，覆盖 release metadata、approvedHook、not-null 前置阻断和只收紧现有列。
2026-09-15T19:26:00+08:00	one-button-app-release-20260911	VERIFICATION	`20260812_mes_route_version_snapshot_identity_enforce` 同名迁移测试补齐后，定向 7 PASS；从后端根执行 target-preflight/release-plan 相邻回归 25 PASS。应用 worktree 待提交后使用新 releaseTag 继续，R56 不复用。
2026-09-16T04:47:18+08:00	one-button-app-release-20260911	RESULT	R73 build-release 在应用仓自动发现测试阶段 fail-fast：旧 Dockerfile CMD 断言与当前 INTRUOYI_EXTRA_ARGS 运行合同不一致，且应用仓 publish-int-ruoyi.ps1 未同步通用 migration metadata（requiresTargetPreflight/applyOrder/sessionProfile/approvedHook/resultAssertion）与 rollback/evidence-only 排除，导致 required SQL 分类提前失败。已提交应用仓 `27f2ceccec7e` 同步通用合同；定向 5 PASS、R73 同组 135 PASS、相邻发布回归 58 PASS、PowerShell AST PASS。R73 未形成完整包、未上传 NAS、未写测试服，下一轮使用 `release-20260916-one-button-app-r74`。
2026-09-16T09:30:00+08:00	one-button-app-release-20260911	RESULT	R79 测试服 publish-test 在版本切换前失败：route-snapshot backfill 报告 `status=READY`、`blockerCount=0`，但 runner SSH exit=1，operation lock=FAILED，测试服 `.env`/实际镜像仍为旧版本。根因是 `MesProRouteVersionSnapshotMigrationRunner` 使用 `SpringApplication.exit(...)` 聚合无关 `ExitCodeGenerator` 覆盖命令 READY 码；应用仓已修复并通过清洁 Maven 定向 6 tests，R79 判废不复用。
2026-09-16T09:30:00+08:00	one-button-app-release-20260911	COMMAND_INTENT	准备 R80：绑定应用仓退出码修复提交与维护仓本段记录提交；目标仅测试服 `172.30.30.58`，范围 `app-release/without-data`，从维护 worktree 根执行 build-release -> publish-test；禁止正式服、审查服、mark-tested、promote-prod、promote-backup、MinIO 数据同步和全量数据库复制。
2026-09-16T16:40:00+08:00	one-button-app-release-20260911	REQUEST	用户提供发布按钮静态代码审查 FAIL 结论，并要求继续按“通用、长期可用的按钮发布功能”方向修复。
2026-09-16T16:40:00+08:00	one-button-app-release-20260911	COMMAND_INTENT	暂停 R80 publish-test，不操作服务器；在应用 worktree 以 BDD/TDD 修复按钮 workflow 通用机制：app-release 合同、维护仓脚本绑定、workflow context 授权、派发前 operation 绑定、稳定等待态 heartbeat、lease 生命周期和前端显式 workflow 选择。
2026-09-16T16:57:00+08:00	one-button-app-release-20260911	RESULT	PASS: 应用仓静态审查纠偏已完成本机验证；前端静态合同 PASS，后端 `RuntimeControlServiceImplTest,ReleaseWorkflowOrchestratorTest` 80 tests PASS，`corepack pnpm run ts:check` PASS，旧脚本路径/旧 scope/固定第一条 workflow 源码扫描无命中。R80 不复用；下一步需提交应用修复后以全新 releaseTag 重新 build-release -> publish-test。未执行正式服、审查服、mark-tested、promote-prod、promote-backup、MinIO 数据同步或全量数据库复制。
2026-09-16T23:10:00+08:00	20260916-release-button-remediation	REQUEST	用户要求继续处理发布按钮最新静态复查报告中的 R1-R5。
2026-09-16T23:10:00+08:00	20260916-release-button-remediation	COMMAND_INTENT	在应用 worktree 本机修复并验证 ReleaseWorkflow：来源 tuple 去重、超时恢复 lease、派发/授权后未知态隔离、正式发布前置拒绝、构建阶段 marker；不执行真实发布、服务器/数据库/E2E、提交、推送或 worktree 清理。
2026-09-16T23:10:00+08:00	20260916-release-button-remediation	RESULT	PASS: R3 补充 RED 复现后 GREEN；ReleaseWorkflow clean 定向回归 31/31 PASS，RuntimeControlServiceImpl 低层合同 2/2 PASS，bug-regression validator 与 task-owned diff check PASS。任务状态 ready_for_closeout，最终提交/推送/completed 需当轮明确授权。
2026-09-17T01:00:00+08:00	20260916-release-button-remediation	REQUEST	用户要求继续修复最新发布按钮静态复查报告，修复完成后通过发布按钮执行一次测试服发布，并处理发布后的问题。
...
2026-09-20T16:54:00+08:00	20260919-button-only-prod-release	RESULT	PASS: 应用后端补充正式晋级按钮参数单测，证明 `promote-prod` 只生成 `deploy-release -Environment prod -ReleaseTag <same>` 并携带测试 operation/evidence，不重新 build-release 或 publish-test；`RuntimeControlOperationActionPromotionTest,ReleaseWorkflowOrchestratorTest` 共 27 passed。未重新构建、未部署、未写服务器。
## 2026-09-20 17:40 CST — 注册证通知角色方案 2

- User request: 选择方案 2，仅使用现有角色 `910218` 作为注册证通知接收角色，不包含、不要求 `910231`。
- Action intent: 更新注册证提醒任务 SQL 与角色公司范围回填迁移；移除旧的 `910231` 通知角色要求，不引入 query-current 权限 fallback。
- Evidence: `python -X utf8 -m pytest .\script\tests\test_dcc_registration_certificate_notification_role_scope_backfill_sql.py -q` -> 3 passed；`mvn -pl yudao-module-dcc '-Dtest=DccRegistrationCertificateReminderSchemaTest' '-Dsurefire.failIfNoSpecifiedTests=false' test` -> 6 tests passed；迁移策略门禁 -> `status=passed`。
- Deployment status: 仅代码与测试修正，未重新构建、未部署测试服、未 mark-tested、未晋级正式服。
