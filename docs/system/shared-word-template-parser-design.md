...
- 展示边界：截图中的工序主表应直接给出输入物料、输出物料、设备和设备参数摘要；展开行用于承载全量结构化明细。只把设备参数放在展开行，用户会认为“没有显示设备参数信息”。
...
- 验证方式：前端静态合同同时锁定 parse-only API、`.json` 下载、单一 `el-tree`、输出物料 -> 设备 -> 设备参数 UI、额外字段节点，并负向禁止表格、描述面板、展开行和独立完整 JSON 面板。
