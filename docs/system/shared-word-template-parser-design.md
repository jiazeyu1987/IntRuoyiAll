...
- 常见卡点：只在主表展示投入/产出/设备数量会让用户看不到物料名称编号、设备名称编号、参数范围和默认值；只改 DOM 不补同名设备卡样式会造成多设备看起来仍不像一线生产。正确做法是保留 JSON 编辑核对入口，并让右侧预览承担一线生产样式展示。
- 展示边界：当前表单解析结果区采用左侧 JSON 编辑、右侧一线生产预览。用户明确要求隐藏的一线生产顶部产品卡、员工卡、主页按钮不得重新出现；多设备必须使用一线生产同款设备卡切换区和当前设备参数区，不能回退为每台设备纵向堆叠完整参数卡。
- UI 预览规则：设备参数列需要按解析出的 `ui.control/defaultValue/step/min/max/unit/options` 展示控件预览；`number` 渲染数字步进控件和目标范围，`select` 渲染下拉选项，未知控件必须显示未识别原码，不能降级成普通文本冒充已支持。
- 树状图边界：当用户要求“只需要一个树状图”时，结果区必须删除描述面板、表格、展开行和独立 JSON 面板；树节点承担全部展示，按产品/工序/输入物料/输出物料/设备/设备参数组织，额外未知字段也进入树节点，避免靠完整 JSON 面板兜底。
- 验证方式：前端静态合同需要随最终产品形态更新，当前应同时锁定 parse-only API、`.json` 下载、JSON 编辑应用、一线生产预览、设备卡类名/样式、当前设备参数 UI，并负向禁止旧树状图、旧表格、顶部产品/员工/主页卡和 Jimu schema 下载逻辑。
...
- 触发场景：同一“表单解析”页面同时支持生产批记录、QA 检验规程或其它业务 Word 解析，并要求左侧 JSON 编辑、右侧业务预览。
- 隔离规则：每类文档必须分别拥有结果页签、上传状态、JSON 文本、搜索位置、当前工序和预览草稿；切换页签只能切换视图，不能复用或覆盖另一类文档状态。
- 数据流规则：生产批记录必须走批记录总识别 parser 和按钮权限；QA 检验规程必须走 `MesQaInspectionRegulationWordParser` 与独立 parse-only 接口，并复用表单解析页面查询权限，避免未部署额外权限时按钮被隐藏。QA 解析不得调用生产批记录接口，也不得调用会创建草稿、发布版本或写 DCC 数据的正式 QA 导入接口。
- QA JSON 规则：保留规程编号、名称、版本、生效日期，并按源文档顺序聚合工序和检验项；检验项至少保留接受标准、检验方法、检验器具/设备、抽样方案、首检数量和巡检比例。右侧按一线 PQC 的工序/项目切换和详情控件展示，人工修正必须点击“应用”后才刷新预览。
- 验证方式：使用包含合并单元格和“首件 + AQL”抽样的真实 DOCX 验证解析；前端合同同时断言双页签独立状态、QA 不引用批记录 API 且按钮不依赖额外权限；后端合同断言 parse-only 服务不注入 Mapper、不加事务、不写数据库。
- Forbidden action：禁止用一个通用 JSON 状态承载不同业务文档，禁止用生产批记录 schema 包装 QA 数据，禁止为复用 UI 而让 QA 和生产批记录共享当前工序或编辑草稿；禁止把解析链路隔离扩大成未经部署的按钮权限依赖。
- Evidence: `doc/tasks/20260910-form-parser-qa-regulation-preview/execution-log.md`。
...
- 触发场景：从真实 `.doc` / `.docx` 批记录表头或设备列表读取 `□`、`☑`、`☒` 等勾选符号时。
- 经验规则：同一业务字段必须优先读取源 Word 表格原文，并同时覆盖 `.docx` 的标准勾选符号与 `.doc` 遗留字体映射后的文本形态，例如 `(`、`（`、`þ`、专用区 glyph。缺少勾选项或两个互斥项同时不可判定时必须 fail fast。
- 常见卡点：`.doc` 中设备编号前的旧勾选符号可能不是 `☑`，如果正则只接受标准复选框，会漏掉设备编码，进而导致设备组数量和参数组数量不一致。
- 验证方式：真实 `.doc` 与 `.docx` fixture 必须各有回归；测试至少断言所有工序均输出关键工序布尔值，并覆盖一个关键工序、一个非关键工序和一个含旧勾选符号的设备分组工序。
...
- 触发场景：QA 通用包装检验规程版本从受控 Word 导入，真实模板不是产品专属 QA 的 8 列结构，而是“序号 / 检验项目 / 接受标准 / 检验方法 / 检具 / 检验规则”等 6 列结构。
- 经验规则：真实受控文档中的同义表头只有在 fixture 回归锁定后才能纳入正式模板变体，例如 `生效日期` / `实施日期`、`检验器具及设备` / `检具`、`抽样方案` / `检验规则`。不得按文件名、产品名或自由文本猜测模板类型。
- 通用包装规程没有独立“工序”列时，业务工序名称应来自规程名称；空白续行必须继承上一条已解析检验项目的项目名、检具和抽样/检验规则，只覆盖本行填写的标准或方法。
- 抽样规则必须 fail fast：同一行超过一个 AQL 判定为结构错误；只有首件数量、没有 AQL 的行只能生成首检适用项，不得静默补出巡检比例或默认 AQL。
- 验证方式：用 A/B 版本真实 `.docx` fixture 做静态回归，覆盖 6 列模板、表头同义词、空白续行继承、首件-only 抽样和产品 QA 8 列模板兼容；E2E 再走前端导入、发布版本选择、产品绑定和一线 PQC 显示链路。
