...
> 本文保留早期全局目标。当前活跃订单主链按[策略交付规格](../../doc/tasks/20260924-edhr-gxp-audit-coverage/docs/system/policy-delivery.md)实施：后端根config为目标唯一源，经构建打包和受控批准激活；仓库根旧配置及旧测试引用待M1退役，不能当作当前运行策略已加载的证据。
