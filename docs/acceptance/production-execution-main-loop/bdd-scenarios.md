...
### Scenario: 活跃订单完成后统一回填正式资料

Given 活跃订单生产进度达到 100%
And 活跃订单检验进度达到 100%
And 当前路线工序存在正式逐工序批记录表单绑定
And 一线生产、生产工单、领料单、一线 PQC 和组长复核来源完整
And `PROCESS_POOL_REPORT` 字段映射完整且目标单元格可写
When 生产组长在活跃订单点击完成
Then 系统在同一业务节点回填批记录和过程检验单
And 存在生产损耗时回填损耗单
And 不存在生产损耗时不生成损耗单
And 回填完成后系统创建批次执行
...
Given 一条生产提交已经完成报工、记录本、复核、FIFO 分配、订单工序完成和活跃订单完成统一回填
And 同工序存在关联 PQC 结果
