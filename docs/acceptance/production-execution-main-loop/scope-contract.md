...
本文档把 P0 后续开发重点收敛为“生产执行主闭环”：以一次“工序池提交事件”为主事实源，串联生产报工、记录本、PQC、电子签名、班组长复核、生产工单 FIFO 分配、订单工序完成、活跃订单完成统一回填和正式批记录追溯。
...
- 当前代码证据：`MesProFrontlineFeedbackSubmitServiceImpl` 已把报工、记录本和工序池事件放在同一事务；`MesTeamLeaderReportConfirmationServiceImpl` 已做组长确认和分配；当前代码同时存在工序完成回填和活跃订单放行资料回填相关服务，后续目标口径必须收敛到活跃订单完成节点统一回填。
...
- 批记录追溯：活跃订单生产进度 100%、检验进度 100% 后点击完成，才能统一回填批记录、过程检验单和有损耗时的损耗单；不得在一线提交、组长复核或单个工序完成时提前写最终表单，不得使用 `formBindings`、默认 `MAIN`、工序开始配置或前端文案替代正式来源。
...
- 批记录、过程检验单和损耗单回填必须在活跃订单双进度 100% 且班组长点击完成后执行；确认分配、订单工序完成、PQC 组长复核只提供正式来源 ID。若采用异步任务或事件驱动，任务状态、来源 ID、幂等键和失败重试状态必须结构化持久化，trace 在统一回填完成前保持 `batchRecord.status=BLOCKED`。
...
- FIFO 确认写事务的固定顺序必须是：锁定或重新读取生产提交根事件；校验 PQC 结构化绑定和可分配数量；校验所有强制复核和复核签名；校验活跃订单和剩余数量；写入确认/分配/完成来源事实。任一前置失败时，不得先写确认终态再回滚式补 blocker；最终批记录、过程检验单和损耗单回填只允许由活跃订单完成节点触发。
- 生产组长复核是 FIFO 确认的强制前置；缺生产组长复核、复核签名或复核来源事件时，不得写入分配或订单工序完成，更不得进入活跃订单完成统一回填。
...
- P0 后端定向测试覆盖生产提交、PQC 入池、复核签名、FIFO 确认、活跃订单完成统一回填和统一 trace。
- P0 前端静态合同覆盖真实入口、请求字段、错误展示和 trace 页面。
- P0 真实 E2E 使用真实前端路径完成生产提交、PQC 提交、班组长复核、FIFO 确认、活跃订单完成统一回填、批次执行文件上传和闭环追溯。
