...
- `PASS`：Playwright 从真实前端页面完成生产提交、PQC 提交、班组长复核、FIFO 确认、工序完成、批记录回填和 trace 完整断言，并完成只读核验与清理/保留证据。
...
- 记录关键目标请求、方法、HTTP 状态和业务码：生产提交 `POST 2xx code=0`、PQC 提交 `POST 2xx code=0`、复核签名 `POST 2xx code=0`、FIFO 确认 `POST 2xx code=0`、trace 查询 `GET 2xx code=0`；批记录回填由后端 trace 和只读核验证明。
