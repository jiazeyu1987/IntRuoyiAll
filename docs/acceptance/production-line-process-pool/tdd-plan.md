...
| T07 | F2 payload 拆分 | `mvn -pl yudao-module-mes -am "-Dtest=MesProFrontlineFeedbackPayloadSplitterTest" test` | 设备参数、输出、损耗无法稳定拆分到报工和记录本，或仍要求一线手填上工序输入。 | 建立字段映射器和正式来源关联；上一工序输入数量不进入一线提交合同。 | 同 RED 命令 PASS。 | 设备参数不得硬塞进报工备注；不得用文本匹配追溯。 |
