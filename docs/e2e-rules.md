...
- Preflight check: 先在目标 worktree 和当前工作区分别运行同一静态合同；读取源码时对只检查模板片段的静态合同统一归一化 CRLF 为 LF；定位 Vue/SFC 弹框或组件块时优先按稳定 class、data 属性或组件名回找边界，不用缩进数量精确匹配；确认真实 E2E 脚本与当前页面真实用户路径一致。
