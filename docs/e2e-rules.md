...
- 真实 E2E 前必须盘点同一前后端端口是否已有其它 Playwright/runner 正在运行，尤其是会写入同一业务域或同一批测试数据的长流程 runner。发现共享运行态并发写入时，当前验证必须记录为 `BLOCKED`，不得继续重跑扩大数据污染，不得停止非任务自有进程，也不得把并发导致的业务上下文错误直接判定为产品缺陷；应等待并发任务结束，或切换到任务自有隔离运行态和测试数据后重跑。
- 只读验证必须说明使用的数据来源和只读范围。
- 需要逐步截图验收的 Playwright 运行必须保存 `trace.zip`，再从通过的 trace 导出每一步操作截图；只保留入口落点截图或最终截图时，不得宣称“每一步截图”已经完成。
- 截图验收依赖接口数组渲染时，必须同时记录正式页面网络响应里目标数组数量和页面 DOM 数量；若接口本身返回空数组，只能记录为目标数据缺失或不适用，不能把空 DOM 截图写成布局通过或布局失败。
- 页面可达、按钮可见或命中业务文字只能证明入口存在，不能作为业务步骤 PASS；写入、审批、状态推进、文件生成等步骤必须执行真实动作并从同一路径核验结果。页面请求即使 HTTP 200，也必须检查统一响应业务码；`pageerror`、点击失败、业务错误提示和响应解析失败均须进入失败证据，禁止空 `catch` 后继续记 PASS。列表 DOM 为 0 时还要区分接口空数组、接口失败、前端过滤和真实无数据。
- 提交型后台任务的真实 E2E 必须区分“页面提交成功”和“业务执行完成”：前端 POST 返回成功只表示任务已受理，必须通过同一真实页面刷新或轮询确认成功/失败终态；若仍显示运行中，必须记录任务记录、访问日志和线程栈证据，禁止重复提交。
- ERP 同步类页面如果同步总览路由在当前菜单中不可达，先回到业务列表页查找该同步类型的正式按钮；提交后必须进入系统定时任务执行日志页，按返回的 `jobId` 通过页面自然刷新确认成功/失败终态。不得把 `/erp/sync`、旧路由或 API-only 日志查询当成默认入口。
- Element Plus `el-input-number` 在真实页面可编辑时，内部 `input` 仍可能带 `aria-disabled="true"`，导致 Playwright `fill()` 或键盘输入被误判为不可编辑。写入型 E2E 应先检查原生 `disabled`、保存按钮状态和页面权限提示；若加减控件可见可用，优先点击可见的增加/减少按钮完成真实页面改值，禁止改 DOM value、直接调用 API 或把 Playwright actionability 误判成产品权限失败。

### DCC 版本生命周期真实验收门禁

- DCC 的检出、检入、撤销检出和升大版本必须分别核对真实页面入口、当前运行后端 OpenAPI、运行 Jar 来源及已应用 schema；源码新增入口但 48081 仍是旧包时，真实页面验收必须 BLOCKED。
- 页面入口必须完成真实动作：选择源文件上传并得到检入 ticket、检入生成 A/2、从选定 WORKING 小版本点击升大版本生成 B/1；API/DB 只能做最终只读核验，不能补齐缺失入口。
- 页面有版本历史时，必须同时展示直接前驱、正式基线、操作者/时间、前后哈希和审批/发布结果；只返回 `versionHistory` 或只在数据库保存字段不算页面验收通过。
...
- Production/PQC interleaving：eDHR主链runner必须交错执行生产提交、生产复核、PQC提交和PQC复核；静态合同不得固化“全部生产完成后再补做全部PQC”的错误顺序。
- Production/PQC full coverage：eDHR主链runner不得写死生产提交数、PQC提交数或只覆盖前两个工序。E2E01生成本轮订单后，必须冻结本轮全部生产工序、工序目标数量、输出物料页签和PQC待检任务到Manifest；E2E02按全部生产工序提交并复核，E2E03按全部PQC任务提交并复核。E2E04前必须重新通过真实页面确认生产进度100%、检验进度100%；未达成时直接失败并输出缺失工序或任务，禁止在完工阶段补救或假定通过。
...
- eDHR 活跃订单正式事实包含资料上传时，E2E不得只证明P1/P2/P3、PQC放行和上市放行状态；必须通过真实详情页上传任务自有文件，并在上市放行后的历史追溯详情中复核文件名、上传人、上传时间及资料上传操作事实。历史列表附件计数为空不能直接代表详情资料缺失，验收口径应以统一活跃订单详情中的可见文件和操作事实为准。
...
- 触发场景：通过后台定时任务、每日提醒任务或通知配置验证站内信、待办、短信等消息投递。
- 经验规则：被验收动作必须由真实页面完成，包括业务数据准备、通知接收人配置、正式任务入口触发和收件人登录查看；API/DB 只允许做只读辅助核验或最终佐证，不能代替点击任务、配置收件人或阅读通知。
- Preflight check: 后端菜单 `component` 不是前端可直接访问 URL，定时任务页应按真实路由/菜单进入；例如组件 `infra/job/index` 对应页面路由可能是 `/infra/job`，直接访问组件路径出现 404 时应先核对菜单生成后的真实 path，不得把 404 误判为功能缺失。Element Plus 表格“更多/执行一次”这类行内操作可能存在图标、下拉和固定列 DOM 拆分，脚本必须锚定目标业务行后点击同一行可见操作，不能用全页第一个同名按钮或 API trigger 替代。
- Preflight check: 每日任务类 E2E 的业务日期不得复用同租户已有成功或失败执行记录；应选择任务自有、有效且不晚于当前日期的业务日期，并在日志中记录日期来源。
- 验证方式：先记录业务日期、命中节点、业务对象编号、接收人账号标签和任务入口；触发任务后用被配置接收人登录站内信页面，逐条断言可见标题、内容、业务对象、节点、日期和接收人。站内信列表的可见标题可能来自模板昵称，模板名称、模板编码和页面标题应分别核对，不能把其中一个字段的旧预期硬套到另一个字段。
- 禁止做法：禁止直接写库造通知、直接调用 trigger API 冒充页面触发、用管理员消息列表替代收件人本人页面、或只看通知表存在就宣称用户可见。
...
- Trigger: 详情页或工作台按当前卡片、工序、提交记录、检验记录生成只读表单、记录表或打印预览。
- Preflight check: E2E 必须先通过真实页面进入目标主 tab 和当前工序/分组 tab，再点击可见“表单/记录/预览”入口；断言必须同时覆盖入口数量、当前分组名称、表单字段标题和至少一行当前列表可见事实，不能只看按钮存在或弹窗打开。
- Verification: 证据需记录目标租户/账号标签、业务编号、当前 tab/工序名称、首行或抽样行可见事实、生成表单中对应事实、自然触发请求 HTTP 状态和截图路径。若只允许前端操作，脚本只能监听页面自然请求和读取 DOM，不得直接调用接口或解析接口 JSON 作为通过依据。
- Forbidden action: 禁止用静态合同、API-only 响应、手工拼 URL、隐藏 DOM 命中、或默认/推断字段值冒充生成表单真实可见。
- Evidence: `doc/tasks/20260906-active-order-production-record-form-button/verification-report.md`，生产提交工序“表单”入口通过真实前端验证：目标活跃订单详情页进入生产提交，点击粗洗工序表单，表单复用当前工序首行输出物料、设备、数量、提交人与复核人等可见事实。

### 冻结/禁用类流程真实动作验证门禁

- Trigger: 业务流程要求冻结、锁定、作废、停用或禁止某些后续动作，例如禁止报工、提交、放行、审批、生成指令等。
- Preflight check: 先区分“页面展示冻结提示”和“目标动作被真实阻止”。E2E 必须在冻结状态下通过真实页面逐个尝试每一个被禁止动作，并监听对应正式写接口；按钮隐藏/禁用时记录 DOM 状态和写请求数，按钮可点击时断言正式业务错误码和没有成功写入。若一个任务自有批次不能同时支撑全部动作入口，可创建单独任务自有夹具，但必须记录来源、恢复方式和最终状态。
- Blocker: 只检查详情页提示文案、只看状态字段、只调用 API、只验证其中一个动作、或冻结样本缺少目标页面所需一线/质量上下文时，不能宣称冻结闭环通过。
- Verification: 证据至少包含目标租户/账号、样本标识、每个禁止动作的页面入口、按钮可见/禁用状态或业务错误码、目标写请求数、最终业务状态和任务夹具恢复结果。
- Forbidden action: 禁止用静态合同、提示语截图、SQL 状态或 API-only 替代真实页面动作；禁止为了让动作入口出现而修改共享路线/模板/生产基线数据；禁止把未覆盖的禁止项合并写成“均已禁止”。

### 多阶段状态流转完整链路门禁

- Trigger: 业务流程跨越发起、冻结/锁定、审批、作废/移除、重建、追溯或新对象重新进入活跃列表等多个阶段。
- Preflight check: 验收口径必须覆盖用户要求的最终业务结果，不能停在中间态。若需求是“升级后作废旧单并新建活跃订单”，E2E 至少要证明真实页面完成审批动作、旧对象不再出现在活跃入口、新对象按目标快照进入活跃入口并可打开详情；DB/API 只允许作为只读佐证。
- Frontend-only check: 当用户明确要求“只能通过前端实际操作，不能直接调用接口”时，E2E 的业务断言必须来自页面可见 DOM、输入值、弹窗、按钮状态或页面导航结果；脚本可以监听页面自然触发的正式请求并记录 HTTP 状态，但不得直接 `fetch`/`axios`/`APIRequest`，不得解析接口 JSON 作为通过 oracle，也不得用 DB 或 shell HTTP 调用补足前端断言。
- Repeated-section dialog check: 详情弹窗按工序重复渲染同名分区标题时，Playwright 断言应先锚定业务对象弹窗，再使用 `first()`、计数或滚动采集 DOM 文本验证“至少存在”和目标内容；不得因 `getByText(..., exact)` 命中多处触发 strict mode 就判定业务失败。若目标输入物料位于后续工序，必须在弹窗内部滚动验证，不能只截首屏。
- Blocker: 只有旧对象进入冻结/待审批、只有审批接口返回成功、只有数据库状态变更、只有新对象 ID 回写，或只验证新对象存在但未通过真实页面打开目标入口时，不能宣称完整链路 PASS。
- Verification: 证据需区分每个阶段：发起入口、审批待办、审批通过响应、旧对象终态、新对象前端可见性、新对象详情入口、关键版本快照、追溯入口是否覆盖旧对象。
- Forbidden action: 禁止把“冻结成功”写成“升级闭环成功”；禁止用 API-only 或历史数据库行替代真实页面终态，除非任务明确是只读审计并标注不是完整 E2E。
- Evidence: `doc/tasks/20260902-active-order-latest-version-upgrade-restart-code/verification-report.md`，活跃订单升级重启验收从旧订单冻结扩展到审批完成、旧活跃订单移出、新活跃订单进入活跃池并可打开详情。
...
- Preflight check: 任务目录下的一次性真实 E2E 脚本如果直接 `require('playwright')`，Node 会按脚本目录而不是前端目录解析依赖；必须显式从 `IntRuoyiFronted/node_modules/playwright` 加载，或把脚本放在前端测试目录并从前端工作目录运行。Element Plus tabs/table 会保留隐藏的非激活 DOM，表头或单元格断言必须锚定 `:visible`、当前激活 tab 或业务容器；不得把隐藏旧表头命中写成产品页面失败。worktree 验证前必须确认浏览器打开的是该 worktree 登记端口而非 `int_main` 基准端口；若访问 `/login` 被已有认证会话重定向，应识别为已登录状态并继续目标路径，不得把登录表单不存在误判为业务页面失败。
...
- Preflight check: 审批中心“我发起的”对 Native BPM 可能先按流程实例名称过滤，再由 provider 解析业务摘要；若业务编号或中文业务标题筛选 total 为 0，必须补充无关键词最新记录、processInstanceId 或正式请求 ID 的只读核验，区分“真实没有发起记录”和“业务摘要字段未参与上游过滤”。不得把按业务摘要关键词查不到直接写成发起链路不存在。
- Preflight check: 下载申请、访问申请等页面快捷按钮即使可见，也可能在发正式 POST 前被页面必需字段拦截；点击后必须同时监听正式写请求和页面错误提示。若页面明确提示缺少项目代码、主档 ID、文件 ID 等必需业务身份，应记录为 E2E BLOCKED 的前置数据缺口，不得把等待请求超时误写成后端接口失败。
...
- Verification: 需要验证“其他人可见同一受控文件状态”时，E2E 前必须先通过真实前端为两个不同账号分别证明同一个 ACTIVE 文件行可见，再执行检出、观察、检入清理；若第二账号只有菜单入口但列表为空，或看不到检出账号选中的同一文件，只能记录为账号/查看矩阵基线阻塞，不能用 admin 单账号、接口结果或数据库状态替代跨账号页面验收。
...
- Preflight check: 历史追溯页只读断言必须限定到可见且 enabled 的操作按钮或明确的写请求监听；不要用整页文本正则搜索“审批/提交/放行/保存”等词判断写入口，因为统一时间线、审批记录来源、状态说明等合法只读文案会包含这些词。
...
- Preflight check: Element Plus 多选下拉若使用 collapse tags、teleport 或弹层复用，必须锚定当前可见下拉面板逐项选择，并在继续填写前确认每个目标业务字段已显示为选中标签；不得只凭点击过选项就继续提交。
...
- Preflight check: 先按 `label-width + grid-template-columns + gap` 核算真实输入区宽度；关键字段必须使用专用布局类和静态合同覆盖。`el-input-number` 默认宽度可能大于网格列，必须显式设置 `width: 100%` 收敛到所在列；文本输入列需要 `min-width: 0` 和 `width: 100%`。必要时在 `el-select` 控件作用域内覆盖 `.el-select__tags-text` 默认省略宽度。窄栏里的 Switch 主标签与状态提示不得全部挤在一个可收缩 flex 行内，状态提示较长时应独占行或使用明确 grid 布局，并对关键标签设置不换行；禁用提示不能只用过浅灰色小字，应有足够对比度或明确状态条承载。Playwright 操作 Element Plus Switch 时，不得点击隐藏的 `input[role="switch"]`；应先等待可见 `.el-switch` 不含 `is-disabled`，再点击可见 `.el-switch` 或 `.el-switch__core`，最后读取隐藏 input 的 `aria-checked` 轮询校验状态，避免页面初始加载禁用态点击被吞掉。Element Plus 组件上的 `data-*` 可能透传到真实 `input` 而不是外层 wrapper，脚本应先用真实 DOM 快照确认属性落点，再决定使用 `input[data-*]` 还是 `[data-*] input`。
...
- Preflight check: 脚本必须在每个写响应 `code=0` 后立即更新机器可读数据状态，不得等列表刷新或后续断言完成；异常分支应通过同一真实页面路径停用、删除或恢复任务数据，并记录清理是否尝试、是否完成和是否仍有启用残留。重复运行会选择用户、角色、设备等远程下拉候选时，必须先避开已由任务残留占用的唯一组合，或先通过同一真实页面收尾旧残留，不能固定选第一个候选导致后续重复失败。
- Blocker: 写请求成功后脚本失败却只关闭浏览器、依赖成功路径末尾才清理、列表刷新失败导致脚本不知道写入已发生、异常分支找不到任务数据，或唯一键/业务约束因上次任务残留返回“已存在”时必须将精确残留标识和影响写入结果，不得宣称任务已清理。
...
- 触发场景：真实前端管理页已保存角色、矩阵、目录或数据范围授权，保存 HTTP 200 且预览包含目标账号，但目标账号重新登录业务页后仍看不到预期行。
- 经验规则：先证明授权页面保存和目标业务页均由 Playwright 真实操作完成，再检查业务列表候选查询是否在正式权限判断前被其它范围条件提前截断，例如空分配文件集合、目录候选集、状态预筛选或只查本人范围；授权矩阵应参与最终可见性判断，不能被前置候选 SQL 直接归零。
- 验证方式：补一个合同测试锁定“空范围不得截断后续授权判断”，再复跑真实前端跨账号 E2E；通过条件必须包含观察账号可见同一业务行、行内状态/按钮符合预期、以及写入动作通过同一前端路径清理。
- 禁止做法：禁止在授权保存 200 后直接宣称验收通过；禁止为了让观察账号有数据而用 API/SQL 补行或绕过页面；禁止把 API 只读结果替代业务页 DOM 可见性。
...
- 触发场景：用 Playwright 对 50 步以上业务主流程做只读或非提交型页面探针，且同一流程同时包含配置页、工作台、写入提交、审批、上传、归档和历史追溯。
- 经验规则：结果必须区分“页面/入口可达”和“业务动作闭环通过”。页面命中标题、菜单、表头或按钮只能证明对应入口可见；需要订单、待办、上传任务、不合格评审单、签名或状态流转的步骤，如果当前租户缺少任务自有数据，应记录 `BLOCKED` 和缺失条件，不得把相邻业务词命中写成完整 PASS。
- 验证方式：长流程矩阵至少记录每步状态、真实页面入口、截图或 DOM 证据、缺失数据条件、控制台/pageerror。错误文案匹配必须锚定明确页面错误短语，例如“页面不存在”“请求地址不存在”“系统异常”；不得用裸 `403`/`404` 正则扫描整页文本，因为业务编号、单据号或数据列可能包含这些数字并造成误判。
- 禁止做法：禁止用页面可达性替代写入、复核、放行、上传或归档动作；禁止用共享历史数据补齐任务自有链路；禁止忽略 pageerror 后继续把审批或待办链路判定为完整通过。
...
- Stale foreground check: 生产组长在另一个页签执行固定测试重置、重建或移出/重新加入活跃订单后，一线 PQC 旧页签可能保留重置前的空候选或旧选中状态。排查“看不到单子”时必须同时记录刷新前 DOM、回前台/聚焦后的自然 `/pqc/active-orders` 响应和重新选中结果；若接口已有目标订单而 DOM 仍停在 `未选择`，优先修复前台刷新/选单同步，不得归因于 `0%` 进度或后端任务缺失。
...
- Trigger: 用户要求清空多个页面、页签或互相关联的共享业务数据，且动作只能通过真实前端完成。
- Preflight check: 首次删除前必须逐页核对当前数量、可见删除/移除入口、批量能力、分页方式、权限和业务锁；确认每个目标页面都有正式可执行清理路径，并记录可能导致部分完成的依赖顺序。没有批量入口时，应先确认逐行操作能够覆盖全部分页和全部业务状态。
- Blocker: 任一目标页面没有删除入口、记录被审批/放行/归档等正式生命周期锁定、或清理必须改用 API/数据库才能继续时，必须明确记录已完成数量和剩余范围并停止；不得把其它页签的数据变化推断为目标页已清空。
- Verification: 删除后通过每个原始页面分别刷新并记录最终数量、空态或逐条阻塞文案；逻辑删除仍按本文件“逻辑删除表真实 E2E 断言门禁”核对有效记录口径。
- Forbidden action: 禁止在尚未核对全部目标页面清理能力前盲目开始跨页面批量删除；禁止用接口、数据库、隐藏列表、清空筛选或改变状态冒充页面全量删除；禁止吞掉部分失败后宣称全部完成。
- Evidence: `doc/tasks/20260912-clear-active-orders-reporting-data/verification-report.md`。
...
- Trigger: 真实页面 E2E 验证在多页签工作台内点击写入动作后自动打开详情，或验证“来源对象 → 新生成对象”的同会话详情映射，例如 Stage1 模拟后来源订单详情应显示新测试订单结果。
- Preflight check: 进入页面后必须先等待工作台根节点，再显式点击目标页签，最后等待目标模块根节点；不得在默认页签上直接等待非激活页签内容。写入动作成功后，必须同时断言弹框标题/提示中的来源对象和新对象映射，以及再次点击来源行详情仍命中本次生成对象。
- Blocker: 页面停留在默认页签、目标模块根节点不可见、弹框只显示来源对象自身、再次点击详情回到旧对象、或只从网络 JSON 判断新对象 ID 而页面没有可见映射时必须停止；不得把旧对象自身空态误写成新对象详情失败。
...
- Trigger: 修复审批候选人、待办可见性、claim 或授权有效期后，真实页面复验仍命中修复前已经生成的 `RUNNING/BPM_BOUND` 单据。
- Preflight check: 先只读核对目标单据的创建时间、绑定候选人、当前 Flowable assignee、流程变量和业务状态，区分旧在途数据残留与新代码行为；新代码有效性必须用修复后新建的任务自有单据复验。
- Blocker: 修复前旧单据的候选人或流程变量不包含当前验收账号、且页面无法用该账号正式处理时，不能把它直接判定为新代码失败；必须记录旧数据差异，并用真实页面通过正式有权账号处理或重新准备任务自有新单据。
- Verification: 旧单据清理动作必须走真实前端页面，记录页面入口、筛选方式、审批结果和随后新单据的真实提交/审批/下载证据；DB/API 只允许只读核验，除非用户明确授权特定数据模拟。
- Forbidden action: 禁止用 SQL/API 直接审批旧单据，禁止把旧单据候选人缺失作为新逻辑失败的唯一证据，禁止用管理员账号审批后冒充指定验收账号已处理新单据。

## 通知类 E2E 业务消息误命中门禁

- Trigger: 真实页面 E2E 验证站内信、待办通知、提醒通知、重配收件人后投递、或同一业务对象会同时产生审批流消息和业务提醒消息。
- Preflight check: 先确认被验收通知的正式模板标识、发送人昵称、页面正文关键字段和业务对象显示方式；若页面正文显示的是对象 ID 而不是业务编号，必须记录编号与 ID 的只读映射，并用页面可见模板语义 + 对象 ID/节点文案断言。
- Blocker: 页面列表中出现同一注册证编号、单据编号或业务标题，但模板属于审批待办、审批通过、系统通用消息而非本次验收的业务提醒时，不能判定接收人已收到目标通知；必须收窄断言或进入详情核对模板语义。
- Verification: 收件人和未收件人必须使用独立浏览器上下文重新登录，避免前端列表缓存造成跨账号误判；通过证据至少包含配置页面保存/回显、正式触发入口、接收人可见截图、非接收人无目标模板截图，以及必要的只读投递表/消息表映射。
- Forbidden action: 禁止只按业务编号全文匹配站内信；禁止把上传审批、审批通过、任务分派等伴生消息算作到期提醒；禁止用数据库投递行替代收件人真实页面可见性。

## 动态工作流runner身份与回执门禁
- 生产工序和QA工序必须分别使用正式身份，不按数组下标配对；关联依据来自页面正式数据。
- 一次页面动作可能产生多条业务回执。先冻结任务集合，逐条收集自然响应并核对、复核每个事件；不能只等待首条成功回执。
- 页面百分比可能四舍五入；完工条件需同时核对可见进度和页面自然加载的原始进度，禁止把显示100%当作原始数量已完整。
- 调度替身测试只证明runner算法，静态合同只证明源码约束，均不能代替真实Playwright主链成功证据。

### 一线自动化初始化与签名身份
- 页面容器或工序标签可见不代表初始化完成；读取物料、选择订单前核对页面正式加载门禁。订单号精确匹配需定位真正的值节点，不把“编码”等标签算入值。
- 固定测试员工必须以正式候选ID选择并使用其本人签名凭据；禁止任取第一候选后套用登录账号密码。缺少凭据应在重置测试数据前明确阻塞。
- 新增需要电子签名的业务动作时，后端必须在同一业务事务内调用统一签名服务重新认证，并由服务端生成正式 `signatureId`；前端只提交签名口令，不得提交或回显签名 ID、签名身份或签名口令。
- 签名失败必须在状态冻结、业务事实和成功审计记录写入前 fail fast；验收应同时证明失败路径没有业务副作用，成功路径的页面证据能够定位到正式签名记录。
- 无物料工序是否允许报工必须核对正式前后端合同；已支持工序数量模式时应明确冻结该模式，不能补造物料或把合法空集合误判配置缺失。
