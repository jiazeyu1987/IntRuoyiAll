...
- Preflight check: 检查 `.git/index.lock` 的绝对路径、长度和最后写入时间，并只读枚举活动 `git` / `git-lfs` 进程、命令行和归属；同时核对 `MERGE_HEAD`、rebase、cherry-pick 等中间态。只有锁文件为 0 字节、超过 60 秒未更新且不存在活动 Git 进程时，才允许删除该精确锁文件。
- Blocker: 存在活动或无法归属的 Git 进程、锁文件非空、锁文件不足 60 秒、目标不是当前仓库精确 `.git/index.lock`、Git 中间态未判明，或删除后仍无法正常读取 Git 状态时必须停止。
...
- Evidence: `doc/tasks/20260728-commit-int-main-frontend-backend-code-round2/execution-log.md`，基线暂存前确认零字节陈旧锁且无活动 Git 进程后恢复；`doc/tasks/20260731-merge-int-main-start-runtime/execution-log.md`，合并超时后按进程归属和合并中间态恢复。
...
- Preflight check: 将每个 Maven `-D...` 参数整体加单引号或双引号，例如 `'-Dtest=CodexTestCaseServiceImplTest'` 与 `"-Dsurefire.failIfNoSpecifiedTests=false"`；也可使用 `Start-Process -ArgumentList @(...)` 数组传参。多模块目标测试继续保留 `-pl <module> -am`。
