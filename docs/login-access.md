...
- Deployment consistency: 测试服助手由 `erp-invoice-voucher-print-assistant.service` 托管时，更新镜像后必须同步更新服务单元 `ExecStart` 的镜像标签，再执行 `systemctl daemon-reload` 和该服务重启；仅手工 `docker run` 会被 systemd 按旧标签重新创建。发布后必须同时核对实际容器镜像、所需依赖文件和助手 HTTP 响应，不能只以镜像构建成功判定生效。
