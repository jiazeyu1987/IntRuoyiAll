...
- 访问测试服务器、正式服务器或审查服务器前，必须先取得当前任务中的明确授权，并核对 `docs/server-access.md`。
- 未经当前任务明确授权，不得登录、联调、E2E、发布、重启、排障或验证远端环境。
- 正式服务器与审查服务器按生产等级处理；即使只读登录或 HTTP 探测，也必须有当前任务授权。
...
- 验收文档固定了申请人、审批人或普通用户时，即使本机默认 `芋道源码/admin` 可以登录，也只能把 `admin` 作为环境可用性对照；固定账号登录失败必须阻塞并报告，不能静默切换为 admin 或其它账号继续完成审批、下载等被验收动作。
...
- 触发场景：管理员重置密码与用户自助修改密码对当前密码、历史密码复用采取不同业务策略，或两条入口共用同一密码校验服务。
- 经验规则：先按 Controller、Service 方法和请求模型区分管理员重置与自助改密，再决定复用校验的适用入口；共用密码强度校验或密码历史记录写入，不代表复用限制也必须共用。
- 排查顺序：先为每个入口分别建立当前密码命中、历史密码命中和强度失败场景，再检查校验调用点；调整一条入口时保留另一条入口的明确回归测试，避免只删共享校验方法导致策略整体放宽。
- 验证方式：管理员重置和自助改密分别执行服务层回归；成功路径仍核对加密更新、凭据状态和历史记录写入，失败路径核对 `USER_PASSWORD_REUSE_FORBIDDEN` 只在仍受限的入口返回。
- 禁止做法：禁止根据错误文案或前端页面猜测策略边界，禁止为绕过一个入口的限制删除共享校验方法、删除密码历史表，或把密码强度校验一并移除。
...
- 本机前端端口变更、融合后首次 E2E 或新租户入口验证时，必须只读核对 `system_tenant.websites` 是否包含当前 `location.host`；补齐本机绑定后若页面仍报租户识别失败，应优先按租户缓存/运行态未刷新处理，重启或清理缓存后用同一租户账号重新登录，不得改用其它租户或直接塞 `tenant-id` 绕过页面识别。
- 不得静默切换账号、租户、后端地址、验证码模式或远端环境。
- 不得在文档、任务日志、提交信息或命令记录中写入密码、令牌、私钥、连接串密钥或其他凭据。
- 真实前端 E2E 发现目标业务账号密码错误或账号锁定时，必须记录目标入口、租户/账号标签、页面原始提示和影响后停止该身份链路；不得继续猜测密码、用相似账号替代验收身份、直接接口解锁、数据库改状态或重置密码来冒充页面路径通过。若只使用管理员账号完成了相邻页面验证，结论只能标记为局部通过，不能覆盖目标账号验收。
...
- Trigger: ERP 菜单接入独立外部工具、iframe 打印助手、发票凭证打印助手、分贝通费用报销助手，或用户要求“知道助手地址也不能直接访问”。
...
- Deployment consistency: 测试服助手由 `erp-invoice-voucher-print-assistant.service` 托管时，更新镜像后必须同步更新服务单元 `ExecStart` 的镜像标签，再执行 `systemctl daemon-reload` 和该服务重启；仅手工 `docker run` 会被 systemd 按旧标签重新创建。多进程外置项目增加统一鉴权入口后，公开端口的 service 必须启动该统一入口，不能继续启动旧静态前端或无鉴权代理；旧公开服务必须从安装/启用链路移除。发布后必须同时核对实际进程入口、镜像或文件、所需依赖和助手 HTTP 响应，不能只以构建成功判定生效。
...
- Nested base-data field check: 金蝶单据列表显示的业务列可能实际来自分录关联的基础资料，而不是单据头或分录自定义字段。接入前应先用目标单据 `View` 的正式模型定位字段所属对象和完整路径，再用只包含业务编号与目标字段的最小 `ExecuteBillQuery` 验证返回列数与读取权限；禁止根据同名旧字段、页面标题或历史账套映射复用字段标识。
...
- Preflight check: 先确认 `system_users` 已有失败计数、锁定标记和锁定时间正式字段；重置锁定必须显式清零失败计数、锁定标记和锁定时间，不能依赖 ORM 默认忽略 null；前端空闲退出必须挂在认证布局的统一 hook，监听鼠标、键盘、触摸、滚动和可见性变化。空闲退出时长必须来自管理员可维护的 `infra_config` 可见参数，不能在前端写死分钟数；对应配置必须有正式 SQL 种子并纳入本地重启探针；缺失或非法配置要显式失败并清理本地会话。
- Blocker: 失败次数达到阈值但未锁定、解锁后锁定时间仍残留、成功登录未清零、前端空闲超时只弹窗不退出/锁屏、或超时后未清空标签/锁状态时必须停止。
- Verification: 后端单测必须覆盖第 5 次失败锁定、锁定后拒绝登录、成功登录清零、解锁后恢复；真实 E2E 中第 5 次错误登录可直接返回“账号已锁定”，断言应以“最终进入锁定状态且随后正确密码仍被拒绝”为准；前端静态合同或类型检查必须覆盖空闲退出配置读取、禁止硬编码分钟数、活动事件监听、登录页重定向和注销时清空锁状态；SQL 合同、release migration policy gate 和本地运行库只读探针必须证明配置唯一、可见且值在允许范围内。
