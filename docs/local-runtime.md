...
- Child-process configuration boundary: 本地重启脚本生成 Java 子进程时，运行所需配置必须通过 `backendArgs` 或等价启动参数显式传递，仍需覆盖 DCC 预览、OnlyOffice 和电子签名证据等现行必需配置；只依赖父进程环境继承会把配置在 Spring Bean 初始化阶段丢失。DCC 下载已改为授权后直接返回原文件字节，不再读取或注入旧下载密钥环境变量，也不再传递旧下载加密启动参数。静态脚本合同必须断言下载加密配置不存在，并继续核对其他必需配置未丢失；发现必需配置缺失时必须停止，不得用默认值、切换数据源或绕过校验掩盖真实缺失配置。
