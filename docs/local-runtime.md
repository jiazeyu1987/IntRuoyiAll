...
- Preflight check: 隔离构建完成后、停止现有 `48081` 进程前必须重新 fetch `origin/int_main`。若远端后端 tree 已变化，必须基于新 tree 重新 clean test/package；若远端仅有文档提交，必须用 `git rev-parse <build-commit>:IntRuoyiBackend` 与 `git rev-parse origin/int_main:IntRuoyiBackend` 证明 tree 完全一致，不能只凭提交说明判断无需重建。
