...
- Version-policy rule: DCC 版本大/小判定必须走 `DccControlledFileVersionPolicy`。默认 `majorIdentitySegmentCount=1` 时 `A/1 -> A/2` 是小版本、`A/1 -> B/1` 是大版本；配置为 `2` 时 `A/1/1 -> A/1/2` 是小版本，`A/1`、`B/1` 等前两段是大版本身份。发布后续通知、影响评估、检入版本生成、提交流程版本字段和关联新版候选必须使用同一策略，禁止在通知层或单个服务中硬编码 `revisionCode` / `iterationNo`。
...
- Repair verification extension: 修复验收必须核对新增读取字段是否由正式生产方写入，不得只在测试夹具手工构造字段；新增业务检查调用旧服务时，应核对当前任务完整身份与多项目/多规程基数，不能把mock返回PASS当成实际判定正确。强制某类证据前必须追到真实业务入口及持久化生产方；有表、有写方法但无调用链不等于证据可生成。去重按来源身份，不能把同类多条合法明细当重复。补签需覆盖“已有复核ID但签名不完整”，冻结恢复需覆盖同轮与后续多轮外部原因变化；快照一致性需覆盖“业务对象冻结后、下游对象创建前”主数据升版的时间窗口，不能只验下游创建后的变化；修复原分支不等于上下游闭环通过。
...
- DCC version gate rule: 关联文件新版通知只允许由配置化大版本身份变化触发；小版本发布和首次发布必须在创建 follow-up 批次、影响评估任务、通知候选或投递前返回。接收人决定是否关联新版的流程只保留在大版本分支，不能由通知服务再用 `ACTIVE` 状态或候选人过滤补判大/小版本。
