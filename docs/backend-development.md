...
- Evidence: `doc/tasks/20260727-remove-lfs-assets/verification-report.md`；`doc/tasks/20260731-merge-int-main-start-runtime/verification-report.md`。

## 2026-07-31 合并后源码完整性与后端启动构建门禁

- Trigger: 合并 `int_main` 后准备启动本地后端、合并提交的内容树明显少于正常三方合并结果、`yudao-server-exec.jar` 时间早于合并、或 Maven 报 Lombok getter/setter/builder 和跨模块正式类型不存在。
- Preflight check: 先以合并前两端和 merge-base 重算正常三方合并树，核对遗漏/错误路径并保留任务开始前已存在的本地源码；再确认 Java 17 基线、Maven 版本、`maven-compiler-plugin` annotation processor 和合并后源码完整性。
- Blocker: 合并树遗漏正式源码、只存在旧 jar、Java 基线不匹配、依赖模块编译失败、或无法确认被忽略源码的归属时，不得启动后端。
- Verification: 使用明确的 Java 17 `JAVA_HOME` 执行 `mvn.cmd -pl yudao-server -am <quoted -D args> package`，生成新的 `yudao-server-exec.jar`，再启动并验证 `/actuator/health`。
- Forbidden action: 禁止只补首个缺失类型、用旧 jar、跳过失败模块、覆盖任务开始前的被忽略源码、手写 Lombok 生成方法，或把前端单独启动当作前后端启动成功。
- Evidence: `doc/tasks/20260731-merge-int-main-start-runtime/bug-regression-evidence.md`、`doc/tasks/20260731-merge-int-main-start-runtime/verification-report.md`。
