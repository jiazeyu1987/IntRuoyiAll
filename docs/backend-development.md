...
- Trigger: DCC 新建上传、历史文件升大版本、当前版本查询、固定会签矩阵、关联文件、提交重试或表单中心 UPLOAD 策略。
- Preflight check: 当前版本必须按租户、DCC 项目、文件类型叶子和规范化文件编号复合身份查询；正式提交必须校验项目分配、类别 UPLOAD 权限、类别与文件类型绑定，并持久化提交幂等键和 payload hash。申请人提交的会签人员只能与正式矩阵完全一致，不能替换矩阵。NEW 与 REVISION 内容审批通过后都进入 `READY_TO_PUBLISH`，再由独立文控发布动作生效。关联候选只允许当前 ACTIVE 的正式 DCC 文件，登记证附件和历史版本不得混入。
- Regression fixture rule: REVISION 上传和升大版本回归必须按 `revisionTargetControlledFileId` / `revisionSourceControlledFileId` 解析 Master，并 stub/断言 `selectByIdForUpdate(masterId)` 返回含 `currentActiveControlledFileId` 的完整 Master；不得只 stub 旧 `selectListByFileNumber` 分支导致错误码先被 `CONTROLLED_FILE_NOT_EXISTS` 覆盖。
- Blocker: 仅按文件编号查现行版、前端通过但后端缺权限/绑定校验、升版请求丢失版本或源文件 ID、同一幂等键可创建多份文件、申请人覆盖矩阵、NEW 审批后直接 ACTIVE，或旧表单中心入口绕过正式上传治理时必须停止。
- Verification: 后端回归覆盖复合身份、权限拒绝、矩阵不一致、重复键回读与冲突、新建/升版待发布；前端合同覆盖版本、源文件、幂等键和复合查询参数；迁移检查锁定幂等唯一键并退役旧 UPLOAD policy。
...
- 并发审计须列全写入口：预览、下载等读操作若顺带更新业务计数，也必须与心跳/命令共用正确的写入规则；加锁后不能继续信任锁前对象，SQL条件应保护实际写入时状态。前端await返回后须重新判断页面生命周期，不能用请求发出时的可见性启动后台计时。
- 同一业务的重新提交、导入和无新附件路径必须独立复核当前权限与版本身份；是否需要上传票据只决定文件校验，不能决定是否执行业务授权。历史删除应检查正式来源/基线/后继引用，不能只依赖单个投影字段为空。
- 上述交叉入口检查依据：`doc/tasks/20260913-dcc-additional-logic-audit/verification-report.md`；并发推导与运行复现证据分别记录。
...
- Historical voided active-order detail boundary: 作废/历史列表展示活跃订单时，详情入口必须以该活跃订单自身正式工序快照为准；缺少当前工序生产系数、目标数量或工序身份快照时，列表应保留订单身份和真实作废状态，同时返回明确的只读阻断原因并禁止进入会再次报错的详情链路。其它业务入口（例如 PQC 生产放行）能读取另一条有来源的详情记录，不得作为该历史活跃订单的快照重建来源；没有明确授权的数据修复和正式来源时，禁止按当前路线、工单号、批号或页面入口推断/伪造历史快照。
- Stage1 simulated parameter generation extension: Stage1 或其它演练链路生成生产提交事实时，设备参数必须按当前工序选择的设备和正式启用参数规则物化到事件 payload 及物料事实中；数值型参数有显式默认值时使用默认值，缺省且上下限完整时生成范围内具体数值（如 20~30℃ 生成 25），`textValue` 必须是实际提交值文本，不得使用标准范围文本或重复携带单位；文本/选择类参数才可使用 `defaultText/standardText`。有正式选择设备的输出物料还必须在 `selectedDevice`/`selectedDevices[]` 写入设备计量状态，Stage1 默认模拟为 `inMeteringValidityPeriod=true`；清场、物料、清洁三项确认必须作为提交事实写入根 payload 和物料明细，不得由详情页默认推断。不得为了“模拟成功”把 `deviceParameterReadings` 硬编码为空数组。验证必须同时锁定事件 payload 顶层、`materialDetails[]` 和 `MesProFeedbackMaterial` 明细三处参数来源，避免详情页表单只能显示“暂无设备参数”或把范围文本显示成提交值。Evidence: `doc/tasks/20260906-stage1-device-parameter-readings/verification-report.md`、`doc/tasks/20260906-stage1-parameter-value-within-range/`、`doc/tasks/20260906-stage1-device-clearance-status/`。
- Stage1 reset/copy snapshot extension: 重置指定测试订单或复制模拟活跃订单时，不能只把旧 `mes_pro_process_pool_active_order_process_snapshot` 原样复制；凡运行态新增必填冻结事实（如 `productionConfigSnapshotJson/productionConfigSnapshotSha256`、输入/输出物料集合、损耗原因、设备选择、参数规则），克隆路径必须从锁定 `routeVersionId` 的正式 `routeSnapshotJson.configSnapshots` 重建并校验 SHA，或者在旧快照已有完整一致字段时只做一致性校验。旧模板只缺新字段时可以按同一路线版本重建；旧模板已有部分生产配置快照但 hash、工序身份或子快照不一致时必须 fail fast，不得静默覆盖。回归必须覆盖“旧模板缺生产配置快照 -> 新订单克隆后补齐完整快照”和“已有不一致快照 -> 阻断”。Evidence: `doc/tasks/20260916-production-submit-config-snapshot-context/verification-report.md`。
...
- Active order combined QA snapshot boundary: 产品 QA 规程叠加通用包装检验规程时，必须在活跃订单冻结/重建阶段解析正式“产品 - 通用规程版本”绑定，并生成同一套不可变组合 QA 快照或组合 PQC 任务计划；一线 PQC 显示、提交 itemCode 校验、设备选项读取和检验进度分母必须全部来自该组合快照。组合工序列表必须先显示产品专用 QA，再按冻结任务创建顺序追加通用包装 QA，通用包装显示序号从产品 QA 最大序号后连续递增；通用包装标签显示为“小包装”“中大包装”，不得在前端追加“通用包装/通用检验规程”来源后缀；未知来源必须 fail fast。禁止只在 QA 管理前端展示通用规程引用、运行态临时拼接另一个 `regulationVersionId`、把通用项目额外塞进前端 payload，或让进度继续按产品单规程任务行达到 100%。Evidence: `doc/tasks/20260909-common-qa-frontline-pqc-static-audit/verification-report.md`、`doc/tasks/20260916-pqc-regulation-source-order/verification-report.md`。
- Active order PQC submission detail order extension: 生产组长活跃订单详情的 `PQC提交` / 过程检验记录必须沿用一线 PQC 选工序顺序。后端详情投影应显式输出 `qaProcessSort`：产品 QA 使用正式规程工序 `sort`，通用包装从产品 QA 最大序号后连续追加，并继续投影为“小包装”“中大包装”。前端不得按任务创建顺序、提交事件顺序、数据库返回顺序或 `qaProcessId` 推断排序；后端缺正式排序、产品 QA 版本身份或通用包装来源身份时必须 fail fast。Evidence: `doc/tasks/20260916-simulated-pqc-submit-process-order/verification-report.md`。
- Stage1 PQC simulation cardinality extension: Stage1 模拟可复用正式 PQC 任务表，但提交次数、持久进度和详情展示不得直接按生产工序副本行数计算。模拟侧身份应按 `regulationVersionId + qaProcessId + qaItemCode + inspectionRuleKey + inspectionType + businessDate + shiftCode + roundNo` 去重，同一身份优先使用已有 `CONFIRMED/SUBMITTED` 事实，没有事实时才模拟提交一个代表任务；详情侧只在 `simulated=true && simulationStage=STAGE1` 时折叠跨工序副本，真实非 Stage1 PQC 仍保留生产工序身份。重复身份的计划检验数量冲突必须 fail fast，禁止静默选第一条或把工序副本乘到 `PQC提交` / `检验项目` 计数上。Evidence: `doc/tasks/20260916-stage1-pqc-submit-dedup/verification-report.md`。
- EDHR readiness PQC result evidence extension: eDHR 放行预检 `INSPECTION_RESULT` 必须把“任务身份覆盖”和“正式结果证据”分层校验；重复任务身份至少包含 activeOrderId、routeProcessId、processId、regulationVersionId、qaProcessId、qaItemCode、inspectionRuleKey、businessDate、inspectionType、shiftCode、roundNo，产品专用与通用规程、多个 QA 项目或多个检验项目不得被粗粒度工序/班次口径误判为重复。PASS 还必须读取正式 PQC 提交事件、`mes_pro_process_pool_pqc_record` 签名/服务器提交时间/检验结论、逐件 `mes_pqc_inspection_piece_detail` 判定、PQC 组长 `mes_pqc_process_inspection_aggregate_detail` 聚合明细和失败结果对应的已关闭 `PQC_SUBMISSION` 不合格处置；`CONFIRMED` 状态本身不能作为结果完整证据。Evidence: `doc/tasks/20260913-edhr-static-008-reopened-readiness-checks/verification-report.md`。
- Common QA identity and multi-document set extension: 工单 `product_id` 是 MES 物料 ID，通用规程产品绑定使用 MDM 产品主档 ID；组合任务生成必须先从 `mes_md_item.product_master_id` 解析正式主档身份，再查询产品绑定，禁止直接用工单物料 ID 或按产品名称匹配。一个套版本由多份 Word 成员组成时，成员自身的检验项目必须命中该成员已启用的规则，但“必需检验类型是否齐全”按整个套版本的成员并集校验，不要求每份成员文档重复包含全部类型；整套缺失任一必需类型仍须 fail-fast。一线回放、项目设备选项读取和正式提交必须按每个任务来源规程自己的 `dccProjectCodeId + regulationVersionId` 校验，页面响应可继续保留活跃订单的产品 DCC 作为订单上下文。一次页面操作提交多个 item-scoped PQC 任务时，每条统一电子签名的主题和幂等键必须包含正式 `pqcTaskId`，不能只用员工和动作码。Verification: 后端回归至少覆盖 MES 物料 ID 与 MDM 主档 ID 不同、两份成员分别提供 FIRST/PATROL、整套缺必需类型、产品/通用 DCC 不同的显示与提交、以及同员工连续签署多个任务的幂等键互异；真实 Playwright 必须切换查看产品 QA 和每份通用成员，并完成至少一次多任务电子签名提交。Evidence: `doc/tasks/20260911-frontline-pqc-overlay-duplicate-identity/verification-report.md`。
- Common QA set version editing boundary: 通用检验规程套的检验项目编辑属于“套版本 + 成员规程版本”的原子升版动作；不得让前端先发布成员规程、再单独保存套版本，否则旧当前套版本会短暂引用已退役成员。后端升版接口必须校验请求成员与来源套版本成员一致，在同一事务内发布成员新版本并发布新套版本。历史套版本展示应读取锁定的成员快照，允许 `PUBLISHED/RETIRED`，但新增套版本成员仍只能绑定当前已发布成员。Evidence: `doc/tasks/20260916-common-qa-items-edit-version-upgrade/verification-report.md`。
- Route-owned device parameter consumer extension: 设备参数迁移到工艺路线后，不能只验证一线生产 runtime-config；一线 PQC 的检验设备选项显示来源仍是正式 PQC 检验项目设备配置，不得因为生产路线冻结设备编码不相交而隐藏已配置检验设备；路线冻结快照只用于读取并按 `routeProcessId + processId + deviceId` / 正式设备编码把设备参数挂到对应设备选项。PQC 提交只保存检验项目结果和设备身份，不得把生产报工的 `deviceParameterReadings` 塞进 PQC 提交事实。Evidence: `doc/tasks/20260909-route-flow-pqc-device-parameter-flow/verification-report.md`; `doc/tasks/20260921-pqc-equipment-visible-from-config/verification-report.md`。
...
- Production-release route identity extension: 由活跃订单/生产放行创建 eDHR 批次时，批次 `routeId`、`routeVersionId`、`routeVersionNo`、`routeCode`、`routeName` 必须来自命令指定的冻结 `routeVersionId` 与 `routeSnapshotJson`，并校验 `routeId/code/name` 非空且互相匹配；即使路线主档改名、重发或删除，也不得读取当前路线主档补写历史批次。归档 manifest 只能使用批次已保存的冻结路线身份，并包含 `routeVersionId` 与 `routeVersionNo`；缺冻结身份或快照不一致时必须 fail-fast。Evidence: `doc/tasks/20260914-edhr-static-012-route-rename-archive/verification-report.md`。
...
- 注册证上传审批待办可见性: 注册部经理处理上传审批时，审批中心待办不能只依赖 Flowable 当前 assignee，也不能只用任务变量 `processVariableValueEquals` 判断 `UPLOAD_CERTIFICATE`；运行态可能只有流程实例变量可用于业务识别。可认领上传审批必须先校验注册部经理角色和上传审批权限，再从当前租户活跃任务加载流程变量/实例变量，按 `requestType/requestOperation/certificateNo/requestId/certificateId` 识别并展示目标行；非注册证上传待办仍保持原 assignee 校验。
...
- Repair verification extension: 修复验收必须核对新增读取字段是否由正式生产方写入，不得只在测试夹具手工构造字段；新增业务检查调用旧服务时，应核对当前任务完整身份与多项目/多规程基数，不能把mock返回PASS当成实际判定正确。强制某类证据前必须追到真实业务入口及持久化生产方；有表、有写方法但无调用链不等于证据可生成。去重按来源身份，不能把同类多条合法明细当重复。补签需覆盖“已有复核ID但签名不完整”，冻结恢复需覆盖同轮与后续多轮外部原因变化；快照一致性需覆盖“业务对象冻结后、下游对象创建前”主数据升版的时间窗口，不能只验下游创建后的变化；修复原分支不等于上下游闭环通过。业务方修正正式依据或操作确认规则后，复核应分别记录旧缺陷是否消除、最新业务合同是否落地；文档更新不能作为代码完成证据，历史缺陷条目缺失也不能当作修复成功。
- Quantity and verdict reconciliation extension: 以关系表筛选正式来源时，必须同时核对关系上的数值约束；仅有正分配或关联ID，不能把整笔来源数量全量计入目标。共用计算器仍须覆盖部分分配、未分配余量、拆分/合并和纠错后的守恒。消费方重算业务结论时应与正式提交方使用相同输入维度及规则，例如报废数量与逐件检验判定的合成；不能只复用其中一部分字段后把合法持久化结果判为冲突。
- Correction-path parity extension: 业务更正入口必须与首次提交保持相同的数量关系、冻结和权限边界，并更新下游实际消费的正式事实或版本；只改事件JSON和修订日志不等于反馈表、物料事实、回填条件已同步。分别核对总量、合格量、损耗和分配量的定义，不得把含损耗总量与合格分配量直接判等。自由文本备注和客户端自填审计/签名字段不能成为正式来源身份或已验证签名依据。
- 多来源与多载体衔接检查：分配目标和原始记录所属对象需分别校验，跨对象分配后应沿正式关联取来源，不能按目标对象过滤掉合法原始记录。新增动态表单等证据载体后，必须贯通上层回执、空值门禁和追溯消费，不得混用不同实体的ID。多个来源写同一目标实例时，先按目标汇总完整内容和审计再提交生效；逐来源循环调用会锁定实例的动作必须覆盖第二项场景。
...
- Nonconformance active-order fact extension: 不合格评审必须把 `activeOrderId` 作为正式身份保存并进入活跃订单操作事实链；PQC 提交来源只能从正式 PQC 任务解析，PQC 放行来源只能从放行申请解析，批次执行来源只能从批次来源关系唯一解析。批次执行详情和历史追溯展示评审事实时，应先按批次来源唯一解析活跃订单，再按 `activeOrderId` 读取同一活跃订单下的全部评审事实，不能只按 `batchExecutionId` 查询，也不能用批号、工单号或当前用户推断。
- Nonconformance material preview extension: 活跃订单操作事实展示不合格评审材料时，读模型必须同时透传正式材料 JSON 和可预览 `fileId`；历史事实缺少标量 `reviewMaterialFileId` 时，只能从 `reviewMaterialsJson.activeMaterials[].fileId` 这类正式材料事实解析，不能用材料 URL、文件名或默认值绕过受控在线预览。前端可展示“查看评审材料”的前提是能解析到正式文件编号，否则必须明确报错。
- Nonconformance status projection extension: 活跃订单详情、PQC 放行详情、批次执行和追溯等状态展示读取 PQC 放行申请时，`PQC_RELEASE_REJECTED` 只是待不合格评审处置的中间流程状态；若同一放行申请最新 `PQC_RELEASE` 不合格评审已关闭且 `disposition=void`，状态投影必须优先显示“已作废”。“待审查”仅适用于尚未关闭或尚未终态处置的拒绝申请，禁止只按 `application_status` 固定映射，也禁止从页面入口、批次状态或工单冻结状态推断。
- PQC submission freeze boundary: 一线 PQC 提交的冻结校验必须携带正式 PQC 任务的 `activeOrderId`，并排除 `PQC_RELEASE` 放行申请评审；不得复用纯 `workOrderId` 级不合格冻结查询，否则同工单放行评审会误拦截当前 PQC 提交并出现 `batchExecutionId=null` 诊断。
...
- Evidence: `doc/tasks/20260914-edhr-static-026-conditional-loss-form-required/verification-report.md`。
...
- Production active-order picker detail: 一线生产填写页的活跃订单列表必须只返回后续工序/运行配置链路可填写的订单身份；至少同时满足当前组长归属、`activeStatus=ACTIVE` 和 `businessStatus=ACTIVE`。仅凭活跃池状态展示 `businessStatus` 已完工、作废、冻结或其它非 ACTIVE 行，会导致页面自动选中后在 `/active-order/{activeOrderId}/processes` 等严格链路报“班组活跃订单不存在”，因此列表端必须提前按同一正式合同过滤，不得用工单号、订单号、当前路线或其它身份补查兜底。
- Frozen QA equipment contract extension: 一线 PQC 加载或提交检验任务时，检验项目、设备选项和合同校验必须使用任务/活跃单锁定的 `qaRegulationVersionId`；不得按 `dccProjectCodeId` 重新读取“最新草稿优先/当前发布”的 QA 版本来校验同一批项目。当前 QA 版本新增、删除或修改项目后，既有 PQC 任务仍按锁定版本执行；若设备配置缺失，只能表现为该锁定项目无设备选项或明确配置缺失，不得把锁定版本项目误报为 `itemEquipmentConfig.itemCodes.projectMismatch`。Evidence: `doc/tasks/20260905-pqc-idi-project-mismatch-contract/verification-report.md`。
- Multi-material submission detail: 当一个工序按冻结路线产品 BOM 关联多个报工物料时，单行 `mes_pro_feedback` 不能继续冒充逐物料正式事实。运行态与服务端会话快照必须携带同一冻结物料集合；正式请求必须一次提交完整且不重复的 `materialDetails`，每项独立保存物料身份、完成数、损耗、损耗原因、设备和参数快照，任一失败整事务回滚。服务端保存报工事件前必须把正式解析后的 `materialCode/materialName/materialSpecification` 写回 `materialDetails` 快照，后续生产组长报工明细读取快照即可显示真实物料名；不得让前端用序号、编码或 ID 合成可见标题。工序池进度和初始分配取各物料完成数量最小值，不按 BOM 比例折算、不求和；旧主报工仍必须满足 `feedbackQuantity = qualifiedQuantity + unqualifiedQuantity`，其中 qualified 为最小值进度、unqualified 为逐物料损耗合计，禁止让损耗合计从最小值直接相减后产生负合格数量。批号只按生产订单号和物料编码读取系统内同步事实，系统内没有就返回空集合，不直连 ERP、不读取库存替代源、不复制批号到报工事实表。
...
- Preflight check: 模拟入口只能在用户明确确认“模拟数据”后使用；后端必须先校验活跃订单属于当前生产组长且为 `ACTIVE`，订单冻结路线版本、逐工序快照、生产目标数量、PQC 任务、PQC 计划数量和 QA 检验项目身份完整。模拟完成应沿用正式提交/分配/复核/逐件明细/汇集确认链路写入可追溯事实，并用 `simulated=true`、来源标识、`stageCode`、`simulationRunId` 和复核备注区分模拟数据；生产进度和检验进度必须由正式事实重新计算，Stage1 返回成功前还必须按活跃订单列表同口径回读持久化分配和 PQC 任务状态，禁止只信任模拟服务内部返回值或前端布尔值。分段模拟按钮必须是闭环动作：一次触发内先按上一轮 `simulationRunId` 清理本段模拟数据，再创建干净 fixture，再复用正式模拟服务，再自动验证输出。每段自造输入 fixture 必须逐字段、状态枚举和来源语义对照上一段当前输出契约；本段输出必须逐字段对照下一段当前输入契约。涉及现有业务节点时还必须核对后端真实常量及正式门禁引用，禁止为文档方便自造聚合节点、别名节点或双契约 fallback。若验证前需要对同一活跃订单执行“重建”，重建清理必须物理删除该订单自有运行态事件，确保事件幂等唯一键可重复生成，不得用软删除事件替代；缺少 `simulationRunId` 的事实不得纳入自动清理范围。无下游副作用断言若会读取关联实体，迁移与实体字段必须一并核对并先以 schema 合同锁定；不得等到真实 E2E 才因旧表缺字段而失败。
- Material detail boundary: 当一线生产按路线工序输入、输出物料表达时，Stage1 只能从活跃订单冻结路线版本的对应 `routeProcessId` 读取两类物料；输入物料只读取系统同步批号并作为物料平衡追溯事实，不填写完成数量、不参与工序进度；输出物料才写完成数量、损耗和进度。不得从产品 BOM、领料单、旧批记录物料字段、当前路线或库存推断任一物料。点击完工、提交详情、批记录回填、放行和追溯需要物料批号时，必须先按活跃订单 `workOrderId -> workOrder.code -> productionOrderNo` 解析全部正式生产领料单，按领料单 ID 分组逐张校验并形成集合事实；若活跃订单是 `SIM-COPY` 等模拟副本且当前工单编码已经同步出真实生产领料单或生产用料清单，必须优先使用当前工单编码的真实来源，只有当前来源不存在时才允许按受控模拟链路回退 `sourceActiveOrderId`。一张和多张领料单走同一集合链路，禁止事前绑定、任取第一张、合并成代表单或从单条旧绑定补齐。用户要求默认生产设备时，只能从当前组长、当前工序的启用设备绑定中按绑定记录 ID 升序选择第一台且设备本身必须启用；无有效设备时保持空值，禁止将组长、设备账号或工作站编号伪造成选用设备。PQC 模拟是否需要设备必须按锁定 QA 检验项目的正式设备选项判断，存在设备选项时按默认标记、排序和 ID 选择第一台，并同时写入 PQC 事件设备字段、逐件明细 `selectedEquipment*` 和 rawPayload 快照；禁止用生产工序设备绑定替代 PQC 检验设备配置，也禁止只写 rawPayload 而不写正式明细字段。
...
- Verification: 后端合同必须断言模拟入口逐工序生成生产提交、初始分配和生产组长复核，逐任务生成 PQC 逐件明细、PQC 提交和 PQC 组长确认/过程检验汇集，同时负向扫描不得写 `productionProgressPercent`、`inspectionProgressPercent` 等直接进度字段；合同测试必须断言每条生产/PQC模拟事实均带同一轮 `simulationRunId`、`stageCode` 和模拟来源标识，缺任一字段返回 blocker；Stage1 合同还必须断言 response/snapshot 中的进度来自最终持久化回读，前端成功提示展示 response 的真实进度值，不得写死“均为100%”；PQC 设备合同必须捕获 `MesProcessPoolCreatePqcInspectionReqDTO`、逐件明细和 rawPayload，证明有设备选项的检验项同时持久化设备账号、设备 ID、工作站 ID、设备编号和按样本聚合的检验结果。文档级验证必须分别扫描上一段输出、本段输入、本段输出、下一段输入和后端真实节点常量，证明字段、状态枚举、来源字段及节点键一一对应，旧错误键只允许出现在禁止项或负向测试说明中；重建合同必须断言自有运行态事件使用物理删除清理，并覆盖同一活跃订单重复“重建 -> 模拟完成”不再触发幂等唯一键冲突；前端合同必须断言按钮在活跃订单行操作区、确认文案明确“模拟数据”、成功后展示生产/PQC 数量和双进度并刷新列表。真实写入 E2E 只有在具备任务自有活跃订单 fixture、多账号签名前置、`simulationRunId` 落表验证和清理计划时执行。
- Forbidden action: 禁止用 SQL、状态字段、直接进度覆盖、默认成功、mock 提交、空逐件明细、跳过 PQC 任务确认、跳过生产组长/PQC 组长复核、事件软删除、只写 `simulated=true` 但缺 `simulationRunId`、硬编码成功文案、API-only 成功提示或吞异常来伪造双 100%。
...
- Preflight check: 首检数量和巡检比例是 QA 检验项目级正式配置；即使属于同一 QA 工序，不同检验项目也允许取不同值。保存、发布、任务生成和完整性审计必须同时保留工序身份与检验项目身份，并直接读取每个项目的 `applicableInspectionTypes`、`firstInspectionQuantity` 和 `patrolInspectionRatio`，不能提升为工序、产品或项目级全局数量/比例，也不能从抽样方案文字反向解析。PQC 任务生成、详情聚合和过程检验记录展示的唯一身份必须按生产工序 + QA 工序 + QA 项目 + 规则键冻结，`FIRST`、`PATROL_AM`、`PATROL_PM` 和 `FINAL` 都不得只按 QA 工序或末级项目名称合并；同一 QA 工序下多个 FINAL 项目必须生成多个 `FINAL` 任务，详情响应也必须透传 `qaItemCode` 供前端区分复合同名项目。多道冻结生产工序共用同一发布 QA 项目时，生成器必须遍历全部冻结生产工序并把 `routeProcessId + processId` 纳入任务身份、唯一键和重复检查；禁止先校验全部工序再返回第一道工序，禁止让后续工序因为同一 QA 项目/规则键被唯一键挤掉。末检适用性是项目级统一开关，所有工序和检验项目必须服从同一份 `finalInspectionApplicable`；明确适用时各正式 QA 工序都必须具备 FINAL 项目，明确不适用时不得保留 FINAL 项目。涉及比例字段时必须同时核对上下游单位和最终计算公式：当前 `patrolInspectionRatio` 存储百分比原值，任务数量公式再执行 `plannedQuantity × ratio ÷ 100`，因此 `AQL=0.4` 应保存为 `0.4`，前端不得二次除以 100。若一线或发布版需要展示 QA 项目的“抽样方案”“检验器具及设备”等人类可读原文，必须把原文作为独立正式字段贯穿保存、发布和运行态响应；数量、比例和设备选项只承担结构化计算或选择职责，不能替代原文。运行态还必须区分“发现正式 QA 工序”和“使用检验详情”两个边界：工序列表只依赖发布规程的产品、路线、版本、工序和检验项身份，历史原文字段为空时原样返回 `null`；打开详情或正式提交时再按同名字段严格校验。QA 生成的 `plannedInspectionQuantity` 是冻结规程给出的初始建议值：一线 `FIRST` 任务数量应取 `min(firstInspectionQuantity, orderQuantity)`，`FINAL` 和巡检仍按各自正式规则生成；它不是提交时必须相等的限制。一线 `FIRST`、`PATROL_AM`、`PATROL_PM` 和适用的 `FINAL` 都允许提交正整数 `actualInspectionQuantity`，但正式逐件样本数量必须与实际数量一致，且损耗数量不得超过实际数量。
...
### 通用检验规程成员设备选项必须进入升版快照

- Trigger: 通用检验规程套版本保存成员检验项目、`equipmentOptions`、保存并升版、成员版本快照回读，同时产品 QA 规程仍有独立的设备自动保存链路。
- Preflight check: 通用套升版请求必须在成员检验项目中接收结构化设备选项，并在保存、发布和版本详情响应之间保持设备 ID、设备编码、设备名称、设备编号、默认标记和顺序；后端必须校验设备 ID 为正数、设备编号非空、同一项目内设备与编号组合不重复、顺序合法。版本响应必须从该成员版本的正式快照读取设备选项，不能固定返回空列表或回查当前 QA 草稿。产品 QA 的设备自动保存仍是独立边界，不得用通用升版请求替代。
- Blocker: 通用请求未接收设备选项、保存后快照丢失、发布响应固定返回空设备列表、用当前配置或第一台设备补齐历史快照、重复/非法设备选项被静默跳过，或通用升版误修改产品 QA 自动保存数据时必须停止。
- Verification: 后端静态合同和服务回归必须覆盖请求 VO、快照解析、设备选项校验及发布响应回读；至少验证一条带设备选项的成员检验项目在版本响应中保持完整字段，并验证无设备的历史快照仍按正式空列表返回。再运行 `yudao-module-mes` 定向 Maven 测试；若被无关编译错误阻塞，必须记录具体文件和符号，不得把静态合同当作完整编译通过。
- Forbidden action: 禁止只在前端保存设备、把设备选项拼进原文、从当前 QA 配置回填历史成员版本、把空数组当默认成功或吞掉快照解析/校验异常，禁止让产品 QA 自动保存接口承担通用套版本升版职责。
- Evidence: `doc/tasks/20260916-common-qa-equipment-add/verification-report.md`。
...
- Preflight check: 发布产物必须是盖章 PDF；读取源文件或盖章失败必须抛 `CONTROLLED_FILE_STAMP_GENERATION_FAILED`，不得发布原 PDF。审批发布应在进入最终发布事务前完成盖章件绑定，发布事务只消费已绑定的 `publishedFileId`/`stampedFileId`，避免数据库回滚后遗留新物理文件。撤回文件删除只能删除业务记录，不得顺手物理删除 `sourceFileId`、`originalFileId`、`drawingPdfFileId` 等源文件产物；附件/源文件保留策略应走正式归档、保留或受控清理任务。
- Blocker: 出现 `publish original PDF instead`、`new PublishedArtifact(sourceFile.getId(), null, null)`、发布事务内新建物理盖章件却没有补偿、撤回删除调用 `deleteUnreferencedArtifacts` 或 `fileService.deleteFileList` 清源文件时必须停止。
- Verification: DCC 后端合同必须覆盖盖章失败无降级、审批发布缺少预绑定盖章件时 fail-fast、撤回删除不物理删除源文件；事务回归必须覆盖外层事务回滚后失败事实仍由独立事务保存。
- Forbidden action: 禁止用 `skipGovernance`、NAS 导入、历史 PDF 可浏览或前端隐藏按钮绕过盖章失败；禁止把失败状态写在注定回滚的业务事务中；禁止把撤回业务记录删除扩展成源文件物理清理。
- Evidence: `doc/tasks/20260909-high-priority-data-validation/verification-report.md`。
- Evidence: `doc/tasks/20260911-dcc-publication-finalization-hardening/verification-report.md`。

### Excel 导入导出必须有共享完整性下限

- Trigger: 所有调用 `ExcelUtils.read` 的导入预览/确认、所有调用 `ExcelUtils.write` 的导出、导入模板、DCC/MDM/MES/System/CRM 等批量数据入口。
- Preflight check: 导入 VO 必须用 Jakarta Bean Validation 标出必填、长度、范围、格式等行级规则；共享读取必须拒绝空 workbook、空解析结果、空行和违反 VO 注解的行。业务服务仍需补充跨行唯一性、重复键、外键存在性和影响行数守恒。导出写入前必须校验响应、文件名、sheet、head、data 和行对象不为空，并在成功响应中输出导出记录数证据。
- Blocker: 导入入口绕过 `ExcelUtils.read`、VO 缺少关键校验注解、空 Excel 仍进入 service、行级注解失败被收集为成功/忽略、导出没有记录数证据，或业务导入没有重复键/跨行一致性校验时必须停止。
- Verification: 框架回归必须覆盖空导入拒绝、Bean Validation 行失败拒绝、合法行成功；业务导入回归还需覆盖重复键、必填字段、外键和 created/updated/failed 计数守恒。
- Forbidden action: 禁止把解析出的空列表当作导入成功；禁止在业务服务里用默认值补齐 Excel 缺失关键字段；禁止只看 HTTP 200 或导入响应文案判断完整性。
- Evidence: `doc/tasks/20260909-high-priority-data-validation/verification-report.md`。

### MES 主数据 Excel 导入不得自动补码或同批覆盖

- Trigger: MES 物料、供应商、客户、设备台账 Excel 导入，尤其是 `MesMdItemImportExcelVO`、`MesMdVendorImportExcelVO`、`MesMdClientImportExcelVO`、`MesDvMachineryImportExcelVO` 和对应 `import*List` 服务。
- Preflight check: 导入 VO 必须声明关键字段 Bean Validation：编码/名称/关联编码必填，客户类型必填，邮箱字段用 `@Email`，库存和设备产能用范围注解。业务层必须在查库和写库前检查同一 workbook 内编码重复；物料导入空编码必须报错，不能调用自动编码生成。
- Blocker: 发现导入空编码会自动生成、同一 Excel 内重复编码可进入 create/update 分支、重复行依赖数据库唯一键或后续行覆盖前一行、VO 缺少关键字段注解、设备产能小于等于 0 或库存负数可通过时必须停止。
- Verification: 共享 Excel 回归覆盖 Bean Validation 生效；MES 合同测试覆盖四类导入 VO 注解和重复编码拦截。命令为 `mvn -pl yudao-framework/yudao-spring-boot-starter-excel -Dtest=ExcelUtilsTest test`；MES 模块可编译后运行 `mvn -pl yudao-module-mes -Dtest=MesMasterDataExcelImportValidationContractTest test`。
- Forbidden action: 禁止为导入缺失关键编码生成默认值、禁止把同批重复编码当作更新、禁止用导入结果 HTTP 200 替代行级失败明细核验。
- Evidence: `doc/tasks/20260909-high-priority-data-validation/verification-report.md`。

### DCC 项目代码导入确认必须复核预览证据

- Trigger: DCC 项目代码 Excel 导入预览/确认、`dcc_project_code_import_batch`、`dcc_project_code_import_row`、`confirmImport(batchId)`、导入预览行明细被清理/篡改/重复确认。
- Preflight check: 确认前必须重新读取预览行并复核：行列表非空、每行 `batchId` 属于当前批次、`importAction` 属于允许集合、`INVALID` 行必须有失败原因、行明细重新汇总后的 total/create/update/disable/unchanged/failure 计数必须与批次快照一致。复核失败必须在任何 create/update/disable 前 fail-fast。
- Blocker: 只校验批次状态为 `PREVIEWED` 就执行写入、预览行为空仍确认、批次计数与行明细不一致、未知 action 进入 switch、失败行无原因或失败行仍可确认时必须停止。
- Verification: DCC 回归必须覆盖篡改批次摘要后确认失败且未写入项目代码。命令为 `mvn -pl yudao-module-dcc -Dtest=DccProjectCodeServiceImplTest#confirmImportShouldRejectPreviewSummaryMismatchBeforeWrite test`。
- Forbidden action: 禁止用前端预览结果、HTTP 200、批次状态或 switch default 代替服务端二次完整性复核；禁止在确认阶段重新推断缺失预览行。
- Evidence: `doc/tasks/20260909-high-priority-data-validation/verification-report.md`。

### DCC 受控文件元数据导入确认必须先全量预检

- Trigger: DCC 受控文件基础信息 Excel 导入、`DccControlledFileMetadataImportExportServiceImpl.confirmImport`、受控文件 ID/文件名/文件编号批量更新。
- Preflight check: 预览和确认都必须拒绝空数据行；同一 workbook 内 `controlledFileId` 重复必须标记失败；确认必须先以 `applyUpdate=false` 完整评估全部行，只有 failureCount 为 0 才能第二阶段执行更新。
- Blocker: 确认逻辑边评估边更新、前面有效行已更新后后面失败、空 Excel 返回 total=0 成功、重复受控文件 ID 能进入更新、或失败行存在时仍调用 `metadataUpdateService.updateMetadata` 时必须停止。
- Verification: 单测覆盖“后续行失败时前序有效行不更新”、重复受控文件 ID、空数据行拒绝。命令为 `mvn -pl yudao-module-dcc -Dtest=DccControlledFileMetadataImportExportServiceTest#confirmImport_doesNotApplyEarlierValidRowWhenLaterRowInvalid test`；当前任务中 Maven 运行未完成时至少保留静态合同和 diff 证据。
- Forbidden action: 禁止依赖事务回滚掩盖已调用更新服务的副作用；禁止把 preview 的部分成功结果当作 confirm 成功；禁止确认阶段跳过失败行继续更新其他行。
- Evidence: `doc/tasks/20260909-high-priority-data-validation/verification-report.md`。
...
- Reactor-target rule: 定向测试类属于业务模块时，优先用拥有测试类的模块作为 `-pl` 目标，例如 `yudao-module-mes`；`yudao-server -am` 会额外进入服务器打包阶段，可能被 package-phase 插件或 reactor artifact 顺序阻塞。此类阻塞必须单独记录为完整服务器聚合验证限制，不能覆盖业务模块定向单测的 PASS 结论。
...
- Historical migration rule: 旧版本号格式合法或可以排序，只能证明“版本文本可解析”，不能证明版本链可以自动迁移。AUTO_MAP 必须同时满足稳定业务身份唯一、源文件 ownership/hash 证据完整、正式指针一致、检出状态无歧义及平台生命周期引用对齐；任一证据缺失时整条 Master 进入 blocker，先治理正式证据再重跑只读盘点，禁止按 `source_file_id` 存在、当前文件名、最新版本或共享源文件反推并补齐历史事实。涉及共享源的外部副本迁移时，必须先冻结全局引用集合，再按完整共享组执行；组内任一业务写入或明细证据写入失败都必须清理本次外部副本并让数据库事务失败，不能把逐条单测或局部成功当成组级原子性证据。
...
- Working-version submission rule: 创建初始工作版本、检入生成下一 Iteration 或创建下一 Revision `/1` 只形成模块内 WORKING 版本，不得同时创建平台候选或启动审批；只有显式提交当前 Revision 最新且未检出的 WORKING Iteration 时，才在同一事务内冻结路线、登记平台候选并启动审批。送审必须返回并推进原 Iteration，禁止通过上传/复制入口再建一条版本冒充送审对象；NEW 与 REVISION 类型的工作迭代共用该提交边界，OBSOLETE 不得进入。
...
- Preflight check: 业务页面若要“在线查看”上传附件，业务记录必须保存可反查正式归属的文件 ID 或业务文件 ID；物理 URL 只能作为存储位置证据，不能作为预览授权身份，也不能在缺少文件 ID 时直接打开作为兜底。
...
- Trigger: 文件、配置、证书或其它受控对象正式生效后，还要生成通知、影响评估、分发、培训或其它后续工作，并要求这些后续动作不阻塞对象生效。
- Transaction rule: “后续动作不阻塞生效”只指消息送达和人工处理不参与生效条件；用于防止后续工作永久丢失的批次账本、对象身份和冻结快照必须与 ACTIVE/正式指针/旧版替代处于同一事务。账本或任一必需快照写入失败时，主事务回滚；成功提交后再异步发送或人工处理。
...
- Physical artifact rule: 生效事务提交前不得移动、删除或改写上一版 ACTIVE 的物理文件路径。旧版替代状态、当前 ACTIVE 指针和后续快照可以在同一事务内回滚；物理文件归档这类不可随数据库回滚的副作用只能放在提交后可重试链路，或单独证明失败不会让旧正式版不可读。
...
- Distribution extension: 文控分发待办聚合中，收件人记录引用缺失的分发主记录也属于悬空过期投影；应过滤该收件人并继续返回其它有效待办，不能让 `/dcc/distribution-tasks/my-page` 中断个人工作台或待处理数量加载。
...
- 逻辑身份清空扩展：页面允许清空业务编号时，要同时检查显示字段、规范化逻辑键和复合唯一索引。空串经归一化转为null后，捕获updateById入参不能证明旧索引值已清除；读取侧新增一致性报错也不能替代写入侧同步。验收应覆盖非空改名和主动清空两条分支，并验证实际持久化结果。Evidence: `doc/tasks/20260913-dcc-27-bug-code-recheck/verification-report.md`。
...
- Trigger: 新增或修改 GxP 业务记录、基础数据、权限、配置、电子签名、批准、发布、作废、删除、Job、外部同步或系统变更写入口。
- Preflight check: 先在机器可读 GxP 操作登记表中确定稳定对象身份、动作类型、原因规则、签名规则、真实 before/after、证据引用、保存类别、策略版本和 owner；领域服务负责业务语义，统一审计内核只负责安全上下文身份、正式服务器时间、规范化、序号、hash 和只追加持久化。API 访问日志或 Controller 拦截器不能替代领域审计，也不能从请求参数猜测数据库真实变化。
- Interface rule: 统一的是内部审计写入契约、统一账本、统一策略登记、统一查询导出和统一 CI 覆盖门禁；不要把所有业务 Controller 改造成一个远程审计业务接口。业务 API 保持领域语义，成功 GxP 写入在领域服务事务内调用统一 append。
- Implementation placement rule: 当前统一 GxP 审计内核第一阶段落在 `yudao-module-signature` 的 `signature.gxp` 子包，电子签名、可信时间、归档和特权审计是可关联证据，不等同于统一事件账本本身。后续接入 eDHR/DCC/BPM/Showroom/权限/配置时，应依赖 `GxpAuditTrailService.append`，不得复制电子签名专用表结构冒充跨域账本。
- Business-evidence boundary: 业务审批发起、发布申请、配置变更等非电子签名动作只能记录自身业务对象、版本、原因、before/after、幂等键和后续可关联证据；不得为满足签名字段而伪造 `signatureRecordId`、签名人或签名时间。电子签名证据必须来自真实签名链路，并通过独立审计事件或证据引用关联。
- Transaction rule: 成功业务数据与成功审计事件必须由同一 Spring 事务和同一物理数据库事务提交；审计写入、hash、链头或必需证据失败时业务整体回滚。失败尝试如需独立记录，必须使用明确 `FAILED` 结果和单独策略，不能补写或伪造成功事件。
...
- Trigger: DCC 审批复用统一电子签名服务，但 DCC 查询、验签或审计页面仍依赖领域签名投影。
- Preflight check: 明确统一签名事实和 DCC 领域投影各自的证据合同；两者必须在同一业务事务写入。DCC 投影保存 DCC 自己的规范化证据、HMAC 算法和密钥版本，不能把统一签名服务的通用摘要直接冒充 DCC HMAC。
- Time-source rule: DCC HMAC 生成使用的 `signedAt` 必须与领域投影保存、验签接口重算使用的 `signedAt` 是同一个服务端时间值；不得在统一签名返回后再用第二个当前时间覆盖投影，否则跨秒签名会必然验签失败。
- Response identity rule: 审批接口返回给前端的 `signatureId`、证据短哈希和签名时间必须指向 `dcc_controlled_file_signature` 领域投影；统一签名表 ID 和 SHA-256 摘要只能作为底层签名事实，不能冒充 DCC 验签入口身份。
- Blocker: 统一签名成功但领域投影缺失、投影写入失败后审批仍推进、算法/密钥版本与领域验签器不一致、或响应 ID 不能直接用于 DCC 验签时必须停止。
- Verification: 单测同时断言统一签名调用、领域投影 insert、同一 signedAt 参与 HMAC 与持久化、参与人和文件证据快照、HMAC 算法/密钥版本、响应身份为 DCC 投影；真实审批后按文件和任务核对每个审批节点恰有一条可验投影。
- Forbidden action: 禁止查询时临时拼装投影、失败后异步补写、用 SHA-256 摘要替代 DCC HMAC，或因测试环境而跳过投影完整性。
- Evidence: `doc/tasks/20260907-dcc-release-notification-impact/verification-report.md`。

## 正式依据变更与条件表单全链路门禁

- Trigger: 正式数量依据从现场记录切换为外部业务单据，或条件表单只在最终放行时求值。
- Preflight check: 沿读取、完工回填、来源哈希、回执、写表、建批、待办、前置关系、进度及最终放行逐层检查同一业务事实；过程证据与正式依据保留各自身份，禁止继续用旧来源的数量恒等式校验新来源。
- Confirmation rule: “没有查到单据”与“用户确认无此业务”分开记录；查询失败、未审核、数量无效和匹配歧义不得变成无业务确认。确认必须传到服务端并保留操作者，成功回执重放应沿用已记录确认。
- Applicability rule: 条件不适用须在表单实例和待办生成之前处理，并同步用于工序前置关系、并行推进、进度分母及最终门禁；按工序求值时使用路线工序身份，不能用整批存在损耗推断每个工序都需要损耗表。
- Verification: 覆盖现场有损耗但正式无损耗、现场无损耗但正式单据有数量、多张单据全部留痕且不重复累计、取消确认零写入、已确认回执重放，以及条件表单从建批到最终放行的一致性。扩展测试失败须用隔离的原代码复现来区分基线失败，不改运行中服务或共享源码来切换基线。
- Cross-order extension: 共享分配修复须检查列表进度、完工守卫、正式来源读取和写表校验四层；来源事件工单与数量归属工单必须分开校验。重新分配删除某个来源后，lastEventId/lastReviewId必须仍指向保留的正式来源。
- Typed-receipt extension: 新增动态表单证据时，不仅修改writer，还需检查资料汇总、业务批准、持久化回执、幂等回放、Controller序列化与前端类型；至少一条测试调用真实相邻服务，不能两层分别mock成功后宣称链路通过。
- Evidence: `doc/tasks/20260914-edhr-main-flow-fixes/verification-report.md`。

## 跨设备域身份映射与路线快照完整性门禁

- Trigger: 工艺路线班组设备、设备台账、PQC 检验设备来自不同表或不同领域，但需要在运行态关联设备参数；候选路线配置支持完整 JSON、识别导入和结构化局部保存。
- Preflight check: 先确认不同设备表的主键不可互换。跨域关联使用双方正式且唯一的设备编码，并在候选版本保存时校验设备存在、启用、同工序编码唯一；参数规则必须精确匹配容器的 `routeProcessId + processId + deviceId`。
- Concurrency rule: 候选快照的 `expectedRouteSnapshotSha256` 必须进入数据库 UPDATE 条件，读取后在 Java 中比较再无条件 `updateById` 不能防止两个并发请求后写覆盖。
- Candidate lifecycle rule: 候选快照 UPDATE 除 ID 和旧哈希外，还必须在数据库条件中约束 `DRAFT` 且未激活；否则并发提交或发布后，旧保存请求仍可能把生命周期覆盖回草稿。
- Candidate import rule: JSON/Word 识别写入候选配置时，工序覆盖范围必须读取该候选快照自己的 `configSnapshots.flowGraph.nodes`；不得读取当前正式路线工序表来替代尚未发布的新增、删除或重排结果。
...
- Trigger: 从任意既有小版本选择源版本创建下一大版本，同时项目配置了新文件目录/分类模板。
- Preflight check: 显式区分 NEW 与 REVISION。REVISION 必须继承原 Master、文件编号和身份链，并继续校验源版本归属及下一大版本序号；新文件模板只约束新身份首次创建，不得重新要求既有 Master 选择当前模板。
- Blocker: 合法既有版本因项目模板变化而无法升版、为绕过模板连带关闭 Master/版本链校验、或升版时创建第二个逻辑身份时必须停止。
- Verification: 覆盖从非最新小版本选择源版本升大版、模板已变化/缺少当前选择、身份继承、重复大版本冲突和审批发布链；断言只跳过新建模板校验，不跳过版本链规则。
- Forbidden action: 禁止把 REVISION 当新上传重新编号，禁止复制新文件校验总开关后整体关闭治理校验，禁止自动猜测新的目录模板项。
- Evidence: `doc/tasks/20260907-dcc-release-notification-impact/verification-report.md`。

## 自动回填与人工流转调用链门禁

- Trigger: 正式来源自动写入表单中心，同时存在人工FILL待办、工序推进及独立报告上传阶段。
- Preflight: 区分表单创建人、实际提交人及已验证自动回填的调用身份；自动权限只允许服务端构造，不能从formData解析。对下游尚无人工待办、首工序已有不同候选人待办、主记录最后完成这三种顺序分别追踪。
- Verification: 使用真实效果执行器和待办服务验证自动证据完成、已有待办审计收口、真实人工权限及下一工序派发；不能只mock writer成功。独立报告阶段不得与普通记录阶段形成相互等待，最终放行应统一检查两者证据。
- Source contract: 配置字段应从真实前端保存请求追到冻结快照和执行读取；若同一物料集合存在两份JSON路径，不能让用户重复维护隐藏字段，也不能以服务互相猜测补齐。正式配置缺失必须在对应边界明确报错。

## 电子签名身份与业务详情一致性门禁

- Trigger: 放行、批准或审计详情需要展示可跳转的电子签名。
- Rule: 正式签名服务返回的 `signatureId` 必须在同一业务事务的领域记录或决策字段中持久化；审计 JSON 只能作为关联快照，不能替代领域存储，也不能用事务 ID、操作事实 ID 或哈希冒充。
- Audit association: 审计适配器必须把命令携带的正式事务 ID 写入关联快照，避免读模型依赖一个写入端未持久化的字段。
- Read rule: 详情读模型按审计行独立持久化的正式业务关联键读取领域签名字段；关联快照 ID 存在时必须与正式记录交叉校验。历史快照字段缺失时，只有存在严格唯一的正式关系才可读取；领域签名 ID 缺失时保持未签名并阻止跳转，不得按签名、时间或文本猜测其它记录。
- Verification: 覆盖正式签名失败阻止业务成功、幂等重试复用签名身份、领域字段与审计事实一致、历史快照字段缺失但唯一正式关系仍可读取、快照与领域记录不一致时 fail-fast、旧领域记录不回填，以及前端缺少真实 ID 时禁用跳转。
