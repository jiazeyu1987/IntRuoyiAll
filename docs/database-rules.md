...
- Cross-migration retirement: 同一个 legacy 页面可能由多份历史迁移分别登记页面菜单、打印策略或补打子菜单；下线前必须按稳定菜单 ID 冻结完整集合，新增一份只做软下线和授权集合清理的迁移，并同步收紧前端路由、旧静态合同和 release coverage exclusion，保留后端 API、业务表和审计数据。
