...
- 触发：开发文档位于任务目录，但技能validator默认检查仓库的docs/product、docs/system或docs/acceptance。
- 规则：执行前检查validator实际读取的文件路径；针对任务包可在独立进程映射同名文档再运行结构检查，不能拿全局旧文档PASS证明新任务包。
- 验证：报告写明实际检查目录、操作步骤字数、验收编号及语义与PRD/BDD/状态的对应关系；对Git忽略或未跟踪文件直接检查内容，git diff --check不能单独作为覆盖证据。
- 禁止：仅添加OP/AC编号就判逻辑一致；仅检查接口名包含trace就断言真实页面入口和正式来源相同。
- 主链核对：用户给出完整业务链时，逐节点记录输入、动作、输出和验收，区分进度条件、完工回执与不同层级放行；复核当前页面实际handler及被调用服务分支，不能把旧说明、未调用方法或相邻历史分支当成现行必经步骤。
