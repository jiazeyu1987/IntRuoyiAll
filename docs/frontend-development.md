...
- Preflight check: 先按截图或稳定 DOM 锚点定位真实可见区域，再追溯该区域的映射函数、正式响应 VO 和提交载荷；明确描述字段只负责展示，编码和 ID 只负责 key、编辑定位或提交身份。
  ...
  - Submission-material extension: 生产组长报工管理、活跃订单报工详情或其它报工明细展示物料标题时，必须直接读取后端正式 `materialName` 或服务端写入提交快照的 `materialName`；`materialCode`、`materialId` 和行号只用于身份、定位或提交，不得拼成 `物料 N` 占位或作为正常展示回退。历史快照缺少物料名时应暴露数据链路缺口，并通过新提交快照或正式迁移补齐。
...
- Detail split extension: 同一详情弹框中若“生产提交”和“PQC提交”来自两套不同正式工序模型，不得按可见工序名称或生产工序容器把两者强行组合。前端应拆成业务类型主 tab，再在各自主 tab 下按各自正式身份字段分组；后端详情响应必须透传 PQC 自有工序身份（如 `qaProcessId/qaProcessName`），缺失时 fail-fast 暴露数据合同问题，不得用生产 `processName`、数组位置或首条记录兜底。
