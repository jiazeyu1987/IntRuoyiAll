...
- Keywords: 切换填写人, 协助填写人, 填写人快照, `assistSwitchTasks`, `candidateUserSnapshot`, `getEdhrBatchExecution`, batchTaskId, execution_id, active 执行记录, `batchExecutionId + workOrderId + routeProcessId + batchRecordReportId + batchCode`, `mes_pro_batch_record_execution.task_id` 禁止写批次任务 ID, 弹窗打开耗时过长 -> `docs/backend-development.md#切换填写人快照读取边界`
