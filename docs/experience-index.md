...
- Keywords: 零排产活跃订单, 无排产工单加入, active-order candidates add, mes_pro_route_product, 唯一 ACTIVE 路线版本, route_snapshot_json scheduleUseConfigs, ERP 计划开工日期, 多排产冲突 -> `docs/backend-development.md#零排产活跃订单必须使用发布态正式路线`
...
- Keywords: PQC 填写, PQC 组长复核, QA 检验规程, QA 规程配置状态, QA 检验规则跟随产品, 不同产品不同检验规则, qaInspectionTypeRules, qaProductRuleDrafts, 产品级规则草稿, 已配置 QA 规程, 待配置 QA 规程, project-statuses, mes_qa_inspection_regulation.product_id, DCC 项目代码 productMasterId, IDI 压力泵规则, QA规程截图逐页, 合并单元格分组, 检验设备, 设备编号, 接收标准, 检验方法, 参数上下限, itemResults, rawPayload.pqcPieceValues, pqcItemDetails, 固定 length appearance seal pressure, 项目级检验快照, active-order/processes, 活跃路线全部当前工序, 单工序正式 fixture, AC-M15, PQC 末检, 末检不适用, finalInspectionApplicable, finalInspectionNotApplicableReason, FINAL 检验项目, AC-M21, 过程检验记录汇集, aggregateApprovedPqcSubmission, processInspectionAggregationStatus, mes_pqc_process_inspection_aggregate_detail, CONFIRMED, source_piece_detail_id -> `docs/backend-development.md#mes-pqc-项目级检验快照门禁`
