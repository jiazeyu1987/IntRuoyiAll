...
- Keywords: Playwright 外部头像 502, 外部资源异常, 非目标链路异常, 非当前页签接口, TODO ERR_ABORTED, DONE 目标链路, targetBadResponseCount, target write request count, requestfailed 归因, 目标链路错误数, DCC 写请求为 0 -> `docs/e2e-rules.md#playwright-目标链路与外部资源异常归因门禁`
...
- Keywords: Java 17 测试 API, List.getFirst, getFirst(), getLast(), Java 21-only collection API, testCompile cannot find symbol getFirst, Maven Java 17 baseline, 不切 Java 21, 不跳过 testCompile -> `docs/powershell-memory.md#maven-java-17-测试-api-基线门禁`
- Keywords: 共享 Word parser, typed failure 错误映射, FormTemplateRecognition failureCode, canonical raw result, sourceHash, fileNameHash, adapter 中间层丢失错误码 -> `docs/system/shared-word-template-parser-design.md#跨层错误码与-canonical-等价门禁`
...
- Keywords: 验证 worktree 编译基线, verification unblocker, 非当前任务编译阻塞, compile baseline, QA/PQC 基线补丁, 未到达 Surefire, isolated worktree baseline diff -> `docs/worktree-memory.md#隔离验证-worktree-编译基线差异门禁`
- Keywords: No worktree port registry entry is registered, worktree port registry entry, 附加 worktree 提交钩子, branch-runtime-port-guard, reserve-worktree-slot, 不启动服务但提交失败, slot frontendPort backendPort -> `docs/worktree-memory.md#worktree-端口段与原子槽位门禁`
...
- Keywords: system_role, system_role_menu, system_user_role, tenant 1 admin, package_id=0, menu_ids 扫描, QA 角色, 只有目标角色可见, admin 赋权, 动态菜单权限角色 -> `docs/database-rules.md#系统角色菜单授权-tenant-1-admin-门禁`
...
- Keywords: DCC 产品立项, 产品建档申请, DCC 项目代码 MDM 产品绑定, dcc_product_onboarding_request, dcc_project_code.product_master_id, productMasterId, MDM 产品主数据, 受控文件提交按项目代码带出产品主数据, H2 fixture 缺 product_master_id, MesBatchRecordBaseSchemaTest, DCC mapper 跨模块 DB 回归缺列 -> `docs/database-rules.md#dcc-项目代码-mdm-产品建档绑定门禁`
...
- Keywords: UnifiedListTemplate, 标准列表多维筛选, TableMultiFilter, 条件 Tab, 默认空条件, 必填默认条件可见, submitDate, setCondition 预置隐藏筛选, addConditionTab, removeActiveConditionTab, activeConditionId, 快速过滤, 复合工具栏, flex 0宽, 筛选控件不可见, multi-filter 0 width, 正式 query 透传, 交集查询, 重复正式参数校验, 不发送 multiFilters, 禁止页面级 maxInlineFilters 特例, 同页多列表, 同步工单, showMultiFilter 显式接入, definitions state events, 旧 quick filter 残留 -> `docs/frontend-development.md#统一列表复合工具栏布局门禁`
...
- Keywords: 角色内容页签拆分, 生产组长专门页签, PQC组长专门页签, leaderType, TeamLeaderWorkbenchPage, edhr-batch-production-leader, edhr-batch-pqc-leader, process-pool/production-leader, process-pool/pqc-leader, 组长工作台不再显示, 专门做一个页签, 类似批次执行, QA下面, 主导航页签, 内部 Tab 误判, 页面内部功能模块 Tab, 人员管理 报工管理 损耗管理 历史表单, 报工管理空表, 页签切换不加载, 重复 module tabs, data-pqc-leader-module-tab-history, showPqcFormHistoryModule, isPqcFormHistoryTab, 独立列池 tableKey, showProductionModuleTabs, activeProductionModuleTab, 共享 gate, 相邻角色合同, 相反角色半成品 -> `docs/frontend-development.md#前端角色内容页签拆分口径门禁`
...
- Keywords: 运行记录状态数字, 状态 20, AUTO MANUAL, epoch millis, 13位时间戳, failureMessage, 自动同步运行记录, 用户可读运行状态 -> `docs/frontend-development.md#业务运行记录用户可读展示门禁`
- Keywords: ERP表格自动同步, Profile ERP 自动同步, kingdee-table-auto-sync, ERP 模块已禁用, 生产工单同款同步, runIncrementalSyncJob, JobApi.updateJob, JobApi.updateJobStatus, infra/job cron, handlerName 正式 Job -> `docs/frontend-development.md#erp-表格同步-job-链路门禁`
...
- Keywords: isolated backend jar, dirty main workspace, int_main 48081, jar SHA256, local E2E reload, clean worktree build, 重启到最新后端, 先构建再停旧进程, 临时 detached worktree 构建, 未登录 401, 登录态路由验证, route-not-found, 旧业务错误, fat jar 内嵌模块, BOOT-INF/lib, 运行 Jar 缺少关键 class, 跨模块 API 方法, 接口 调用方 实现类 服务 Mapper 成组替换, Controller 注解, RequestParam required=false, 缺省 keyword, javap -v, DccUploadDirectoryResolver, Spring Boot nested jar, jar uf0, compress_type=0, 内嵌 jar 压缩导致 classpath resource missing, schema 字段核对 -> `docs/local-runtime.md#2026-07-24-隔离构建-jar-加载门禁`
...
- Keywords: 设备账号上下文不完整, post workstation binding, loginUserId, postIds, 压力泵全工序, frontline-pressure-pump, hasAnyPermissionsInRoles, hasAnyPermissions, 权限角色切换工序, 岗位14 -> `docs/backend-development.md#mes-一线设备账号权限门禁`
...
- Keywords: PQC 填写, PQC 组长复核, QA 检验规程, QA 规程配置状态, QA 检验规则跟随产品, 不同产品不同检验规则, qaInspectionTypeRules, qaProductRuleDrafts, 产品级规则草稿, 已配置 QA 规程, 待配置 QA 规程, project-statuses, mes_qa_inspection_regulation.product_id, DCC 项目代码 productMasterId, IDI 压力泵规则, QA规程截图逐页, 合并单元格分组, 检验设备, 设备编号, 接收标准, 检验方法, 参数上下限, itemResults, rawPayload.pqcPieceValues, pqcItemDetails, 固定 length appearance seal pressure, 项目级检验快照, AC-M15, PQC 末检, 末检不适用, finalInspectionApplicable, finalInspectionNotApplicableReason, FINAL 检验项目, AC-M21, 过程检验记录汇集, aggregateApprovedPqcSubmission, processInspectionAggregationStatus, mes_pqc_process_inspection_aggregate_detail, CONFIRMED, source_piece_detail_id -> `docs/backend-development.md#mes-pqc-项目级检验快照门禁`
