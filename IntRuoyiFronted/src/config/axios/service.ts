...
type RequestCustomHeaders = InternalAxiosRequestConfig['headers'] & {
  isToken?: boolean
  isEncrypt?: boolean
  isEncrypted?: boolean
}
...
const isBinaryAttachmentResponse = (response: AxiosResponse<any>) => {
  ...
  return contentDisposition.includes('attachment') && contentType.includes('application/json')
}
...
const createApiError = (message: string, code: number | string, data: any): ApiError => {
  ...
  error.backendErrorCode =
    details && typeof details === 'object' && !Array.isArray(details)
      ? (details as Record<string, unknown>).backendErrorCode
      : undefined
  return error
}

const isModuleDisabledReminder = (message: string) => {
  return message.includes('已禁用') && message.includes('参考') && message.includes('开启')
}
...
service.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    const headers = config.headers as RequestCustomHeaders
    ...
    if (getAccessToken() && isToken) {
      headers.Authorization = 'Bearer ' + getAccessToken() // 让每个请求携带自定义 token
    }
    // 设置租户
...
service.interceptors.response.use(
  async (response: AxiosResponse<any>) => {
    let { data } = response
    const config = response.config as RequestCustomConfig
    const ignoreErrorMessage = config.ignoreErrorMessage === true
    if (!data) {
      // 返回“[HTTP]请求没有返回值”;
      throw new Error()
    }
    ...
    if (ignoreMsgs.indexOf(msg) !== -1) {
      ...
      if (!isRefreshToken) {
        ...
        try {
          ...
          requestList = []
          if ((config.headers as RequestCustomHeaders)?.isEncrypt) {
            ;(config.headers as RequestCustomHeaders).isEncrypted = true
          }
        ...
        return new Promise((resolve) => {
          requestList.push(() => {
            ;(config.headers as RequestCustomHeaders).Authorization = 'Bearer ' + getAccessToken() // 让每个请求携带自定义token 请根据实际情况自行修改
      ...
      if (!ignoreErrorMessage) {
        if (isModuleDisabledReminder(msg)) {
          ElMessage.warning(msg)
        } else {
          ElMessage.error(msg || t('sys.api.errMsg500'))
        }
      }
      return Promise.reject(createApiError(msg, code, data))
    } else if (code === 901) {
      ElMessage.error({
        offset: 300,
