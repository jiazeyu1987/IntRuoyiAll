...
const assertExistingProcessDataLoaded = () => {
  if (!['definition', 'update', 'copy'].includes(actionType)) {
    return
  }
  if (formData.value.type === BpmModelType.BPMN && !formData.value.bpmnXml?.trim()) {
    throw new Error('流程 XML 未加载，请检查模型数据')
  }
  if (formData.value.type === BpmModelType.SIMPLE && !formData.value.simpleModel) {
    throw new Error('流程设计数据未加载，请检查模型数据')
  }
}
...
const initData = async () => {
  if (actionType === 'definition') {
    ...
    assertExistingProcessDataLoaded()
    syncProcessData()
