    ...
    >
      ...
      <el-table-column label="操作" align="center" width="120">
        <template #default="scope">
          ...
          <el-button
            v-if="scope.row.status === IoTOtaTaskStatusEnum.PAUSED.value"
            link
            type="primary"
            @click="handleResumeTask(scope.row.id)"
          ...
          <el-button
            v-if="
              [
                IoTOtaTaskStatusEnum.IN_PROGRESS.value,
                IoTOtaTaskStatusEnum.PAUSED.value
...
const handlePauseTask = async (id: number) => {
  try {
    await message.confirm('确认要暂停该升级任务吗？')
    await IoTOtaTaskApi.pauseOtaTask(id)
...
const handleResumeTask = async (id: number) => {
  try {
    await message.confirm('确认要继续该升级任务吗？')
    await IoTOtaTaskApi.resumeOtaTask(id)
