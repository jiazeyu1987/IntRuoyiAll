...
type ProductSalesFormData = Partial<WmProductSalesVO> & {
  noticeId?: number
  salesDate?: string
...
const createDefaultFormData = (): ProductSalesFormData => ({
  id: undefined,
  code: undefined,
  name: undefined,
  status: undefined,
...
const formData = ref<ProductSalesFormData>(createDefaultFormData())
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = (await WmProductSalesApi.getProductSales(id)) as ProductSalesFormData
    } finally {
...
/** 从发货通知单发起销售出库单 */
const openByNotice = async (notice: WmSalesNoticeVO) => {
  dialogVisible.value = true
  formType.value = 'create'
  resetForm()
  await generateCode()
  formData.value.noticeId = notice.id
  formData.value.name = notice.name ? `${notice.name}-销售出库` : `${notice.code}-销售出库`
  formData.value.salesOrderCode = notice.salesOrderCode
...
defineExpose({ open, openByNotice })
</script>
