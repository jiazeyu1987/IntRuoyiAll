<template>
  ...
  <ProductSalesForm ref="productSalesFormRef" @success="handleProductSalesSuccess" />
</template>
...
const productSalesFormRef = ref<InstanceType<typeof ProductSalesForm>>()
...
const handleFinish = async () => {
  if (!formData.value.id) {
    message.warning('发货通知单不存在，无法继续销售出库单流程')
    return
  }
  try {
    await message.confirm('当前发货通知单将进入销售出库单流程，是否继续生成销售出库单？')
    dialogVisible.value = false
    await nextTick()
    await productSalesFormRef.value?.openByNotice(formData.value as unknown as WmSalesNoticeVO)
  } catch {
  }
}

const handleProductSalesSuccess = () => {
  emit('success')
}
