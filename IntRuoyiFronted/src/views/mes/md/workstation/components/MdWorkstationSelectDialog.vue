  ...
  <Dialog title="工作站选择" v-model="dialogVisible" width="80%">
    ...
    <ContentWrap>
      ...
      >
        ...
        <el-table-column label="班次小时" align="center" prop="shiftHours" width="110">
          <template #default="{ row }">
            {{ formatWorkstationCapacity(row.shiftHours, '小时') }}
          </template>
        </el-table-column>
        <el-table-column label="班次产能" align="center" prop="todayCapacity" width="120">
          <template #default="{ row }">
            {{ formatWorkstationCapacity(row.todayCapacity, '/班次') }}
          </template>
        </el-table-column>
        <el-table-column
          label="设备标准小时产能"
        ...
        >
          <template #default="{ row }">
            {{ formatWorkstationCapacity(row.machineryStandardHourlyCapacity, '/h') }}
          </template>
        </el-table-column>
...
const buildWorkstationDialogPageParams = () => ({
  ...queryParams,
  processId: normalizePositiveProcessId(queryParams.processId)
})

const formatWorkstationCapacity = (value?: number | string | null, unit = '') => {
  ...
  const label = numberValue.toLocaleString('zh-CN', { maximumFractionDigits: 6 })
...
const getList = async () => {
  loading.value = true
  try {
    const data = await MdWorkstationApi.getWorkstationPage(buildWorkstationDialogPageParams())
...
const resetQuery = () => {
  ...
  queryParams.processId = normalizePositiveProcessId(props.processId) // 保持外部传入的工序过滤
...
const open = async (selectedIds?: number[]) => {
  ...
  // 重置查询条件 + 页码
  queryParams.code = undefined
  queryParams.processId = normalizePositiveProcessId(props.processId) // 外部传入 processId 则默认按工序过滤
  queryParams.workshopId = undefined
  queryParams.status = CommonStatusEnum.ENABLE
  queryParams.pageNo = 1
  // 清空上一次的选中状态
