  ...
  <el-row :gutter="20">
    ...
    <el-col :span="4" :xs="24">
      <ContentWrap class="h-1/1">
        <MdItemTypeTree ref="typeTreeRef" @node-click="handleTypeNodeClick" />
      </ContentWrap>
    </el-col>
...
const loadListFromRoute = async () => {
  ...
  await getList()
  const openDetailId =
    typeof route.query.openDetailId === 'string' ? route.query.openDetailId : ''
  if (!openDetailId) {
    openedDetailId.value = ''
    return
  }
  if (openedDetailId.value === openDetailId) {
    return
  }
  openedDetailId.value = openDetailId
  openForm('detail', Number(openDetailId))
}
...
/** 初始化 **/
watch(
  () => [route.query.code, route.query.openDetailId],
  async () => {
