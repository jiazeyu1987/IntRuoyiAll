...
const handleSyncErpBom = async () => {
  ...
  await message.confirm('确认从 ERP 同步并覆盖当前产品 BOM 组成吗？当前 BOM组成 将被替换。')
  erpBomSyncLoading.value = true
  try {
    const result = await MdProductBomApi.syncErpBom(formData.value.id)
    await productBomFormRef.value?.reload?.()
