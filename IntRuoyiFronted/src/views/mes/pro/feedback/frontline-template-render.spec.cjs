...
const templateSource = viewSource.slice(0, viewSource.indexOf('<script'))
...
assert.match(viewSource, /configuredDefectReasons\s*=\s*computed[\s\S]*runtimeConfig\?\.defectReasons/, 'production defect reasons must come from team leader runtime configuration.')
assert.match(viewSource, /v-for="defect in configuredDefectReasons"/, 'production UI must render configured process defect reasons dynamically.')
...
assert.doesNotMatch(templateSource, /DETECTION_SUCCESS|DETECTION_FAILED/, 'PQC operator UI must not show production-style success/failure buttons.')
assert.doesNotMatch(templateSource, /检验方法/, 'PQC operator UI must not show inspection method row.')
