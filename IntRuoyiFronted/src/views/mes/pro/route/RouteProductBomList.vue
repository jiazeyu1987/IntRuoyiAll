  ...
  <div>
    ...
    <el-row class="mb-10px">
      <el-button
        ...
        :disabled="!activeProcessId || bomActionDisabled"
        :title="bomActionDisabled ? CANDIDATE_EDIT_REQUIRED_MESSAGE : ''"
      >
...
import { ProRouteProcessApi, type ProRouteBaseProcessVO } from '@/api/mes/pro/route/process'
...
const bomActionDisabled = computed(() => !isDraftCandidateEdit.value)
...
const processList = ref<ProRouteBaseProcessVO[]>([]) // 工序列表（用于 Tab）
const activeProcessId = ref('') // 当前选中的工序 Tab

/** 查询工序列表 */
const loadProcessList = async () => {
  const list = await ProRouteProcessApi.getRouteBaseProcessListByRoute(props.routeId)
...
const openForm = (type: string, row?: ProRouteProductBomVO) => {
  requireCandidateRouteVersionId('BOM 物料打开')
  ...
  if (type === 'create') {
    formData.value = {
      routeId: props.routeId,
      routeVersionId: requireCandidateRouteVersionId('BOM 物料打开'),
      processId: Number(activeProcessId.value),
      productId: props.productId,
      quantity: 1
    }
...
const submitForm = async () => {
  ...
  try {
    const payload = {
      ...formData.value,
      routeVersionId: requireCandidateRouteVersionId('BOM 物料保存')
    }
    if (formType.value === 'create') {
      await ProRouteProductBomApi.createRouteProductBom(payload)
      message.success(t('common.createSuccess'))
    } else {
      await ProRouteProductBomApi.updateRouteProductBom(payload)
...
const handleDelete = async (id: number) => {
  try {
    ...
    await ProRouteProductBomApi.deleteRouteProductBom(id, requireCandidateRouteVersionId('BOM 物料删除'))
    ...
    message.error(resolveRouteOperationErrorMessage(error, '删除 BOM 物料失败，请查看后端返回错误'))
  }
}
