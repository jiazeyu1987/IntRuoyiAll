...
type ImportTableQueryParams = {
  name: string | undefined
  comment: string | undefined
  dataSourceConfigId: number
}

const queryParams = reactive<ImportTableQueryParams>({
  name: undefined,
  comment: undefined,
...
/** 查询表数据 */
const resolveDataSourceConfigId = () => {
  const dataSourceConfigId = dataSourceConfigList.value[0]?.id
  if (typeof dataSourceConfigId !== 'number') {
    throw new Error('缺少可用的数据源配置，无法导入表')
  }
...
const resetQuery = async () => {
  ...
  queryParams.dataSourceConfigId = resolveDataSourceConfigId()
  await getList()
}
...
const tableRef = ref<{ toggleRowSelection: (row: CodegenApi.DatabaseTableVO) => void }>() // 表格的 Ref
const tableList = ref<string[]>([]) // 选中的表名

/** 处理某一行的点击 */
const handleRowClick = (row: CodegenApi.DatabaseTableVO) => {
  tableRef.value?.toggleRowSelection(row)
}

/** 多选框选中数据 */
const handleSelectionChange = (selection: CodegenApi.DatabaseTableVO[]) => {
