    ...
    <el-row :gutter="16" class="row">
      <el-col :md="6" :sm="12" :xs="24" :loading="loading">
        <ComparisonCard
          ...
          prefix="￥"
          :decimals="2"
          :value="toDisplayNumber(fenToYuan(orderComparison?.value?.orderPayPrice || 0))"
          :reference="toDisplayNumber(fenToYuan(orderComparison?.reference?.orderPayPrice || 0))"
        />
      </el-col>
      <el-col :md="6" :sm="12" :xs="24" :loading="loading">
        <ComparisonCard
          tag="今日"
          title="用户访问量"
          :value="toDisplayNumber(userComparison?.value?.visitUserCount)"
          :reference="toDisplayNumber(userComparison?.reference?.visitUserCount)"
        />
      </el-col>
      <el-col :md="6" :sm="12" :xs="24" :loading="loading">
        <ComparisonCard
          tag="今日"
          title="订单量"
          :value="toDisplayNumber(orderComparison?.value?.orderPayCount)"
          :reference="toDisplayNumber(orderComparison?.reference?.orderPayCount)"
        />
      </el-col>
      <el-col :md="6" :sm="12" :xs="24" :loading="loading">
        <ComparisonCard
          tag="今日"
          title="新增用户"
          :value="toDisplayNumber(userComparison?.value?.registerUserCount)"
          :reference="toDisplayNumber(userComparison?.reference?.registerUserCount)"
        />
      </el-col>
    </el-row>
    <el-row :gutter="16" class="row">
...
const toDisplayNumber = (value: string | number | undefined) => Number(value || 0)
