...
const createDefaultFormData = (): OrderUpdatePriceFormData => ({
  id: undefined,
  adjustPrice: 0,
  payPrice: '',
  newPayPrice: ''
...
const formData = ref<OrderUpdatePriceFormData>(createDefaultFormData())
watch(
...
const submitForm = async () => {
  ...
  try {
    const currentFormData = cloneDeep(unref(formData))
    const data = {
      id: currentFormData.id,
      adjustPrice: convertToInteger(currentFormData.adjustPrice)
    }
