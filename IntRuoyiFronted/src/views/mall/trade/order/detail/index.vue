    ...
    <el-descriptions :column="4" title="收货信息">
      ...
      <div v-if="formData.deliveryType === DeliveryTypeEnum.EXPRESS.type">
        ...
        <el-descriptions-item v-if="formData.deliveryTime" label="发货时间: ">
          {{ formatDate(formData.deliveryTime as Date) }}
        </el-descriptions-item>
        ...
        <el-descriptions-item v-if="expressTrackList.length > 0" label="物流详情: ">
          <el-timeline>
            <el-timeline-item
              ...
              :key="index"
              :timestamp="formatDate(express.time as Date)"
            >
              {{ express.content }}
            </el-timeline-item>
          </el-timeline>
...
const expressTrackList = ref<ExpressTrackItem[]>([]) // 物流详情
const pickUpStore = ref<Partial<DeliveryPickUpStoreApi.DeliveryPickUpStoreVO>>({}) // 自提门店
onMounted(async () => {
  await getDetail()
  // 如果配送方式为快递，则查询物流公司
