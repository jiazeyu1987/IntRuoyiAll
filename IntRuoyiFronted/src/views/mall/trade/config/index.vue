...
type TradeConfigFormData = TradeConfigPayload & {
  id: number | null
...
const createDefaultFormData = (): TradeConfigFormData => ({
  id: null,
  ...
  brokerageEnabledCondition: undefined as unknown as number,
  brokerageBindMode: undefined as unknown as number,
  brokeragePosterUrls: [],
...
const formRef = ref()
const formData = ref<TradeConfigFormData>(createDefaultFormData())
...
const submitForm = async () => {
  ...
  try {
    const data = cloneDeep(unref(formData.value)) as TradeConfigFormData
    // 金额放大
    ...
    await ConfigApi.saveTradeConfig(data as unknown as ConfigApi.ConfigVO)
...
const getConfig = async () => {
  formLoading.value = true
  try {
    const data = (await ConfigApi.getTradeConfig()) as TradeConfigFormData | null
    if (data != null) {
      formData.value = {
