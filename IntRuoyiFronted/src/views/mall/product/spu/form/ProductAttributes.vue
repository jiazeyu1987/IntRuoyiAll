...
const getAttributeItem = (index: number): AttributeItem | undefined => {
...
const handleCloseValue = (index: number, valueIndex: number) => {
  const attribute = getAttributeItem(index)
...
const handleInputConfirm = async (index: number, propertyId: number) => {
  ...
  if (inputValue.value) {
    ...
    if (existValue) {
      if (existValue.id == null) {
        throw new Error('property value id is required')
      }
      attributeIndex.value = null
      inputValue.value = ''
