...
const createDefaultFormData = (): PropertyValueFormData => ({
  id: undefined,
...
const formData = ref<PropertyValueFormData>(createDefaultFormData())
...
const open = async (type: string, propertyId: number, id?: number) => {
  ...
  if (id) {
    ...
    try {
      const data = await PropertyApi.getPropertyValue(id)
      formData.value = {
        id: data.id,
