...
const createDefaultFormData = (): CommentFormData => ({
  id: undefined,
  userId: undefined,
  userNickname: '',
  userAvatar: '',
...
const formData = ref<CommentFormData>(createDefaultFormData())
  ...
  if (id) {
    ...
    try {
      ...
      formData.value = {
        ...
        spuId: data.spuId,
        skuId: data.skuId,
        descriptionScores: data.descriptionScores,
        benefitScores: data.benefitScores,
...
const submitForm = async () => {
  ...
  try {
    if (formType.value === 'create') {
      await CommentApi.createComment(unref(formData.value) as unknown as CommentApi.CommentVO)
