...
type SeckillActivityListItem = Omit<
  SeckillActivityApi.SeckillActivityVO,
  'configIds' | 'products'
> & {
  configIds: number[]
  picUrl?: string
  spuName?: string
  marketPrice?: number
  products: SeckillActivityApi.SeckillProductVO[]
}

const loading = ref(true) // 列表的加载中
const total = ref(0) // 列表的总页数
const list = ref<SeckillActivityListItem[]>([]) // 列表的数据
