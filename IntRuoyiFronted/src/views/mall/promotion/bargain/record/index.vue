...
type BargainRecordListItem = BargainRecordApi.BargainRecordVO & {
  avatar?: string
  nickname?: string
  activity?: {
    name?: string
...
const list = ref<BargainRecordListItem[]>([]) // 列表的数据
...
const openForm = (type: string) => {
  message.error(`当前后端未提供管理后台砍价记录${type}接口：/promotion/bargain-record/create`)
}
...
const handleExport = async () => {
  ...
  try {
    message.error('当前后端未提供管理后台砍价记录导出接口：/promotion/bargain-record/export-excel')
  } finally {
