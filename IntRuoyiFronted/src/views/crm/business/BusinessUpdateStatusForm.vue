...
const statusList = ref<any[]>([]) // 商机状态列表
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const currentStatus = Number(formData.value.status || 0)
    await BusinessApi.updateBusinessStatus({
      id: formData.value.id,
      statusId: currentStatus > 0 ? currentStatus : undefined,
      endStatus: currentStatus < 0 ? -currentStatus : undefined
    } as any)
