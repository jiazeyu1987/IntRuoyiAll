...
const fileBeforeUpload: UploadProps['beforeUpload'] = (file: UploadRawFile) => {
...
const appCertUpload = async (options: UploadRequestOptions) => {
  const content = await options.file.text()
  formData.value.config.appCertContent = content
  options.onSuccess(content)
}

const alipayPublicCertUpload = async (options: UploadRequestOptions) => {
  const content = await options.file.text()
  formData.value.config.alipayPublicCertContent = content
  options.onSuccess(content)
}

const rootCertUpload = async (options: UploadRequestOptions) => {
  const content = await options.file.text()
