...
interface RefundTableRow extends RefundApi.RefundVO {
  no?: string
  payPrice: number
  refundPrice: number
...
type RefundQueryParams = Omit<RefundApi.RefundPageReqVO, 'createTime' | 'successTime'> & {
  merchantRefundId?: string
  payPrice?: number
  refundPrice?: number
  createTime: string[]
...
const list = ref<RefundTableRow[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await RefundApi.getRefundPage(queryParams as unknown as RefundApi.RefundPageReqVO)
...
const handleExport = async () => {
  try {
    ...
    const data = await RefundApi.exportRefund(
      queryParams as unknown as RefundApi.PayRefundExportReqVO
    )
