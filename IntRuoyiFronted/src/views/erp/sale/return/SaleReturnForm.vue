...
const handleSaleOrderChange = (order: SaleOrderVO) => {
  ...
  // 将订单设置到退货单
  formData.value.orderId = orderData.id
  formData.value.orderNo = orderData.no
  formData.value.customerId = orderData.customerId
  formData.value.accountId = orderData.accountId
  formData.value.saleUserId = orderData.saleUserId
  formData.value.discountPercent = orderData.discountPercent
  formData.value.remark = orderData.remark
  formData.value.fileUrl = orderData.fileUrl
  // 将订单项设置到退货单项
  ...
  formData.value.items = orderData.items.filter((item: any) => item.count > 0)
}
