...
const getSummaries = (param: SummaryMethodProps) => {
  ...
  columns.forEach((column, index) => {
    ...
    if (key && ['count', 'totalPrice'].includes(key)) {
      const sum = getSumValue(data.map((item) => Number(item[key])))
      sums[index] = key === 'count' ? erpCountInputFormatter(sum) : erpPriceInputFormatter(sum)
    } else {
      sums[index] = ''
    }
  })
...
/** 处理仓库变更 */
const onChangeWarehouse = (_warehouseId: number | undefined, row: Record<string, any>) => {
  // 加载库存
  setStockCount(row)
}

/** 处理产品变更 */
const onChangeProduct = (productId: number, row: Record<string, any>) => {
...
/** 加载库存 */
const setStockCount = async (row: Record<string, any>) => {
