...
const getSummaries = (param: SummaryMethodProps) => {
  ...
  columns.forEach((column, index: number) => {
    ...
    if (key && ['totalPrice', 'receiptedPrice', 'receiptPrice'].includes(key)) {
...
const handleAddSaleOut = (rows: SaleOutVO[]) => {
  rows.forEach((row) => {
    formData.value.push({
      ...
      receiptedPrice: (row as any).receiptPrice,
      receiptPrice: row.totalPrice - (row as any).receiptPrice
    })
  })
}
...
const handleAddSaleReturn = (rows: SaleReturnVO[]) => {
  rows.forEach((row) => {
    formData.value.push({
      ...
      bizNo: row.no,
      totalPrice: -row.totalPrice,
      receiptedPrice: -(row as any).refundPrice,
      receiptPrice: -row.totalPrice + (row as any).refundPrice
    })
  })
}
