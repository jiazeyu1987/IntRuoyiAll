...
const getSummaries = (param: SummaryMethodProps) => {
  ...
  columns.forEach((column, index: number) => {
    ...
    if (key && ['totalPrice', 'paidPrice', 'paymentPrice'].includes(key)) {
...
const handleAddPurchaseIn = (rows: PurchaseInVO[]) => {
  rows.forEach((row) => {
    formData.value.push({
      ...
      paidPrice: (row as any).paymentPrice,
      paymentPrice: row.totalPrice - (row as any).paymentPrice
    })
  })
}
...
const handleAddPurchaseReturn = (rows: PurchaseReturnVO[]) => {
  rows.forEach((row) => {
    formData.value.push({
      ...
      bizNo: row.no,
      totalPrice: -row.totalPrice,
      paidPrice: -(row as any).refundPrice,
      paymentPrice: -row.totalPrice + (row as any).refundPrice
    })
  })
}
