...
const hoverConversationId = ref<number | string | null>(null) // 悬浮上去的对话
...
const deleteChatConversation = async (conversation: ChatConversationVO) => {
  try {
    ...
    // 发起删除
    await ChatConversationApi.deleteChatConversationMy(String(conversation.id))
...
watch(activeId, async (newValue) => {
  activeConversationId.value = newValue == null ? null : Number(newValue)
})
