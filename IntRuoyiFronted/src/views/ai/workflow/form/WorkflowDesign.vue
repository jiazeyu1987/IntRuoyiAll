...
const tinyflowRef = ref<any>()
const workflowData = inject('workflowData') as Ref
const showTestDrawer = ref(false)
const params4Test = ref<Array<{ key: string; value: string }>>([])
const paramsOfStartNode = ref<Record<string, any>>({})
...
watch(showTestDrawer, (value) => {
  ...
  function mergeIfRequiredButNotSet(target: Array<{ key: string; value: string }>) {
    let needPushList: Array<{ key: string; value: string }> = []
