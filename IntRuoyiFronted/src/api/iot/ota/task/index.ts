...
export const IoTOtaTaskApi = {
  ...
  // 暂停 OTA 升级任务
  pauseOtaTask: async (id: number) => {
    return await request.put({ url: `/iot/ota/task/pause?id=` + id })
  },

  // 继续 OTA 升级任务
  resumeOtaTask: async (id: number) => {
    return await request.put({ url: `/iot/ota/task/resume?id=` + id })
  }
}
