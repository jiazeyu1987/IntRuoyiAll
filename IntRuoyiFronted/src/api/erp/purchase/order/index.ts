...
// ERP 金蝶采购订单同步结果 VO
export interface KingdeePurchaseOrderSyncRespVO {
  createdCount: number // 本次创建的采购订单数量
  skippedCount: number // 本次跳过的 Kingdee 来源单数量
  createdPurchaseOrderIds: number[] // 本次创建的 IntRuoyi 采购订单编号
  skippedSourceFids: string[] // 本次跳过的 Kingdee FID
}
...
export const PurchaseOrderApi = {
  ...
  // 从 Kingdee K3Cloud 同步采购订单
  syncKingdeePurchaseOrders: async (): Promise<KingdeePurchaseOrderSyncRespVO> => {
    return await request.post({ url: `/erp/purchase-order/sync-kingdee` })
  },
