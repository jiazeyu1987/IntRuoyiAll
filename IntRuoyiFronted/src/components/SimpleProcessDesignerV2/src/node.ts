...
export function useWatchNode(props: { flowNode?: SimpleFlowNode | null }): Ref<SimpleFlowNode> {
  const node = ref<SimpleFlowNode>(props.flowNode as SimpleFlowNode)
  watch(
    () => props.flowNode,
    (newValue) => {
      node.value = newValue as SimpleFlowNode
    }
  )
  return node
}
