...
// 商品列表
const spuList = ref<Array<ProductSpuApi.Spu & { price: number | string }>>([])
watch(
  () => props.property.spuIds,
  async () => {
    const spuIds = props.property.spuIds.filter((id): id is number => id !== undefined)
    spuList.value = spuIds.length > 0
      ? ((await ProductSpuApi.getSpuDetailList(spuIds)) as Array<
          ProductSpuApi.Spu & { price: number | string }
        >)
      : []
  },
  {
