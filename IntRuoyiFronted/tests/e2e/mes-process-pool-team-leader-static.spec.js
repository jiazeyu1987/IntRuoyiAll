...
assert(
  ...
  'eDHR PQC组长页签必须锁定 PQC 工作台内容并关闭内部切换。'
)
assert(
  /<TeamLeaderWorkbenchPage[\s\S]*leader-type="PQC"[\s\S]*:show-leader-type-tabs="false"/.test(edhrTeamLeaderPage),
  '组长工作台包装页必须锁定非生产内容。'
)
assert(
  !/leader-type="PRODUCTION"|生产组长/.test(edhrTeamLeaderPage),
  '组长工作台包装页不得显示生产组长内容。'
)
