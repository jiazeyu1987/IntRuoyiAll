...
const submitAbnormalBlock = page.slice(
  ...
  page.indexOf('const resetActiveOrderForm')
)
assert(
...
assert(
  /const handleLeaderTypeChange = async[\s\S]*resetSubmissionMultiFilter\(\)/.test(page) &&
    /const resetSubmissionMultiFilter = \(\) => \{[\s\S]*updateSubmissionMultiFilterState\(\{ conditions: \[\], activeConditionId: undefined \}\)[\s\S]*submissionList\.value = \[\][\s\S]*submissionTotal\.value = 0/.test(page) &&
