...
assert.doesNotMatch(pageSource, /导出排产工艺路线/, '工作台不得继续展示“导出排产工艺路线”按钮。')
assert.doesNotMatch(pageSource, /导入排产工艺路线/, '工作台不得继续展示“导入排产工艺路线”按钮。')
assert.match(pageSource, /导出全部数据包/, '工作台必须提供“导出全部数据包”按钮。')
assert.match(pageSource, /导入全部数据包/, '工作台必须提供“导入全部数据包”按钮。')
assert.doesNotMatch(pageSource, /routeConfigInputRef/, '工作台不得保留排产工艺路线隐藏文件选择器引用。')
assert.doesNotMatch(pageSource, /handleRouteConfigFileChange/, '工作台不得保留排产工艺路线配置包上传事件。')
assert.doesNotMatch(pageSource, /exportRouteConfigPackage/, '工作台不得保留排产工艺路线导出按钮处理函数。')
assert.doesNotMatch(pageSource, /openRouteConfigImport/, '工作台不得保留排产工艺路线导入按钮处理函数。')
