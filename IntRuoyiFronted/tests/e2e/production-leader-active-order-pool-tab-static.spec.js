...
const sliceDialogByMarker = (marker) => {
  ...
  assert.notEqual(start, -1, `Expected dialog start for marker: ${marker}`)
  assert.notEqual(end, -1, `Expected dialog end for marker: ${marker}`)
  return source.slice(start, end)
}
...
assert.doesNotMatch(
  apiSource.match(/interface TeamLeaderActiveOrderAddReqVO\s*\{[\s\S]*?\}/)?.[0] || '',
