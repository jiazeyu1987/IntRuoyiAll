...
assert.match(openSelectedTemplateBody, /openSelectedTemplateDesigner\('preview'\)/, '打开按钮必须进入批记录设计器预览路径')
assert.match(editSelectedTemplateBody, /openSelectedTemplateDesigner\('edit'\)/, '编辑按钮必须进入批记录设计器编辑路径')
assert.match(fillSelectedTemplateBody, /resolveSelectedTemplateBatchRecordBinding/, '填写按钮必须先校验批记录绑定关系')
assert.match(fillSelectedTemplateBody, /router\.push/, '填写按钮必须跳转批记录模拟填写页')
