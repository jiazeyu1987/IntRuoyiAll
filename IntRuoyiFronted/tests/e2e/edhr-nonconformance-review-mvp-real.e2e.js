...
const BASE_URL = (process.env.NCR_E2E_BASE_URL || 'http://127.0.0.1:8042').replace(/\/+$/, '')
const BACKEND_URL = (process.env.NCR_E2E_BACKEND_URL || 'http://127.0.0.1:48042').replace(/\/+$/, '')
...
const PQC_FILL_PATH = '/mes/pro/feedback/edhr-batch-pqc-fill'
...
const BDD_SCENARIOS = [
  'BDD: 两个来源复用统一评审入口 -> Given 受控模拟批次可从批次放行和PQC填写进入不合格审查, When 分别点击不合格审查, Then 两次均进入同一不合格评审页面并创建同一类评审单。',
...
function validateConfig() {
  assert.equal(BASE_URL, 'http://127.0.0.1:8042', 'E2E frontend must use registered int_batch slot 1')
  assert.equal(BACKEND_URL, 'http://127.0.0.1:48042', 'E2E backend must use the paired slot 1 port')
...
async function openQaListFromMenu(page) {
  if (new URL(page.url()).pathname === REVIEW_PATH) {
    ...
    await clickVisible(page.getByRole('button', { name: '刷新' }), 'QA review refresh button')
    const response = await responsePromise
    const body = await response.json()
...
async function findSourceRouteRow(page) {
  ...
  const filter = page.locator('.table-quick-filter[data-table-key="mes.pro.route.main.admin-layout-v1"]').first()
  await filter.waitFor({ state: 'visible', timeout: 30000 })
  const fieldControl = filter.locator('.table-quick-filter__field')
  if (!/路线编码/.test((await fieldControl.innerText()).trim())) {
    await fieldControl.click()
    const routeCodeOption = page.locator('.el-select-dropdown__item:visible').filter({ hasText: '路线编码' })
    await routeCodeOption.first().waitFor({ state: 'visible', timeout: 30000 })
    await clickVisible(routeCodeOption, 'route code filter option')
  }
  await filter.locator('.table-quick-filter__value input').fill(SOURCE_ROUTE_CODE)
  const responsePromise = page.waitForResponse(
    (response) => {
      const url = new URL(response.url())
...
async function createBatch(page) {
  ...
  assert.ok(batch?.id, 'open-or-create must return batch execution id')
...
async function openExistingBatchFromList(page) {
  await openBatchListFromMenu(page)
  const filter = page.locator('.table-quick-filter[data-table-key="mes.pro.edhrBatch.execution.main"]').first()
  await filter.waitFor({ state: 'visible', timeout: 30000 })
  const fieldControl = filter.locator('.table-quick-filter__field')
  ...
  await filter.locator('.table-quick-filter__value input').fill(EXISTING_BATCH_EXECUTION_CODE)
...
async function openPqcReviewEntry(page, batch) {
  await page.goto(`${BASE_URL}${PQC_FILL_PATH}?batchExecutionId=${encodeURIComponent(batch.id)}&workOrderId=${encodeURIComponent(batch.workOrderId || '')}&workOrderCode=${encodeURIComponent(batch.workOrderCode || '')}`, {
    waitUntil: 'domcontentloaded',
    timeout: 60000
  })
  const reviewButtons = page.getByRole('button', { name: '不合格审查' })
  await reviewButtons.first().waitFor({ state: 'visible', timeout: 90000 })
  const button = await firstVisible(reviewButtons, 'PQC nonconformance review button')
  assert.equal(await button.isDisabled(), false, 'PQC nonconformance review button must receive batch context')
  await button.click()
  await page.waitForURL((url) => url.pathname === REVIEW_PATH, { timeout: 60000 })
  const url = new URL(page.url())
...
async function openBatchTrace(page, batch, traceExecution, expectedLabels) {
  ...
  const processHead = page.locator('.edhr-batch-detail__process-task-group-head').nth(traceExecution.processIndex)
...
async function run() {
  ...
  try {
    ...
    if (EXISTING_BATCH_ID) {
      ...
      sourceRouteNeedsRestore = true
      await setSourceRouteEnabled(page, true)
