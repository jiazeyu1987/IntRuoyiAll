...
assertIncludes(uploadPage, "createWorkingControlledFile", 'DCC upload must use the WORKING creation API')
assertIncludes(uploadPage, 'const submitForm = async () =>', 'DCC upload must keep the official submit handler')
assertIncludes(uploadPage, "const submitButtonText = computed(() => (isExternalReview.value ? '提交评审' : '创建工作版本'))")
