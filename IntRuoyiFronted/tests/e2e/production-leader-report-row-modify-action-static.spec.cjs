...
const markerIndex = source.indexOf('data-team-leader-correction-event-id')
assert.notEqual(markerIndex, -1, '报工管理行操作必须保留修改按钮稳定锚点。')
...
assert.match(
  source,
  /const canCorrectSubmission = \(row: ProcessPoolTimelineEventVO\) =>[\s\S]*!\(isProductionReportHistoryTab\.value \|\| isPqcFormHistoryTab\.value\)[\s\S]*\(isProductionLeader\.value \|\| !row\.released\)[\s\S]*Boolean\(row\.id\)/,
  '生产组长报工管理必须允许行级修改入口；PQC 仍按已放行状态限制修改。'
)
assert.match(
  source,
  /if \(!canCorrectSubmission\(event\)\) \{[\s\S]*ElMessage\.error\(isPqcSubmissionRow\(event\) \? 'PQC历史表单不能修改' : '当前报工不能修改'\)[\s\S]*return[\s\S]*\}/,
  'openCorrection 必须二次阻断不允许修改的行，并使用当前正式“修改”口径提示。'
)
assert.doesNotMatch(
  source,
