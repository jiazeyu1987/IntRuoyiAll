...
assert.match(page, /<el-table-column\s+label="活跃订单"\s+min-width="360"[\s\S]*class="team-leader-workbench__allocation-order-select"/)
...
assert.match(page, /<el-table-column\s+label="分配数量"\s+min-width="340"/)
...
assert.match(page, /\.team-leader-workbench__allocation-quantity-cell[\s\S]*grid-template-columns:\s*minmax\(100px, 1fr\) repeat\(3, max-content\)/)
