...
const { createRunId, buildManifest, FIXED_TEMPLATE_WORK_ORDER_CODE, FIXED_TEMPLATE_QUANTITY } = require('./edhr-ai-loop/manifest.cjs')
...
const fixedTemplateManifest = buildManifest({ runId, mode: 'full' })
assert.equal(fixedTemplateManifest.templateWorkOrderCode, FIXED_TEMPLATE_WORK_ORDER_CODE)
assert.equal(fixedTemplateManifest.templateSource, 'FIXED_ACTIVE_ORDER_COPY')
assert.equal(fixedTemplateManifest.orders[0].sourceWorkOrderCode, FIXED_TEMPLATE_WORK_ORDER_CODE)
assert.equal(fixedTemplateManifest.orders[0].workOrderCode, null)
assert.equal(fixedTemplateManifest.orders[0].batchCode, null)
assert.equal(fixedTemplateManifest.orders[0].simulationRunId, `${runId}-O01`)
...
assert.equal(manifest.orders[0].sourceWorkOrderCode, 'EDHR-E2E-TEMPLATE-WO')
assert.equal(manifest.orders[0].quantity, FIXED_TEMPLATE_QUANTITY)
...
const failure = writeFailureArtifacts({ rootDir: output, runId: createRunId(new Date('2026-09-15T04:00:02.000Z'), 'A1B4'),
  ...
  message: '缺少固定母单', pageUrl: '/login', stages: [{ stage: 'S01', status: 'PASS' }] })
