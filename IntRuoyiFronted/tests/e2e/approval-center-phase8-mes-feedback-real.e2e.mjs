...
const config = {
  ...
  targetPath: '/approval-center?moduleCode=MES_FEEDBACK&viewType=TODO',
  headed: process.env.APPROVAL_CENTER_E2E_HEADED === '1',
  taskDir:
    process.env.APPROVAL_CENTER_TASK_DIR ||
    'D:/ProjectPackage/Int/IntRuoyi/doc/tasks/20260624-unified-approval-platform-phase8/e2e-artifacts'
}

const screenshots = {
  loginFailed: path.join(config.taskDir, 'phase8-mes-login-failed.png'),
  center: path.join(config.taskDir, 'phase8-mes-approval-center.png'),
  timeline: path.join(config.taskDir, 'phase8-mes-timeline.png'),
  detail: path.join(config.taskDir, 'phase8-mes-detail.png')
}
...
async function waitForMesApprovalPage(page) {
  ...
  assert.ok(new URL(tasksResponse.response.url()).origin === new URL(config.baseUrl).origin && new URL(tasksResponse.response.url()).pathname === '/admin-api/approval-center/tasks/page', `tasks request did not hit target backend: ${tasksResponse.response.url()}`)
...
function selectMesFeedbackTask(pageData) {
  const list = Array.isArray(pageData?.list) ? pageData.list : []
  const rowIndex = list.findIndex(
    (item) =>
      item.moduleCode === 'MES_FEEDBACK' &&
      item.sourceTaskType === 'MES_PRO_FEEDBACK' &&
      item.detailRoute === '/mes/pro/feedback' &&
      item.detailQuery?.feedbackId &&
      Array.isArray(item.capabilities) &&
      item.capabilities.includes('TIMELINE') &&
      item.capabilities.includes('AUDIT') &&
      !item.requiresSignature &&
      !item.businessDeleted
  )
  if (rowIndex < 0) {
    throw new Error(
      `mes_feedback_real_todo_missing:${JSON.stringify({
...
async function openTimeline(page, rowIndex) {
  ...
  const timelineResponsePromise = waitForJsonResponse(
    page,
    (response) =>
      response.url().includes('/admin-api/approval-center/tasks/timeline') &&
      response.url().includes('moduleCode=MES_FEEDBACK') &&
      response.request().method() === 'GET',
    { timeout: 60000 }
  )
  ...
  assert.ok(new URL(timelineResponse.response.url()).origin === new URL(config.baseUrl).origin && new URL(timelineResponse.response.url()).pathname === '/admin-api/approval-center/tasks/timeline', `timeline request did not hit target backend: ${timelineResponse.response.url()}`)
  const timelineRows = extractData(timelineResponse.payload)
  assert.ok(Array.isArray(timelineRows) && timelineRows.length > 0, 'MES feedback timeline rows must not be empty')
  assert.ok(
    timelineRows.every((entry) => entry.evidenceType === 'MES_PRO_FEEDBACK' || entry.evidenceType === 'MES_PRO_FEEDBACK_APPROVAL'),
    `MES feedback timeline evidence type mismatch: ${JSON.stringify(timelineRows)}`
  )
...
async function jumpToMesFeedbackPage(page, rowIndex, task) {
  await page.keyboard.press('Escape').catch(() => null)
  await settle(page, 5000)
  const rows = page.locator('.el-table__body-wrapper tbody tr')
  await rows.nth(rowIndex).getByRole('button', { name: zh.open }).click()
  await page.waitForURL((url) => url.pathname === '/mes/pro/feedback', { timeout: 60000 })
  await settle(page, 30000)
  await page.screenshot({ path: screenshots.detail, fullPage: true })
  const detailUrl = new URL(page.url())
  assert.equal(detailUrl.searchParams.get('feedbackId'), String(task.detailQuery.feedbackId))
  const bodyText = await page.locator('body').innerText({ timeout: 10000 }).catch(() => '')
  assert.doesNotMatch(bodyText, /404|Not Found/i, `MES feedback page must not be 404: ${bodyText.slice(0, 1000)}`)
}
...
async function main() {
  ...
  try {
    await login(page)
    const pageData = await waitForMesApprovalPage(page)
    const { rowIndex, task } = selectMesFeedbackTask(pageData)
    const timelineRows = await openTimeline(page, rowIndex)
    await jumpToMesFeedbackPage(page, rowIndex, task)
    assert.deepEqual(pageErrors, [], `page errors: ${pageErrors.join(' || ')}`)
    process.stdout.write(
      `approval-center phase8 MES feedback real e2e passed\n${JSON.stringify(
        {
