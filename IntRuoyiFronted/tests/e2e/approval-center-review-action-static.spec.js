...
assert.match(view, />\s*审核\s*<\/el-button>/, '待办列表操作列必须提供统一审核按钮文案')
assert.match(view, /openReviewAction\(row\)/, '审核按钮必须统一分发直接审核和模块审核入口')
assert.match(view, /openReviewDialog\(row\)/, '支持直接审核的任务必须打开审批中心审核弹窗')
