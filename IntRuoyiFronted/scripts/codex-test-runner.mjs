...
For 工艺路线版本发布 opening the version workspace, after quick-filtering TN-ROUTE-VERSION-001, click only the visible operation-column/fixed-right row action named 版本 in the table body row containing TN-ROUTE-VERSION-001. Do not call global clickVisibleTextAction(/版\\s*本/) or page.getByText('版本'), because those can hit the footer/global version-info entry. Exclude Copyright/footer/header/sidebar text and any 版本信息 or 版本变更说明 trigger from this action lookup. Do not treat the row operation button data-testid="route-version-workspace" as the opened workspace; that testid is on the list row 版本 button. After clicking, scope the workspace locator to the visible 工艺路线版本 dialog body .route-version-workspace__body, or to the visible 工艺路线版本 dialog that contains .route-version-workspace__summary. wait until the dialog body contains 创建候选版本 or 候选版本工作区 or 当前 ACTIVE before calling it open; workspace text of only 版本 means you selected the row button, not the opened workspace. If a 版本变更说明 or 版本信息未生成 overlay appears, close it and report BLOCKED as wrong global version-info action instead of treating the route version workspace as open.
...
For Element Plus row operation link-buttons such as 删除, 编辑, 复制, 版本, or 删除草稿, text is often inside a span rendered by a link-style button. Include span text candidates, climb to closest('button,.el-button,[role="button"],a') || element, and click the resolved ElementHandle directly after checking its DOM state with elementHandle.evaluate. Do not wrap an ElementHandle from evaluateHandle/elementHandle in page.locator(':scope').locator(handle), because Playwright locators cannot be built from ElementHandle objects that way. A resolved BUTTON.el-button--danger.is-link is a valid row action; do not report action handle unavailable for it when it is visible and not disabled/loading. Never report a route delete entry missing while the visible row text contains 删除; instead include the resolved element tag/class/disabled state if the action cannot be clicked.
Do not use locator.filter({ hasText: actionRegex }) with an anchored row-action regex as the only discovery path. In the real Element Plus route table, an anchored regex such as /^复\\s*制$/ can yield zero filtered candidates even while the visible button innerText/textContent is exactly 复制. Instead, collect visible button/.el-button/[role="button"]/a/span candidates first, then test normalized innerText/textContent in Node with actionRegex.test(...). For the visible RT000028 source row, resolve the visible 复制 text/span to closest('button,.el-button,[role="button"],a'), verify the resolved element is visible and enabled, and click that ElementHandle directly. If the action is rendered in a duplicated fixed-right table rather than inside the same row locator, match duplicated fixed-right actions by the target row Y-coordinate when the action is outside the same DOM tr. Never return "复制 action unavailable: visible action text was not resolved to an enabled action element" before attempting the resolved enabled ElementHandle directly.
...
For 工艺路线状态删除 enable/disable verification, prepare TN-ROUTE-STATUS-001 by copying the complete source route RT000028 / 球囊扩张压力泵, then rename the copied route to 测试节点-工艺路线-状态删除 if the copy dialog asks for the target name. Do not create a blank route for the status-delete node, because blank routes do not have composed processes and cannot be enabled. If enabling shows 请先添加组成工序, that means the generated setup used an invalid blank route; treat it as a script/setup failure and recreate the fixed route by copying RT000028 instead of reporting the business status switch as broken. The route list status control is the 状态 column el-switch, not an operation-column text button. The row may show only 产品/编辑/复制/版本/删除 in the operation column; do not report 停用入口不可见 or 启用入口不可见 just because no text button exists. Locate the fixed route row TN-ROUTE-STATUS-001, find its status-column .el-switch / [role="switch"] / .el-switch__core, and click the switch to toggle. Confirm the Element Plus message box text for 启用 or 停用 and wait for the /admin-api/mes/pro/route/update-status response with HTTP success and business code 0 before judging the refreshed state. Judge enabled/disabled state from the target row switch checked state, aria-checked, .is-checked class, input checked value, or active/inactive value after the list refresh; do not require visible row text 启用 or 停用 and do not use the first switch in the table if it is not the TN-ROUTE-STATUS-001 row. If the switch starts inactive, click it to enable first, then optionally disable and enable again so checkpoint 3 can prove the route is enabled before deletion. Only block if the status switch is missing or disabled despite mes:pro-route:update permission.
For 工艺路线基础维护 checkpoint 1 reset, if TN-ROUTE-BASIC-001 is visible before the test, click that row 删除 action, confirm the Element Plus message box, then re-query by route code until no visible table body row remains. Never mark checkpoint 1 FAIL merely because a stale fixed route row exists before attempting the delete; FAIL/BLOCKED is valid only after the row-scoped delete action was attempted, the confirmation was handled, and the refreshed visible table body still contains TN-ROUTE-BASIC-001 with diagnostics.
...
Scope quick-filter interactions to the visible .table-quick-filter or .unified-list-template__quick-filter. Do not scan all page .el-select controls when determining the quick-filter field because unrelated dialogs and page filters may have other selected values. Read the selected field only from .table-quick-filter__field, fill the query text only inside .table-quick-filter__value, and click the 查询 button inside or nearest to the same quick-filter container. After switching 路线编码/路线名称, re-read .table-quick-filter__field before filling. After clicking 查询/搜索, do not read table body rows while an Element Plus loading mask or spinner is visible; wait until .el-loading-mask is hidden or removed before judging rows, and also wait for the route page/list request caused by that query when possible. If the submitted quick-filter value is TN-ROUTE-VERSION-001 but the visible row still contains RT000028, treat it as a stale loading row and keep waiting or re-run the same target quick-filter query; do not report the copied route missing while the page is visibly loading stale source results. Do not throw just because quick-filter option 路线名称 is unavailable; use the current visible field with the matching fixed code/name value. if the visible field is 路线名称, fill the route name, never the route code; if the visible field is 路线编码, fill the route code, never the route name.
...
When a row helper returns a wrapper object such as { row, text, index } or matchedRow, call locator methods only on the wrapper row property. Never call matchedRow.locator(...) unless matchedRow is already a Playwright Locator. If matchedRow is a wrapper, use matchedRow.row.locator(...). If unsure, normalize with const rowLocator = matchedRow?.locator ? matchedRow : matchedRow?.row before calling rowLocator.locator(...), and return BLOCKED only if neither is a Playwright Locator.
