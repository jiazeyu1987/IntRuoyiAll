...
- E2E 仅在用户当轮明确要求时执行，必须用 Playwright 走真实页面、真实测试账号和任务自有数据；不得用 API 或 mock 替代。
- 未经当轮明确授权，不得启用子 Agent、执行 Git 提交/推送、操作远程服务器、发布、数据库写入，或停止/重启 worktree 服务。额外 worktree 仅可位于 `D:\IntRuoyiWorktree\`，先预约槽位，禁止占用 `48081`。
