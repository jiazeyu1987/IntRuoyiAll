...
- Additional worktrees must use an atomically reserved profile slot in `1..19`; reserve it with `scripts\runtime\reserve-worktree-slot.ps1` before starting either service.
...
- After creating an additional worktree and before starting frontend or backend, run `scripts\runtime\reserve-worktree-slot.ps1`; `slot >= 20`, base-port collisions, duplicate active profile slots, and duplicate active ports must fail fast.
