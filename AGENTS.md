...
- PowerShell 禁用 `&&`，中文统一 UTF-8；执行 Maven 等命令时，`-D` 系统属性必须作为一个完整且通常带单引号的参数（如 `'-DskipTests'`、`'-Dkey=value'`），不得把 `-D` 与属性名拆开；只处理当前任务资产，不动并行或无关改动。回复简洁、面向业务；发现需求有误须先核对并指出。
- Thread baseline: eDHR生产主链路以活跃订单为主，依次经过生产组长、一线生产、一线PQC、双100%完工、PQC生产放行和上市放行；eDHR批次执行仅承载下游批记录与证据关联。
